...
private:
    ...
    void globalSuppressions() { // Testing that Cppcheck::useGlobalSuppressions works (#8515)
        ...
        ASSERT_EQUALS(0, cppCheck.checkBuffer(FileWithDetails("test.c"), reinterpret_cast<const std::uint8_t*>(code), sizeof(code))); // <- no unsuppressed error is seen
    ...
    void suppressionWithRelativePaths() {
        ...
        const char code[] =
            ...
            "    int y;\n"
            "};";
        ASSERT_EQUALS(0, cppCheck.checkBuffer(FileWithDetails("/somewhere/test.cpp"), reinterpret_cast<const std::uint8_t*>(code), sizeof(code)));
        ASSERT_EQUALS("",errout_str());
    }
