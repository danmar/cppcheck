...
#define check(...) check_(__FILE__, __LINE__, __VA_ARGS__)
    void check_(const char* file, int line, const char* code, bool inconclusive = false, bool portability = false, cppcheck::Platform::PlatformType platform = cppcheck::Platform::Unspecified, bool onlyFormatStr = false, bool cpp = true) {
    ...
    void wrongMode_simple() {
        ...
        check("void foo(FILE*& f) {\n"
              ...
              "    rewind(f);\n"
              "    fwrite(buffer, 5, 6, f);\n"
              "}", false, false, cppcheck::Platform::Win32W);
        ASSERT_EQUALS("[test.cpp:5]: (error) Write operation on a file that was opened only for reading.\n", errout.str());
        ...
        check("void foo(FILE*& f) {\n"
              ...
              "    rewind(f);\n"
              "    fwrite(buffer, 5, 6, f);\n"
              "}", false, false, cppcheck::Platform::Win32A);
        ASSERT_EQUALS("[test.cpp:5]: (error) Write operation on a file that was opened only for reading.\n", errout.str());
...
#define TEST_PRINTF_ERR_AKA_CPP(format, requiredType, actualType, akaType) \
    ...
    void testFormatStrNoWarn(const char *filename, unsigned int linenr, const char* code,
                             bool cpp = false) {
        check(code, true, false, cppcheck::Platform::Unix32, true, cpp);
        assertEquals(filename, linenr, emptyString, errout.str());
        check(code, true, false, cppcheck::Platform::Unix64, true, cpp);
        assertEquals(filename, linenr, emptyString, errout.str());
        check(code, true, false, cppcheck::Platform::Win32A, true, cpp);
        assertEquals(filename, linenr, emptyString, errout.str());
        check(code, true, false, cppcheck::Platform::Win64, true, cpp);
        assertEquals(filename, linenr, emptyString, errout.str());
    }
    ...
    void testFormatStrWarnAka(const char *filename, unsigned int linenr,
                              ...
                              bool cpp = false) {
        check(code, true, true, cppcheck::Platform::Unix32, true, cpp);
        assertEquals(filename, linenr, testScanfErrAkaString, errout.str());
        check(code, true, true, cppcheck::Platform::Unix64, true, cpp);
        assertEquals(filename, linenr, testScanfErrAkaString, errout.str());
        check(code, true, true, cppcheck::Platform::Win32A, true, cpp);
        assertEquals(filename, linenr, testScanfErrAkaString, errout.str());
        check(code, true, true, cppcheck::Platform::Win64, true, cpp);
...
#define TEST_PRINTF_WARN_AKA_CPP_WIN32(FORMAT, FORMATSTR, TYPE, AKATYPE_WIN32) \
    ...
    void testScanfArgument() {
...

        {
            ...
            check(code, false, true, cppcheck::Platform::Unix32);
            ASSERT_EQUALS(result, errout.str());
            check(code, false, true, cppcheck::Platform::Unix64);
            ASSERT_EQUALS(result, errout.str());
            check(code, false, true, cppcheck::Platform::Win32A);
            ASSERT_EQUALS(result, errout.str());
            check(code, false, true, cppcheck::Platform::Win64);
    ...
    void testPrintfArgument() {
        ...
        check("void foo(size_t s, ptrdiff_t p) {\n"
              "  printf(\"%zd\", s);\n"
              "  printf(\"%tu\", p);\n"
              "}", false, true, cppcheck::Platform::Unix32);
        ...
        check("void foo(size_t s, ptrdiff_t p) {\n"
              "  printf(\"%zd\", s);\n"
              "  printf(\"%tu\", p);\n"
              "}", false, true, cppcheck::Platform::Unix64);
        ASSERT_EQUALS("[test.cpp:2]: (portability) %zd in format string (no. 1) requires 'ssize_t' but the argument type is 'size_t {aka unsigned long}'.\n"
                      "[test.cpp:3]: (portability) %tu in format string (no. 1) requires 'unsigned ptrdiff_t' but the argument type is 'ptrdiff_t {aka signed long}'.\n", errout.str());
        ...
        check("void foo(size_t s, ptrdiff_t p) {\n"
              "  printf(\"%zd\", s);\n"
              "  printf(\"%tu\", p);\n"
              "}", false, true, cppcheck::Platform::Win32A);
        ASSERT_EQUALS("[test.cpp:2]: (portability) %zd in format string (no. 1) requires 'ssize_t' but the argument type is 'size_t {aka unsigned long}'.\n"
                      "[test.cpp:3]: (portability) %tu in format string (no. 1) requires 'unsigned ptrdiff_t' but the argument type is 'ptrdiff_t {aka signed long}'.\n", errout.str());
        ...
        check("void foo(size_t s, ptrdiff_t p) {\n"
              "  printf(\"%zd\", s);\n"
              "  printf(\"%tu\", p);\n"
              "}", false, true, cppcheck::Platform::Win64);
        ...
        check("class A : public std::vector<std::string> {} a;\n"
              ...
              "    printf(\"%c %p\", b[0], b[0]);\n"
              "    printf(\"%c %p\", s[0], s[0]);\n"
              "}\n", false, false, cppcheck::Platform::Unix64);
        ...
        check("void foo (wchar_t c) {\n" // ticket #5051 false positive
              "    printf(\"%c\", c);\n"
              "}\n", false, false, cppcheck::Platform::Win64);
        ASSERT_EQUALS("", errout.str());
    ...
    void testMicrosoftSecurePrintfArgument() {
        ...
        check("void foo() {\n"
              ...
              "}\n", true, false, cppcheck::Platform::Win32A);
