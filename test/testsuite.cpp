...
struct CompareFixtures {
    bool operator()(const TestFixture* lhs, const TestFixture* rhs) const { return lhs->classname < rhs->classname; }
};
} // namespace
...
public:
    ...
    void addTest(TestFixture* t) { _tests.insert(t); }

    const TestSet& tests() const { return _tests; }
};
...
TestFixture::TestFixture(const char* const _name) : mVerbose(false), exename(), quiet_tests(false), classname(_name)
{
...
void TestFixture::assertEqualsFailed(const char* const filename,
...
void TestFixture::assertEqualsWithoutLineNumbers(const char* const filename,
                                                 const unsigned int linenr,
                                                 const std::string& expected,
                                                 const std::string& actual,
                                                 const std::string& msg) const
...
void TestFixture::assertEqualsDouble(const char* const filename,
                                     const unsigned int linenr,
...
void TestFixture::todoAssertEquals(const char* const filename,
                                   const unsigned int linenr,
...
void TestFixture::complainMissingLib(const char* const libname) const { missingLibs.insert(libname); }
...
void TestFixture::reportOut(const std::string& outmsg, Color) { output << outmsg << std::endl; }
