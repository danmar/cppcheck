...
public:
    TestPreprocessor() : TestFixture("TestPreprocessor"), preprocessor0(settings0, this)
    {
...
private:
    ...
    void preprocess(const char* code, std::map<std::string, std::string>& actual, const char filename[] = "file.c")
    {
        ...
        for (const std::string& config : configs) {
            try {
                const std::string& cfgcode =
                    preprocessor0.getcode(tokens, config, files, std::string(code).find("#file") != std::string::npos);
    ...
    void macro_simple17() const
    { // # 5074 - the Token::isExpandedMacro() doesn't always indicate properly if token comes from macro
    ...
    void macroInMacro1() const
    {
        ...
        {
            const char filedata[] = "#define B(A1, A2) } while (0)\n"
        ...
        {
            const char filedata[] = "#define B(x) (\n"
                                    "#define A() B(xx)\n"
                                    "B(1) A() ) )\n";
            ASSERT_EQUALS("\n\n( ( ) )", OurPreprocessor::expandMacros(filedata));
        }
        ...
        {
            const char filedata[] = "#define PTR1 (\n"
    ...
    */
    ...
    void dup_defines()
    {
        ...
        ASSERT_EQUALS_MSG(
            true, (actual.find("A") != actual.end()), "A is expected to be checked but it was not checked");

        ASSERT_EQUALS_MSG(
            true, (actual.find("A;A;B") == actual.end()), "A;A;B is expected to NOT be checked but it was checked");
    }
    ...
    void predefine5()
    { // #3737, #5119 - automatically define __cplusplus
        // #3737...
