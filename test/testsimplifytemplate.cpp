...
private:
    ...
    void template8()
    {
        ...
        ASSERT_EQUALS(
            ...
            "template < typename T > B < T > h ( ) { return B < T > ( ) ; }",
            tok(code));
    ...
    void template26()
    {
        ...
        const char code[] = "template<class T>\n"
                            ...
                            "C<2> a;\n";
        ASSERT_EQUALS(
            "class A<char[2]> ; class C<2> ; C<2> a ; class C<2> : public A<char[2]> { } ; class A<char[2]> { public: char [ 2 ] x ; } ;",
            tok(code));
    }
    ...
    void template32()
    {
        ...
        ASSERT_EQUALS("template < class T1 , class T2 , class T3 , class T4 > struct A { } ; "
                      "struct B<int> ; "
                      "B<int> b ; "
                      "struct B<int> { public: A < int , Pair < int , int > , int > a ; } ;",
                      tok(code));
    }
    ...
    void template36()
    { // #4310 - Passing unknown template instantiation as template argument
    ...
    { // #4544 - A<class B> a;
        {
            const char code[] = "class A { };\n"
                                ...
                                "B<class A> b1;\n"
                                "B<A> b2;";
            ASSERT_EQUALS("class A { } ; class B<A> ; B<A> b1 ; B<A> b2 ; class B<A> { } ;", tok(code));
        }
        {
            const char code[] = "struct A { };\n"
                                "template<class T> class B {};\n"
                                "B<struct A> b1;\n"
                                "B<A> b2;";
            ASSERT_EQUALS("struct A { } ; class B<A> ; B<A> b1 ; B<A> b2 ; class B<A> { } ;", tok(code));
        }
        {
            const char code[] = "enum A { };\n"
                                ...
                                "B<enum A> b1;\n"
                                "B<A> b2;";
            ASSERT_EQUALS("enum A { } ; class B<A> ; B<A> b1 ; B<A> b2 ; class B<A> { } ;", tok(code));
        }
    }
    ...
    void template38()
    { // #4832 - Crash on C++11 right angle brackets
    ...
    void template40()
    { // #5055 - false negatives when there is template specialization outside struct
        const char code[] = "struct A {"
    ...
    void template41()
    { // #4710 - const in template instantiation not handled perfectly
        ...
        ASSERT_EQUALS("int f<int> ( int t ) ; "
                      "int x ( ) { return f<int> ( 123 ) ; } "
                      "int f<int> ( int t ) { return t ; }",
                      tok(code2));
    }

    void template42()
    { // #4878 cppcheck aborts in ext-blocks.cpp (clang testcode)
    ...
    void template43()
    { // #5097 - Assert due to '>>' in 'B<A<C>>' not being treated as end of template instantiation
    ...
    { // #6604
        ...
        ASSERT_EQUALS("template < class T > class AtSmartPtr : public ConstCastHelper < AtSmartPtr < const T > , T > { "
    ...
    void template61()
    { // hang in daca, code extracted from kodi
    ...
    void template133()
    {
        ...
        const char act[] =
            "struct bar<short> ; struct bar<std::array<int,4>> ; "
            "bar<short> bs ; "
            "bar<std::array<int,4>> ba ; "
    ...
    { // unknown macro
        ...
        ASSERT_THROW_EQUALS(
            tok(code),
            InternalError,
            "There is an unknown macro here somewhere. Configuration is required. If BEGIN_VERSIONED_NAMESPACE_DECL is a macro then please configure it.");
    }
    ...
    void template_specialization_1()
    { // #7868 - template specialization template <typename T> struct S<C<T>> {..};
        const char code[] = "template <typename T> struct C {};\n"
                            "template <typename T> struct S {a};\n"
                            "template <typename T> struct S<C<T>> {b};\n"
                            "S<int> s;";
        const char exp[] =
            "template < typename T > struct C { } ; struct S<int> ; template < typename T > struct S < C < T > > { b } ; S<int> s ; struct S<int> { a } ;";
        ASSERT_EQUALS(exp, tok(code));
    }

    void template_specialization_2()
    { // #7868 - template specialization template <typename T> struct S<C<T>> {..};
        const char code[] = "template <typename T> struct C {};\n"
                            "template <typename T> struct S {a};\n"
                            "template <typename T> struct S<C<T>> {b};\n"
                            "S<C<int>> s;";
        const char exp[] =
            "template < typename T > struct C { } ; template < typename T > struct S { a } ; struct S<C<int>> ; S<C<int>> s ; struct S<C<int>> { b } ;";
        ASSERT_EQUALS(exp, tok(code));
    }
    ...
    void template_default_parameter()
    {
        ...
        }
        {
            // #7548
            const char code[] =
                "template<class T, class U> class DefaultMemory {}; "
    ...
    void template_default_type()
    {
        ...
        // ASSERT_EQUALS("[file1.cpp:15]: (error) Internal error: failed to instantiate template. The checking continues
        // anyway.\n", errout.str());
        ASSERT_EQUALS("", errout.str());
    }
    ...
    void template_namespace_6()
    {
        ...
        ASSERT_EQUALS("namespace NS { "
                      ...
                      "NS :: C<int> & NS :: C<int> :: operator= ( const NS :: C<int> & ) { }",
                      tok(code));
    }

    void template_namespace_7()
    { // #8768
        ...
        ASSERT_EQUALS("namespace N1 { "
                      ...
                      "} "
                      "N1 :: N2 :: C c3 ; "
                      "N1 :: N2 :: CT<int> ct3 ; struct N1 :: N2 :: CT<int> { } ;",
                      tok(code));
    }

    void template_namespace_8()
    { // #8768
        const char code[] = "namespace NS1 {\n"
                            "namespace NS2 {\n"
                            "    template <typename T>\n"
    ...
    { // #7145
        ...
        ASSERT_EQUALS("namespace MyNamespace { "
                      ...
                      "} ; "
                      "} int MyNamespace :: TestClass :: TemplatedMethod<int> ( int t ) { return t ; }",
                      tok(code));
    }
    ...
    void template_array_type()
    {
        ...
        //   test.cpp:2:33: error: Syntax Error: AST broken, binary operator '>' doesn't have two operands.
        //   [internalAstError]
    ...
    void templateParameters()
    {
        ...
        ASSERT_EQUALS(
            2U, templateParameters("template<template<typename>...Foo,template<template<template<typename>>>> x;"));
        ASSERT_EQUALS(
            3U, templateParameters("template<template<typename>...Foo,int,template<template<template<typename>>>> x;"));
        ASSERT_EQUALS(
            4U,
            templateParameters("template<template<typename>...Foo,int,template<template<template<typename>>>,int> x;"));
        ASSERT_EQUALS(
            2U,
            templateParameters("template<typename S, enable_if_t<(is_compile_string<S>::value), int>> void i(S s);"));
    ...
    void templateNamePosition()
    {
        ...
        ASSERT_EQUALS(4,
                      templateNamePositionHelper(
                          "template<class T> std::string foo() { static str::string str; return str; }", 4));
        ASSERT_EQUALS(5,
                      templateNamePositionHelper(
                          "template<class T> std::string & foo() { static str::string str; return str; }", 4));
        ASSERT_EQUALS(6,
                      templateNamePositionHelper(
                          "template<class T> const std::string & foo() { static str::string str; return str; }", 4));

        ASSERT_EQUALS(9,
                      templateNamePositionHelper(
                          "template<class T> std::map<int, int> foo() { static std::map<int, int> m; return m; }", 4));
        ASSERT_EQUALS(
            10,
            templateNamePositionHelper(
                "template<class T> std::map<int, int> & foo() { static std::map<int, int> m; return m; }", 4));
        ASSERT_EQUALS(
            11,
            templateNamePositionHelper(
                "template<class T> const std::map<int, int> & foo() { static std::map<int, int> m; return m; }", 4));
        // Class template members
        ASSERT_EQUALS(4,
                      templateNamePositionHelper("class A { template<class T> unsigned foo(); }; "
                                                 "template<class T> unsigned A::foo() { return 0; }",
                                                 19));
        ASSERT_EQUALS(5,
                      templateNamePositionHelper("class A { template<class T> const unsigned foo(); }; "
                                                 "template<class T> const unsigned A::foo() { return 0; }",
                                                 20));
        ASSERT_EQUALS(7,
                      templateNamePositionHelper("class A { class B { template<class T> const unsigned foo(); }; } ; "
        ...
        ASSERT_EQUALS(6,
                      templateNamePositionHelper("template<class T> class A { A(); }; "
                                                 "template<class T> A<T>::A() {}",
                                                 18));
        ASSERT_EQUALS(8,
                      templateNamePositionHelper("template<class T, class U> class A { A(); }; "
                                                 "template<class T, class U> A<T, U>::A() {}",
                                                 24));
        ASSERT_EQUALS(7,
                      templateNamePositionHelper("template<class T> class A { unsigned foo(); }; "
                                                 "template<class T> unsigned A<T>::foo() { return 0; }",
                                                 19));
        ASSERT_EQUALS(9,
                      templateNamePositionHelper("template<class T, class U> class A { unsigned foo(); }; "
                                                 "template<class T, class U> unsigned A<T, U>::foo() { return 0; }",
                                                 25));
        ASSERT_EQUALS(12, templateNamePositionHelper("template<> unsigned A<int, v<char> >::foo() { return 0; }", 2));
    }
    ...
    void findTemplateDeclarationEnd()
    {
        ...
        ASSERT(findTemplateDeclarationEndHelper(
            "template <typename T> constexpr auto func = [](auto x){ return T(x);}; int x;", "; int x ;"));
        ASSERT(findTemplateDeclarationEndHelper(
            "template <class, class a> auto b() -> decltype(a{}.template b<void(int, int)>); int x;", "; int x ;"));
        ASSERT(findTemplateDeclarationEndHelper(
            "template <class, class a> auto b() -> decltype(a{}.template b<void(int, int)>){} int x;", "} int x ;"));
        ASSERT(findTemplateDeclarationEndHelper(
            "template <typename... f, c<h<e<typename f::d...>>::g>> void i(); int x;", "; int x ;"));
        ASSERT(findTemplateDeclarationEndHelper(
            "template <typename... f, c<h<e<typename f::d...>>::g>> void i(){} int x;", "} int x ;"));
    }
    ...
    bool getTemplateParametersInDeclarationHelper(const char code[], const std::vector<std::string>& params)
    {
        ...
        TemplateSimplifier::getTemplateParametersInDeclaration(tokenizer.tokens()->tokAt(2),
    ...
    void expandSpecialized1()
    {
        ...
        ASSERT_EQUALS("class A<int> { A<int> ( ) { } ~ A<int> ( ) { } } ;",
                      tok("template<> class A<int> { A() {} ~A() {} };"));
        ASSERT_EQUALS(
            "class A<int> { A<int> ( ) ; ~ A<int> ( ) ; } ; A<int> :: A<int> ( ) { } ~ A<int> :: A<int> ( ) { }",
            tok("template<> class A<int> { A(); ~A(); }; A<int>::A() { } ~A<int>::A() {}"));
        ASSERT_EQUALS(
            "class A<int> { A<int> ( ) ; A<int> ( const A<int> & ) ; A<int> foo ( ) ; } ; A<int> :: A<int> ( ) { } A<int> :: A<int> ( const A<int> & ) { } A<int> A<int> :: foo ( ) { A<int> a ; return a ; }",
    ...
    { // #8671
        const char code[] = "template <> struct OutputU16<unsigned char> final {\n"
                            ...
                            "};";
        const char expected[] =
            "struct OutputU16<unsignedchar> final { "
    ...
    {
        // Ticket #8175
        ASSERT_EQUALS(false, instantiateMatch("ConvertHelper < From, To > c ;", 2, ":: %name% ("));
        ASSERT_EQUALS(true, instantiateMatch("ConvertHelper < From, To > :: Create ( ) ;", 2, ":: %name% ("));
        ASSERT_EQUALS(
            false,
            instantiateMatch(
                "integral_constant < bool, sizeof ( ConvertHelper < From, To > :: Create ( ) ) > ;", 2, ":: %name% ("));
        ASSERT_EQUALS(
            false,
    ...
    void templateParameterWithoutName()
    {
        ...
        ASSERT_EQUALS(1U,
                      templateParameters("template<template<typename = float> typename T> struct A {\n"
                                         "    void f();\n"
                                         "    void g();\n"
                                         "};n"));
    }

    void templateTypeDeduction1()
    { // #8962
        const char code[] = "template<typename T>\n"
                            "void f(T n) { (void)n; }\n"
                            "static void func() {\n"
    ...
    void simplifyTemplateArgs1()
    {
        ...
        ASSERT_EQUALS("foo<2,2> = 4 ; foo<2,2> ;",
                      tok("template<int N, int M> foo = N * M; foo < ( 1 + 1 ), ( 1 + 1 ) >;"));

        ASSERT_EQUALS("foo<true> = true ; foo<true> ;", tok("template<bool N> foo = N; foo < true ? true : false >;"));
        ASSERT_EQUALS("foo<false> = false ; foo<false> ;",
                      tok("template<bool N> foo = N; foo < false ? true : false >;"));
        ...
        ASSERT_EQUALS("foo<true> = true ; foo<true> ;",
                      tok("template<bool N> foo = N; foo < (1 + 1 ) ? true : false >;"));
        ASSERT_EQUALS("foo<false> = false ; foo<false> ;",
                      tok("template<bool N> foo = N; foo < ( 1 - 1) ? true : false >;"));
    }
    ...
    void fold_expression_1()
    {
        ...
        const char expected[] =
            "template < typename ... Args > bool all ( Args ... args ) { return ( __cppcheck_fold_&&__ ( args ... ) ) ; } x = all ( true , false , true , true ) ;";
        ASSERT_EQUALS(expected, tok(code));
    }
    ...
    void fold_expression_3()
    {
        const char code[] = "template<typename... Args> int foo(Args... args) { return (12 * ... * args); }\n"
                            "x=foo(1,2);";
        const char expected[] =
            "template < typename ... Args > int foo ( Args ... args ) { return ( __cppcheck_fold_*__ ( args ... ) ) ; } x = foo ( 1 , 2 ) ;";
        ASSERT_EQUALS(expected, tok(code));
    }
    ...
    void requires5()
    {
        const char code[] = "template <class T>\n"
                            ...
                            "add<int>(123,456);";
        const char expected[] =
            "int add<int> ( int a , int b ) ; add<int> ( 123 , 456 ) ; int add<int> ( int a , int b ) { return a + b ; }";
        ASSERT_EQUALS(expected, tok(code));
    }
