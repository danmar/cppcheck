...
#define tok(...) tok_(__FILE__, __LINE__, __VA_ARGS__)
    std::string tok_(const char* file, int line, const char code[], bool simplify = true, cppcheck::Platform::PlatformType type = cppcheck::Platform::Native) {
        errout.str("");
...
#define tokenizeAndStringify(...) tokenizeAndStringify_(__FILE__, __LINE__, __VA_ARGS__)
    std::string tokenizeAndStringify_(const char* file, int linenr, const char code[], bool simplify = false, bool expand = true, cppcheck::Platform::PlatformType platform = cppcheck::Platform::Native, const char* filename = "test.cpp", bool cpp11 = true) {
        errout.str("");
...
#define tokenizeDebugListing(...) tokenizeDebugListing_(__FILE__, __LINE__, __VA_ARGS__)
    ...
    void combine_strings() {
        ...
        ASSERT_EQUALS("x = L\"123\" ;", tok(code3, false, cppcheck::Platform::Win64));

        const char code4[] = "x = TEXT(\"1\") L\"2\";";
        ASSERT_EQUALS("x = L\"1\" L\"2\" ;", tok(code4, false, cppcheck::Platform::Win64));
    }
    ...
    void simplifyCallingConvention() {
        ...
        ASSERT_EQUALS("int f ( ) ;", tok("int WINAPI f();", true, cppcheck::Platform::Win32A));
        ASSERT_EQUALS("int f ( ) ;", tok("int APIENTRY f();", true, cppcheck::Platform::Win32A));
        ASSERT_EQUALS("int f ( ) ;", tok("int CALLBACK f();", true, cppcheck::Platform::Win32A));

        // don't simplify Microsoft defines in unix code (#7554)
        ASSERT_EQUALS("enum E { CALLBACK } ;", tok("enum E { CALLBACK } ;", true, cppcheck::Platform::Unix32));
    }
    ...
    void simplifyKnownVariables42() {
        {
            ...
            const char expected[] = "void f ( ) {\n"
                                    ...
                                    "strcat ( a , \"!\" ) ;\n"
                                    "}";
            ASSERT_EQUALS(expected, tokenizeAndStringify(code, true, true, cppcheck::Platform::Native, "test.c"));
        }
    ...
    void simplifyKnownVariables46() {
...

        {
            const char expected[] = "void f ( ) {\n"
                                    ...
                                    "return x ;\n"
                                    "}";
            ASSERT_EQUALS(expected, tokenizeAndStringify(code, true, true, cppcheck::Platform::Native, "test.cpp"));
        }
    }
