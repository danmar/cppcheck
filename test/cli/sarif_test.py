...
def test_sarif_null_pointer():
    ...
    assert 'null' in result['message']['text'].lower() or 'nullptr' in result['message']['text'].lower()
...
def test_sarif_security_rules():
    ...
    security_rule_ids = ['nullPointer', 'arrayIndexOutOfBounds', 'memleak', 'uninitvar', 'doubleFree']
...
def test_sarif_cwe_tags():
    ...
    for rule_id, cwe_tags in rules_with_cwe:
        for tag in cwe_tags:
...
def test_sarif_instance_specific_messages():
    ...
    for result in results:
        ...
        elif rule_id == 'arrayIndexOutOfBounds' and ('array' in message_text or '10' in message_text):
            found_specific_messages = True
    ...
    assert found_specific_messages, "Should find at least some instance-specific messages"
...
def test_sarif_rule_properties():
    ...
    for rule in rules:
        ...
        assert 'problem.severity' in props
        assert props['problem.severity'] in ['error', 'warning', 'style', 'performance', 'portability', 'information', 'note']
...
def test_sarif_rule_coverage():
    ...
    assert len(rule_ids) >= 5, f"Expected at least 5 rules, found {len(rule_ids)}: {rule_ids}"
    ...
    assert found_expected_rules >= 3, f"Expected at least 3 known rules, found {found_expected_rules}"
...
def test_sarif_generic_descriptions():
    ...
    for rule in rules:
        ...
        assert rule['name'] == '', f"Rule {rule_id} name should be empty, got: {rule['name']}"
        assert rule['shortDescription']['text'] == '', \
...
def test_sarif_security_rules_classification():
    ...
    assert found_rule_with_cwe or found_rule_without_cwe, \
...
def test_sarif_results_consistency():
    ...
    for result_rule_id in rule_ids_in_results:
        assert result_rule_id in rule_ids_in_rules, \
