...
def test_fuzz_crash():
    ...
    for f in os.listdir(fuzz_crash_dir):
        ret, stdout, _ = cppcheck(['-q', '--language=c++', '--enable=all', '--inconclusive', '--check-level=exhaustive', f], cwd=fuzz_crash_dir)
        if ret != 0:
            failures[f] = stdout
...
def test_fuzz_timeout():
    ...
    for f in os.listdir(fuzz_timeout_dir):
        try:
            cppcheck(['-q', '--language=c++', '--enable=all', '--inconclusive', '--check-level=exhaustive', f], cwd=fuzz_timeout_dir, timeout=5)
