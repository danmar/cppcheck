...
# python -m pytest helloworld2_test.py
...
__proj_dir = os.path.join(__script_dir, 'helloworld2')
...
def test_relative_path():
    ...
    filename = os.path.join('helloworld2', 'main.c')
    assert ret == 0, stdout
...
def test_addon_with_gui_project(tmp_path):
    shutil.copytree(os.path.join(__script_dir, 'helloworld2'), tmp_path / 'helloworld2')
    project_file = os.path.join('helloworld2', 'test.cppcheck')
...
def test_vs_project_relative_path():
    args = [
        '--template=cppcheck1',
        '--project=' + os.path.join(__proj_dir, 'helloworld2.vcxproj')
    ]
    ret, stdout, stderr = cppcheck(args, cwd=__script_dir)
    filename = os.path.join(__proj_dir, 'main.c')
    assert ret == 0, stdout
...
def test_cppcheck_project_relative_path():
    args = [
        '--template=cppcheck1',
        '--platform=win64',
        '--project=' + os.path.join('helloworld2', 'helloworld2.cppcheck')
    ]
    ret, stdout, stderr = cppcheck(args, cwd=__script_dir)
    filename = os.path.join('helloworld2', 'main.c')
    assert ret == 0, stdout
...
def test_cppcheck_project_absolute_path():
    args = [
        '--template=cppcheck1',
        '--platform=win64',
        '--project=' + os.path.join(__proj_dir, 'helloworld2.cppcheck')
    ]
    ret, stdout, stderr = cppcheck(args)
    filename = os.path.join(__proj_dir, 'main.c')
    assert ret == 0, stdout
...
def test_suppress_command_line_related():
    args = [
        '--suppress=zerodiv:' + os.path.join('helloworld2', 'main.c'),
        'helloworld2'
    ]
    ret, stdout, stderr = cppcheck(args, cwd=__script_dir)
    assert ret == 0, stdout
...
def test_suppress_project_absolute(tmp_path):
    shutil.copytree(os.path.join(__script_dir, 'helloworld2'), tmp_path / 'helloworld2')
    project_file = tmp_path / 'helloworld2' / 'test.cppcheck'
...
def test_missing_include_system():  # #11283
    ...
    assert stderr.replace('\\', '/') == 'helloworld2/main.c:1:2: information: Include file: <stdio.h> not found. Please note: Cppcheck does not need standard library headers to get proper results. [missingIncludeSystem]\n'
...
def test_sarif():
    ...
    assert res['runs'][0]['results'][0]['locations'][0]['physicalLocation']['artifactLocation']['uri'] == 'helloworld2/main.c'
