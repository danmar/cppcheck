...
class CppcheckException(Exception):
    """
    Exception generated by cppcheck addons internals.
    """

    def __init__(self, message, return_code):
        """
        :param return_code: Return code iff program should be terminated due this error.
        """
        super(CppcheckException, self).__init__(message)
...
def ArgumentParser():
    ...
    common arguments definitions supported by each addon.
    """
    ...
    parser.add_argument("-R", "--recursive",
                        help="Find cppcheck dump files in input directories "
                        "recursively.",
                        action="store_true")
...
def GetDumpFiles(paths, is_recursive):
    """
    Generate list of relative file paths to cppcheck dump files.

    :param paths: List of paths to target files or directories iff is_recursive flag is set.
    :param is_recursive: Recursively find cppcheck dump files in given paths.
    """
    scan_dirs = []
    dump_files = []
    ...
    for path in paths:
        if not os.path.exists(path):
            raise CppcheckException("%s not found" % path, 127)
        if os.path.isfile(path):
            dump_files.append(path)
        elif not is_recursive:
            raise CppcheckException("%s is not a regular file! "
                                    "Use '-R' option to find files recursively." % path, 21)
        else:
