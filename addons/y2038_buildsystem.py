...
def run_command(cmd, cwd):
    ...
    try:
        # Ensure cwd is a resolved Path for security
        if isinstance(cwd, str):
            cwd = Path(cwd).resolve()
        elif isinstance(cwd, Path):
            cwd = cwd.resolve()
        else:
            raise ValueError(f"Invalid cwd type: {type(cwd)}")
...
def generate_compile_commands(project_dir):
    ...
    try:
        ...
        print_error(f"Invalid project directory path: {e}")
        return False
    ...
    try:
        ...
        if ".." in path_str or path_str.startswith("/"):
            ...
            if not path_str.startswith(("/home", "/opt", "/usr/local", "/tmp", "/private/var/folders", str(Path.cwd()))):
                print_error(f"Path appears to be outside expected directories: {project_dir}")
                return False
