                                        ...
                                        const MathLib::bigint num)
{
    ...
    for (const Token* tok2 = startTok; tok2; tok2 = tok2->next()) {
        if ((bitop == '&') && Token::Match(tok2->tokAt(2), "%varid% %cop% %num% ;", varid) &&
            tok2->strAt(3) == std::string(1U, bitop)) {
...
void CheckCondition::assignIfError(const Token* tok1, const Token* tok2, const std::string& condition, bool result)
{
    ...
    reportError(locations,
                Severity::style,
                "assignIfError",
                "Mismatching assignment and comparison, comparison '" + condition + "' is always " +
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        if (tok->str() == "|" && tok->astOperand1() && tok->astOperand2() && tok->astParent()) {
            ...
            const bool isBoolean =
                Token::Match(parent, "&&|%oror%") || (parent->str() == "?" && parent->astOperand1() == tok) ||
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const MathLib::bigint num1 : numbers) {
            if (num1 < 0)
                continue;
            if (Token::Match(tok, "==|!=")) {
                if ((expr1->str() == "&" && (num1 & num2) != num2) || (expr1->str() == "|" && (num1 | num2) != num2)) {
                                     ...
                                     bool result)
{
    ...
    const std::string errmsg("Expression '" + expression.str() + "' is always " + (result ? "true" : "false") +
                             ".\n"
                             "The expression '" +
                             expression.str() + "' is always " + (result ? "true" : "false") +
...
void CheckCondition::overlappingElseIfConditionError(const Token* tok, nonneg int line1)
{
    ...
    errmsg << "Expression is always false because 'else if' condition matches previous condition at line " << line1
           << ".";
...
void CheckCondition::multiCondition2()
{
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        visitAstNodes(condTok, [&](const Token* cond) {
            ...
            if (cond->varId()) {
                ...
                if (!nonlocal && var) {
                    if (!(var->isLocal() || var->isArgument()))
                        nonlocal = true;
                    else if ((var->isPointer() || var->isReference()) &&
                             !Token::Match(cond->astParent(), "%oror%|&&|!"))
                        // TODO: if var is pointer check what it points at
                        nonlocal = true;
                }
            } else if (!nonlocal && cond->isName()) {
                // varid is 0. this is possibly a nonlocal variable..
                nonlocal = Token::Match(cond->astParent(), "%cop%|(|[") || Token::Match(cond, "%name% .") ||
                           (mTokenizer->isCPP() && cond->str() == "this");
            } else {
        ...
        visitAstNodes(condTok, [&varsInCond](const Token* cond) {
            if (cond->variable()) {
                const Variable* var = cond->variable();
        ...
        for (MULTICONDITIONTYPE type : types) {
            ...
            for (; tok && tok != endToken; tok = tok->next()) {
                if (Token::Match(tok, "if|return")) {
                    ...
                    const Token* condEndToken =
                        tok->str() == "if" ? condStartToken->link() : Token::findsimplematch(condStartToken, ";");
                    ...
                    // Condition..
                    const Token* cond2 =
                        tok->str() == "if" ? condStartToken->astOperand2() : condStartToken->astOperand1();
                    ...
                    if (type == MULTICONDITIONTYPE::INNER) {
                        visitAstNodes(cond1, [&](const Token* firstCondition) {
                            ...
                            if (!firstCondition->hasKnownIntValue()) {
                                if (!isReturnVar && isOppositeCond(false,
                                                                   mTokenizer->isCPP(),
                                                                   firstCondition,
                                                                   cond2,
                                                                   mSettings->library,
                                ...
                                } else if (!isReturnVar && isSameExpression(mTokenizer->isCPP(),
                                                                            true,
                                                                            firstCondition,
                                                                            cond2,
                                                                            mSettings->library,
                ...
                if (Token::Match(tok, "%type% (") && nonlocal &&
                    isNonConstFunctionCall(tok, mSettings->library)) // non const function call -> bailout if there are
                                                                     // nonlocal variables
                    break;
                ...
                if ((tok->varId() && vars.find(tok->varId()) != vars.end()) || (!tok->varId() && nonlocal) ||
                    ...
                    if (mTokenizer->isCPP() && Token::Match(tok, "%name% <<") &&
                        (!tok->valueType() || !tok->valueType()->isIntegral()))
                        break;
                    if (isLikelyStreamRead(mTokenizer->isCPP(), tok->next()) ||
                        isLikelyStreamRead(mTokenizer->isCPP(), tok->previous()))
                        break;
                    if (Token::Match(tok, "%name% [")) {
                    ...
                    if (Token::Match(tok->previous(), "++|--|& %name%"))
                        break;
                    if (tok->variable() && !tok->variable()->isConst() && Token::Match(tok, "%name% . %name% (")) {
...
void CheckCondition::oppositeInnerConditionError(const Token* tok1, const Token* tok2, ErrorPath errorPath)
{
    ...
    const std::string msg("Opposite inner '" + innerSmt +
                          "' condition leads to a dead code block.\n"
                          "Opposite inner '" +
                          innerSmt + "' condition leads to a dead code block (outer condition is '" + s1 +
...
void CheckCondition::identicalInnerConditionError(const Token* tok1, const Token* tok2, ErrorPath errorPath)
{
    ...
    const std::string msg("Identical inner '" + innerSmt +
                          "' condition is always true.\n"
                          "Identical inner '" +
                          innerSmt + "' condition is always true (outer condition is '" + s1 +
                          "' and inner condition is '" + s2 + "').");
...
void CheckCondition::identicalConditionAfterEarlyExitError(const Token* cond1, const Token* cond2, ErrorPath errorPath)
{
    ...
    const std::string value =
        (cond2 && cond2->valueType() && cond2->valueType()->type == ValueType::Type::BOOL) ? "false" : "0";

    errorPath.emplace_back(ErrorPathItem(cond1, "If condition '" + cond + "' is true, the function will return/exit"));
    errorPath.emplace_back(ErrorPathItem(
        cond2, (isReturnValue ? "Returning identical expression '" : "Testing identical condition '") + cond + "'"));
...
{
    return (op == "==" && value1 == value2) || (op == "!=" && value1 != value2) || (op == ">" && value1 > value2) ||
           (op == ">=" && value1 >= value2) || (op == "<" && value1 < value2) || (op == "<=" && value1 <= value2);
}
...
static std::string conditionString(bool not1, const Token* expr1, const std::string& op, const std::string& value1)
{
    if (expr1->astParent()->isComparisonOp())
        return std::string(not1 ? "!(" : "") + (expr1->isName() ? expr1->str() : std::string("EXPR")) + " " + op + " " +
               value1 + (not1 ? ")" : "");

    return std::string(not1 ? "!" : "") + (expr1->isName() ? expr1->str() : std::string("EXPR"));
}
...
void CheckCondition::checkIncorrectLogicOperator()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            // 'A || (A && B)' is equivalent to 'A'
            if (printStyle && ((tok->str() == "||" && tok->astOperand2()->str() == "&&") ||
                               (tok->str() == "&&" && tok->astOperand2()->str() == "||"))) {
                const Token* tok2 = tok->astOperand2()->astOperand1();
                if (isOppositeCond(
                        true, mTokenizer->isCPP(), tok->astOperand1(), tok2, mSettings->library, true, false)) {
                    ...
                    const std::string cond1 =
                        expr1 + " " + tok->str() + " (" + expr2 + " " + tok->astOperand2()->str() + " " + expr3 + ")";
                    ...
                    const std::string msg = tok2->expressionString() + ". '" + cond1 + "' is equivalent to '" + cond2 +
                                            "'\n"
                                            "The condition '" +
                                            cond1VerboseMsg + "' is equivalent to '" + cond2VerboseMsg + "'.";
                    redundantConditionError(tok, msg, false);
                    continue;
                } else if (isSameExpression(
                               mTokenizer->isCPP(), false, tok->astOperand1(), tok2, mSettings->library, true, true)) {
                    std::string expr1(tok->astOperand1()->expressionString());
                    std::string expr2(tok->astOperand2()->astOperand1()->expressionString());
                    std::string expr3(tok->astOperand2()->astOperand2()->expressionString());
                    // make copy for later because the original string might get overwritten
            ...
            const bool isfloat = astIsFloat(expr1, true) || MathLib::isFloat(value1) || astIsFloat(expr2, true) ||
                                 MathLib::isFloat(value2);

            ErrorPath errorPath;

            // Opposite comparisons around || or && => always true or always false
            if (!isfloat && isOppositeCond(tok->str() == "||",
                                           mTokenizer->isCPP(),
                                           tok->astOperand1(),
                                           tok->astOperand2(),
                                           mSettings->library,
            ...
            if (printWarning && (alwaysTrue || alwaysFalse)) {
                ...
                const std::string text = "If '" + cond1str + "', the comparison '" + cond2str + "' is always true.";
                ...
                //                         cond2str + ".";
                const std::string text = "If '" + cond2str + "', the comparison '" + cond1str + "' is always true.";
                redundantConditionError(tok, text, inconclusive);
            }
        }
    }
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "( %name% [=&|^]")) {
                ...
                if (vt1 && vt1->type == ValueType::BOOL && !Token::Match(tok->astOperand1(), "%name%|(|[|::|.") &&
                    countPar(tok->astOperand1(), tok) == 0)
                    clarifyConditionError(tok, false, true);
                else if (vt2 && vt2->type == ValueType::BOOL && !Token::Match(tok->astOperand2(), "%name%|(|[|::|.") &&
                         countPar(tok, tok->astOperand2()) == 0)
                    clarifyConditionError(tok, false, true);
            }
        }
    }
...
void CheckCondition::alwaysTrueFalse()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            const bool constIfWhileExpression = tok->astParent() &&
                                                Token::Match(tok->astTop()->astOperand1(), "if|while") &&
                                                !tok->astTop()->astOperand1()->isConstexpr() &&
                                                (Token::Match(tok->astParent(), "%oror%|&&") ||
                                                 Token::Match(tok->astParent()->astOperand1(), "if|while"));
            const bool constValExpr = tok->isNumber() && Token::Match(tok->astParent(), "%oror%|&&|?"); // just one
                                                                                                        // number in
                                                                                                        // boolean
                                                                                                        // expression
            const bool compExpr = Token::Match(tok, "%comp%|!"); // a compare expression
            const bool ternaryExpression = Token::simpleMatch(tok->astParent(), "?");
            const bool returnExpression = Token::simpleMatch(tok->astTop(), "return") &&
                                          (tok->isComparisonOp() || Token::Match(tok, "&&|%oror%"));
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const Token* lhs : lhsTokens) {
            ...
            const bool isSignedInteger =
                lhs->valueType() && lhs->valueType()->isIntegral() && lhs->valueType()->sign == ValueType::Sign::SIGNED;
            ...
            for (const Token* expr : exprTokens) {
                ...
                if (!isSameExpression(
                        mTokenizer->isCPP(), true, expr, lhs->astSibling(), mSettings->library, true, false))
                    continue;
...
void CheckCondition::invalidTestForOverflow(const Token* tok, const ValueType* valueType, const std::string& replace)
{
    ...
    std::string errmsg = "Invalid test for overflow '" + expr + "'; " + overflow + " is undefined behavior.";
    if (replace == "false" || replace == "true")
        ...
        errmsg +=
            ...
            replace + "'.";
    reportError(
        tok, Severity::warning, "invalidTestForOverflow", errmsg, uncheckedErrorConditionCWE, Certainty::normal);
}
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->valueType()->pointer > 0)
                continue;
            if (tok->valueType()->type != ValueType::Type::CONTAINER &&
                tok->valueType()->type != ValueType::Type::ITERATOR)
                continue;
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                const auto typeMinValue =
                    (typeTok->valueType()->sign == ValueType::Sign::SIGNED) ? (-(1LL << (bits - 1))) : 0;
                const auto unsignedTypeMaxValue = (1LL << bits) - 1LL;
                const auto typeMaxValue = (typeTok->valueType()->sign == ValueType::Sign::SIGNED)
                                              ? (unsignedTypeMaxValue / 2)
                                              : unsignedTypeMaxValue;

                if (valueTok->getKnownIntValue() < typeMinValue)
                    compareValueOutOfTypeRangeError(
                        valueTok, typeTok->valueType()->str(), valueTok->getKnownIntValue());
...
{
    reportError(comparison,
                Severity::style,
                "compareValueOutOfTypeRangeError",
                "Comparing expression of type '" + type + "' against value " + std::to_string(value) +
