...
static bool isBool(const Variable* var) { return (var && Token::Match(var->typeEndToken(), "bool|_Bool")); }
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->isBinaryOp() && (tok->str() == "&" || tok->str() == "|")) {
                if (astIsBool(tok->astOperand1()) || astIsBool(tok->astOperand2())) {
                    if (tok->astOperand2()->variable() &&
                        tok->astOperand2()->variable()->nameToken() == tok->astOperand2())
                        continue;
                    const std::string expression = astIsBool(tok->astOperand1())
                                                       ...
                                                       const std::string& expression2)
{
    reportError(tok,
                ...
                "The return type of function '" +
                    expression1 + "' and function '" + expression2 +
                    "' is 'bool' "
                    "and result is of type 'bool'. Comparing 'bool' value using relational (<, >, <= or >=)"
                    " operator could cause unexpected results.",
                CWE398,
...
{
    reportError(
        tok, Severity::error, "assignBoolToPointer", "Boolean value assigned to pointer.", CWE587, Certainty::normal);
}
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (minval && minval->intvalue == 0 &&
                (numInRhs ? Token::Match(tok, ">|==|!=") : Token::Match(tok, "<|==|!=")))
                minval = nullptr;
