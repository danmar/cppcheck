                          ...
                          SourceLocation loc = SourceLocation::current())
{
    ...
    if (const Token *castType = getCastTypeStartToken(parent, settings)) {
        if (contains({ValueFlow::Value::ValueType::INT, ValueFlow::Value::ValueType::SYMBOLIC}, value.valueType) &&
            ...
            return;
        const ValueType &valueType = ValueType::parseDecl(castType, *settings, castType->isCpp());
        if (value.isImpossible() && value.isIntValue() && value.intvalue < 0 && astIsUnsigned(tok) &&
...
{
    const ValueType &valueType = ValueType::parseDecl(typeTok, *settings, typeTok->isCpp())
...
static Token * valueFlowSetConstantValue(Token *tok, const Settings *settings, bool cpp, bool isInitList = false)
{
    if ((tok->isNumber() && MathLib::isInt(tok->str())) || (tok->tokType() == Token::eChar)) {
        ...
        if (Token::simpleMatch(tok, "sizeof ( *")) {
            ...
            const ValueType& vt = ValueType::parseDecl(tok2, *settings, tok2->isCpp());
...
static bool isContainerOfPointers(const Token* tok, const Settings* settings)
{
    ...
    ValueType vt = ValueType::parseDecl(tok, *settings, tok->isCpp());
    return vt.pointer > 0;
}
                                                     ...
                                                     bool known = true)
{
    ...
    if (args.size() < 4) {
        ...
        if (valueType->container->stdStringLike) {
            ...
            ValueType vt = ValueType::parseDecl(containerTypeToken, *settings, containerTypeToken->isCpp())
...
static bool getMinMaxValues(const std::string &typestr, const Settings *settings, MathLib::bigint &minvalue, MathLib::bigint &maxvalue)
{
    ...
    const ValueType &vt = ValueType::parseDecl(typeTokens.front(), *settings, typeTokens.front()->isCpp());
