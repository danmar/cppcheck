...
void CheckFunctions::checkProhibitedFunctions()
{
    const bool checkAlloca = mSettings->severity.isEnabled(Severity::warning) &&
                             ((mSettings->standards.c >= Standards::C99 && mTokenizer->isC()) ||
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (checkAlloca && Token::simpleMatch(tok, "alloca (") &&
                (!tok->function() || tok->function()->nestedIn->type == Scope::eGlobal)) {
                if (mTokenizer->isC()) {
                ...
                if (wi) {
                    if (mSettings->severity.isEnabled(wi->severity) && mSettings->standards.c >= wi->standards.c &&
                        ...
                        reportError(
                            tok, wi->severity, daca + tok->str() + "Called", wi->message, CWE477, Certainty::normal);
                    }
                }
            }
        }
                                             ...
                                             const std::string& validstr)
{
    ...
    if (invalidValue && invalidValue->condition)
        errmsg << ValueFlow::eitherTheConditionIsRedundant(invalidValue->condition) << " or $symbol() argument nr "
               << argnr << " can have invalid value.";
    else
        errmsg << "Invalid $symbol() argument nr " << argnr << '.';
    if (invalidValue)
        errmsg << " The value is " << std::setprecision(10)
               << (invalidValue->isIntValue() ? invalidValue->intvalue : invalidValue->floatValue)
...
void CheckFunctions::checkMathFunctions()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (styleC99) {
                if (Token::Match(tok, "%num% - erf (") && Tokenizer::isOneNumber(tok->str()) &&
                    tok->next()->astOperand2() == tok->tokAt(3)) {
                    mathfunctionCallWarning(tok, "1 - erf(x)", "erfc(x)");
                } else if (Token::simpleMatch(tok, "exp (") && Token::Match(tok->linkAt(1), ") - %num%") &&
                           Tokenizer::isOneNumber(tok->linkAt(1)->strAt(2)) &&
    ...
    if (tok) {
        if (numParam == 1)
            reportError(tok,
                        Severity::warning,
                        "wrongmathcall",
                        "$symbol:" + tok->str() + "\nPassing value " + tok->strAt(2) +
                            " to $symbol() leads to implementation-defined result.",
                        CWE758,
                        Certainty::normal);
        else if (numParam == 2)
            reportError(tok,
                        Severity::warning,
                        "wrongmathcall",
                        "$symbol:" + tok->str() + "\nPassing values " + tok->strAt(2) + " and " + tok->strAt(4) +
                            " to $symbol() leads to implementation-defined result.",
                        CWE758,
                        Certainty::normal);
    } else
...
void CheckFunctions::memsetZeroBytesError(const Token* tok)
{
    ...
    const std::string verbose(summary +
...
void CheckFunctions::memsetInvalid2ndParam()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok && (tok != scope->bodyEnd); tok = tok->next()) {
            ...
            if (printWarning && secondParamTok->isNumber()) { // Check if the second parameter is a literal and is out
                                                              // of range
...
void CheckFunctions::checkLibraryMatchFunctions()
{
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (functionName.empty() ||
            mSettings->library.functions.find(functionName) != mSettings->library.functions.end())
            continue;
