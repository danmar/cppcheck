...
{
public:
    explicit Data(const char* pattern)
    : mPattern(pattern)
    {}
...
std::string Regex::Data::compile()
{
    ...
    int erroffset = 0;
    pcre * const re = pcre_compile(mPattern,0,&pcreCompileErrorStr,&erroffset,nullptr);
    if (!re) {
...
std::string Regex::Data::match(const std::string& str, const MatchFn& match) const
{
    ...
    while (pos < (int)str.size()) {
        const int pcreExecRet = pcre_exec(mRe, mExtra, str.c_str(), (int)str.size(), pos, 0, ovector, 30);
...
Regex::Regex(std::string pattern)
    ...
    , mData(std::make_shared<Data>(mPattern.c_str()))
{}

std::string Regex::compile()
{
    return mData->compile();
}
