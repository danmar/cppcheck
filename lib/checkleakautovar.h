...
public:
    ...
    struct AllocInfo {
        ...
        AllocInfo(int type_ = 0, AllocStatus status_ = NOALLOC, const Token* allocTok_ = nullptr)
            : status(status_), type(type_), allocTok(allocTok_)
        {}

        bool managed() const { return status < 0; }
    };
...
private:
    ...
    void checkScope(const Token* const startToken, VarInfo* varInfo, std::set<int> notzero, nonneg int recursiveCount);
    ...
    void changeAllocStatusIfRealloc(std::map<int, VarInfo::AllocInfo>& alloctype,
                                    const Token* fTok,
                                    const Token* retTok);
    ...
    static std::string myName() { return "Leaks (auto variables)"; }
