...
Directive::Directive(const std::string& _file, const int _linenr, const std::string& _str)
    : file(_file), linenr(_linenr), str(trim(_str))
{}
...
Preprocessor::Preprocessor(Settings& settings, ErrorLogger* errorLogger)
    : mSettings(settings), mErrorLogger(errorLogger)
{}
...
static void inlineSuppressions(const simplecpp::TokenList& tokens,
                               Settings& mSettings,
                               std::list<BadInlineSuppression>* bad)
{
    for (const simplecpp::Token* tok = tokens.cfront(); tok; tok = tok->next) {
        ...
        const bool thisAndNextLine =
            tok->previous && tok->previous->previous && tok->next &&
            !sameline(tok->previous->previous, tok->previous) && tok->location.line + 1 == tok->next->location.line &&
            tok->location.fileIndex == tok->next->location.fileIndex && tok->previous->str() == "{";
...
void Preprocessor::inlineSuppressions(const simplecpp::TokenList& tokens)
{
    ...
    for (std::map<std::string, simplecpp::TokenList*>::const_iterator it = mTokenLists.begin(); it != mTokenLists.end();
         ++it) {
        if (it->second)
...
void Preprocessor::setDirectives(const simplecpp::TokenList& tokens)
{
    ...
    for (const simplecpp::TokenList* tokenList : list) {
        for (const simplecpp::Token* tok = tokenList->cfront(); tok; tok = tok->next) {
            ...
            for (const simplecpp::Token* tok2 = tok; tok2 && tok2->location.line == directive.linenr;
                 tok2 = tok2->next) {
                if (tok2->comment)
                    continue;
                                 ...
                                 const std::set<std::string>& undefined)
{
    ...
    for (; sameline(iftok, cond); cond = cond->next) {
        ...
        if (sameline(iftok, dtok) && dtok->name && defined.find(dtok->str()) == defined.end() &&
            undefined.find(dtok->str()) == undefined.end())
...
{
    return std::any_of(
        configs_if.cbegin(), configs_if.cend(), [=](const std::string& cfg) { return hasDefine(userDefines, cfg); });
}
                       ...
                       std::set<std::string>& ret)
{
    ...
    for (const simplecpp::Token* tok = tokens.cfront(); tok; tok = tok->next) {
        ...
        if (cmdtok->str() == "ifdef" || cmdtok->str() == "ifndef" || cmdtok->str() == "if") {
            ...
            if (cmdtok->str() == "else" && cmdtok->next && !sameline(cmdtok, cmdtok->next) &&
                sameline(cmdtok->next, cmdtok->next->next) && cmdtok->next->op == '#' &&
...
void Preprocessor::preprocess(std::istream& istr,
...
static void splitcfg(const std::string& cfg, std::list<std::string>& defines, const std::string& defaultValue)
{
    for (std::string::size_type defineStartPos = 0U; defineStartPos < cfg.size();) {
        ...
        std::string def = (defineEndPos == std::string::npos)
...
void Preprocessor::error(const std::string& filename, unsigned int linenr, const std::string& msg)
{
    ...
    mErrorLogger->reportErr(
        ErrorMessage(locationList, mFile0, Severity::error, msg, "preprocessorErrorDirective", Certainty::normal));
}
    ...
    for (const std::string& define : defines) {
        ...
        for (const simplecpp::MacroUsage& mu : macroUsageList) {
            ...
            if (mu.macroName != macroName)
                continue;
            bool directiveLocation = std::any_of(mDirectives.cbegin(), mDirectives.cend(), [=](const Directive& dir) {
...
void Preprocessor::validateCfgError(const std::string& file,
                                    ...
                                    const std::string& macro)
{
    ...
    const ErrorMessage errmsg(locationList,
                              mFile0,
                              Severity::information,
                              "Skipping configuration '" + cfg + "' since the value of '" + macro +
