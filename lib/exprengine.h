...
#ifdef _MSC_VER
#pragma message(__FILE__ "(" _CRT_STRINGIZE(                                                                           \
    __LINE__) ")"                                                                                                      \
...
public:
    explicit DataBase(const std::string& currentFunction, const Settings* settings)
        : currentFunction(currentFunction), settings(settings)
    {}
...
public:
    ...
    virtual ~Value() {}
    virtual std::string getRange() const { return name; }
    virtual std::string getSymbolicExpression() const { return name; }
    ...
    virtual bool isUninit() const { return false; }
...
public:
    ...
    bool isUninit() const OVERRIDE { return true; }
};

class IntRange : public Value {
public:
    IntRange(const std::string& name, int128_t minValue, int128_t maxValue)
        : Value(name, ValueType::IntRange), minValue(minValue), maxValue(maxValue), loopScope(nullptr)
    {}
...
public:
    FloatRange(const std::string& name, long double minValue, long double maxValue)
        : Value(name, ValueType::FloatRange), minValue(minValue), maxValue(maxValue)
    {}

    std::string getRange() const OVERRIDE { return std::to_string(minValue) + ":" + std::to_string(maxValue); }
...
public:
    ...
    ConditionalValue(const std::string& name, const Vector& values)
        : Value(name, ValueType::ConditionalValue), values(values)
    {}
...
public:
    StringLiteralValue(const std::string& name, const std::string& s)
        : Value(name, ValueType::StringLiteralValue), string(s)
    {}

    std::string getRange() const OVERRIDE { return "\"" + string + "\""; }
...
public:
    ...
    std::string getRange() const OVERRIDE { return getSymbolicExpression(); }
...
public:
    AddressOfValue(const std::string& name, int varId) : Value(name, ValueType::AddressOfValue), varId(varId) {}

    std::string getRange() const OVERRIDE { return "&@" + std::to_string(varId); }
...
public:
    BinOpResult(const std::string& binop, ValuePtr op1, ValuePtr op2)
        : Value(getName(binop, op1, op2), ValueType::BinOpResult), binop(binop), op1(op1), op2(op2)
    {}
...
public:
    IntegerTruncation(const std::string& name, ValuePtr inputValue, int bits, char sign)
        : Value(name, ValueType::IntegerTruncation), inputValue(inputValue), bits(bits), sign(sign)
    {}
...
public:
    explicit FunctionCallArgumentValues(const std::vector<ExprEngine::ValuePtr>& argValues)
        : Value("argValues", ValueType::FunctionCallArgumentValues), argValues(argValues)
    {}
...
public:
    ...
    bool isEqual(const DataBase* /*dataBase*/, int /*value*/) const OVERRIDE { return true; }
    bool isUninit() const OVERRIDE { return true; }
};
...
void CPPCHECKLIB executeAllFunctions(ErrorLogger* errorLogger,
                                     const Tokenizer* tokenizer,
                                     const Settings* settings,
                                     const std::vector<Callback>& callbacks,
                                     std::ostream& report);
void executeFunction(const Scope* functionScope,
