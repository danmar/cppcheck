...
static const ValueFlow::Value* getBufferSizeValue(const Token* tok)
{
    ...
    const auto it =
        std::find_if(tokenValues.begin(), tokenValues.end(), std::mem_fn(&ValueFlow::Value::isBufferSizeValue));
...
static bool getDimensionsEtc(const Token* const arrayToken,
                             ...
                             ErrorPath* const errorPath,
                             bool* const mightBeLarger,
                             MathLib::bigint* path)
{
    ...
    if (array->variable() && array->variable()->isArray() && !array->variable()->dimensions().empty()) {
        *dimensions = array->variable()->dimensions();
        if (dimensions->size() >= 1 && ((*dimensions)[0].num <= 1 || !(*dimensions)[0].tok)) {
            visitAstNodes(arrayToken, [&](const Token* child) {
    ...
    } else if (array->valueType() && array->valueType()->pointer >= 1 &&
               (array->valueType()->isIntegral() || array->valueType()->isFloat())) {
...
static std::vector<const ValueFlow::Value*> getOverrunIndexValues(const Token* tok,
...
void CheckBufferOverrun::arrayIndex()
{
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        while (Token::Match(array, ".|::"))
            array = array->astOperand2();
        if (!array ||
            ((!array->variable() || array->variable()->nameToken() == array) && array->tokType() != Token::eString))
            continue;
        ...
        if (!mightBeLarger) { // TODO check arrays with dim 1 also
            const std::vector<const ValueFlow::Value*>& indexValues =
                                     ...
                                     const Token* condition)
{
    ...
    const std::string array =
        std::accumulate(dimensions.begin(), dimensions.end(), tok->astOperand1()->expressionString(), add_dim);

    std::ostringstream errmsg;
    if (condition)
        errmsg << ValueFlow::eitherTheConditionIsRedundant(condition)
               << " or the array '" + array + "' is accessed at index "
               << stringifyIndexes(tok->astOperand1()->expressionString(), indexValues) << ", which is out of bounds.";
    else
        errmsg << "Array '" << array << "' accessed at index "
               << stringifyIndexes(tok->astOperand1()->expressionString(), indexValues) << ", which is out of bounds.";
...
void CheckBufferOverrun::arrayIndexError(const Token* tok,
                                         const std::vector<Dimension>& dimensions,
                                         const std::vector<const ValueFlow::Value*>& indexes)
{
    if (!tok) {
        reportError(tok,
                    Severity::error,
                                            ...
                                            const std::vector<const ValueFlow::Value*>& indexes)
{
    if (!tok) {
        reportError(
            tok, Severity::error, "negativeIndex", "Negative array index", CWE_BUFFER_UNDERRUN, Certainty::normal);
        return;
    }
...
void CheckBufferOverrun::pointerArithmetic()
{
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (!indexToken || !indexToken->valueType() || indexToken->valueType()->pointer > 0 ||
            !indexToken->valueType()->isIntegral())
            continue;
                                                ...
                                                const ValueFlow::Value* indexValue)
{
    ...
    if (indexValue->condition)
        errmsg = "Undefined behaviour, when '" + indexToken->expressionString() + "' is " +
                 MathLib::toString(indexValue->intvalue) + " the pointer arithmetic '" + tok->expressionString() +
                 "' is out of bounds.";
    else
...
ValueFlow::Value CheckBufferOverrun::getBufferSize(const Token* bufTok) const
{
    ...
    MathLib::bigint dim = std::accumulate(var->dimensions().begin(),
                                          var->dimensions().end(),
                                          1LL,
                                          [](MathLib::bigint i1, const Dimension& dim) { return i1 * dim.num; });
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            for (int argnr = 0; argnr < args.size(); ++argnr) {
                if (!args[argnr]->valueType() || args[argnr]->valueType()->pointer == 0)
                    continue;
                const std::vector<Library::ArgumentChecks::MinSize>* minsizes =
                    mSettings->library.argminsizes(tok, argnr + 1);
                ...
                const bool error = std::none_of(
                    minsizes->begin(), minsizes->end(), [=](const Library::ArgumentChecks::MinSize& minsize) {
                        return checkBufferSize(tok, minsize, args, bufferSize.intvalue, mSettings);
                    });
                if (error)
                    bufferOverflowError(args[argnr],
                                        &bufferSize,
                                        (bufferSize.intvalue == 1) ? Certainty::inconclusive : Certainty::normal);
            }
        }
    }
}
...
void CheckBufferOverrun::bufferOverflowError(const Token* tok,
                                             ...
                                             const Certainty::CertaintyLevel& certainty)
{
    reportError(getErrorPath(tok, value, "Buffer overrun"),
                Severity::error,
...
bool CheckBufferOverrun::isCtuUnsafeBufferUsage(const Check* check,
                                                const Token* argtok,
                                                ...
                                                int type)
{
    ...
    if (type == 1 && Token::Match(argtok, "%name% [") && argtok->astParent() == argtok->next() &&
        !Token::simpleMatch(argtok->linkAt(1), "] ["))
        indexTok = argtok->next()->astOperand2();
    else if (type == 2 && Token::simpleMatch(argtok->astParent(), "+"))
        indexTok = (argtok == argtok->astParent()->astOperand1()) ? argtok->astParent()->astOperand2()
...
Check::FileInfo* CheckBufferOverrun::getFileInfo(const Tokenizer* tokenizer, const Settings* settings) const
{
    ...
    fileInfo->unsafePointerArith =
        CTU::getUnsafeUsage(tokenizer, settings, &checkBufferOverrun, isCtuUnsafePointerArith);
...
bool CheckBufferOverrun::analyseWholeProgram1(
    ...
    ErrorLogger& errorLogger)
{
    ...
    if (type == 1) {
        ...
        if (unsafeUsage.value > 0)
            errmsg = "Array index out of bounds; '" + unsafeUsage.myArgumentName + "' buffer size is " +
                     MathLib::toString(functionCall->callArgValue) + " and it is accessed at offset " +
                     MathLib::toString(unsafeUsage.value) + ".";
        else
            errmsg = "Array index out of bounds; buffer '" + unsafeUsage.myArgumentName + "' is accessed at offset " +
                     MathLib::toString(unsafeUsage.value) + ".";
        cwe = (unsafeUsage.value > 0) ? CWE_BUFFER_OVERRUN : CWE_BUFFER_UNDERRUN;
    } else {
        errorId = "ctuPointerArith";
        errmsg = "Pointer arithmetic overflow; '" + unsafeUsage.myArgumentName + "' buffer size is " +
                 MathLib::toString(functionCall->callArgValue);
        cwe = CWE_POINTER_ARITHMETIC_OVERFLOW;
    }

    const ErrorMessage errorMessage(
        locationList, emptyString, Severity::error, errmsg, errorId, cwe, Certainty::normal);
