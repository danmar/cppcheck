...
Scope *clangimport::AstNode::createScope(TokenList *tokenList, Scope::ScopeType scopeType, const std::vector<AstNodePtr> & children2, const Token *def)
{
    ...
    symbolDatabase->scopeList.emplace_back(Scope(nullptr, nullptr, nestedIn));
...
Token *clangimport::AstNode::createTokens(TokenList *tokenList)
{
    ...
    if (nodeType == EnumConstantDecl) {
        ...
        scope->enumeratorList.emplace_back(Enumerator(nameToken->scope()));
    ...
    if (nodeType == EnumDecl) {
        ...
        // Create enum type
        mData->mSymbolDatabase->typeList.emplace_back(Type(enumtok, enumscope, enumtok->scope()));
    ...
    if (nodeType == RecordDecl) {
        ...
        mData->mSymbolDatabase->typeList.emplace_back(Type(classDef, recordScope, classDef->scope()));
...
void clangimport::AstNode::createTokensFunctionDecl(TokenList *tokenList)
{
    ...
    if (!nameToken->function()) {
        nestedIn->functionList.emplace_back(Function(nameToken, unquote(getFullType())));
    ...
    for (int i = 0; i < children.size(); ++i) {
        ...
        if (!prev) {
            function->argumentList.emplace_back(Variable(vartok, child->getType(), nullptr, typeEndToken, i, AccessControl::Argument, recordType, scope));
            if (vartok) {
...
void clangimport::AstNode::createTokensForCXXRecord(TokenList *tokenList)
{
    ...
    if (isDefinition()) {
        ...
        mData->mSymbolDatabase->typeList.emplace_back(Type(classToken, scope, classToken->scope()));
...
Token * clangimport::AstNode::createTokensVarDecl(TokenList *tokenList)
{
    ...
    scope->varlist.emplace_back(Variable(vartok1, unquote(type), startToken, vartok1->previous(), 0, scope->defaultAccess(), recordType, scope));
...
void clangimport::parseClangAstDump(Tokenizer *tokenizer, std::istream &f)
{
    ...
    symbolDatabase->scopeList.emplace_back(Scope(nullptr, nullptr, nullptr));
