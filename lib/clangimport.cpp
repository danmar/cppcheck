...
static std::string unquote(const std::string& s) { return (s[0] == '\'') ? s.substr(1, s.size() - 2) : s; }
    ...
    while (pos1 < line.size()) {
        ...
        if (line[pos1] == '<')
            ...
            while (pos2 < line.size() &&
                   (line[pos2] == '_' || line[pos2] == ':' || std::isalnum((unsigned char)line[pos2])))
                ++pos2;
            ...
            pos2 = line.find(" ", pos1) - 1;
            if ((std::isalpha(line[pos1]) || line[pos1] == '_') && line.find("::", pos1) < pos2 &&
            ...
            if ((std::isalpha(line[pos1]) || line[pos1] == '_') && line.find("<", pos1) < pos2 &&
                line.find("<<", pos1) != line.find("<", pos1) && line.find(">", pos1) != std::string::npos &&
                line.find(">", pos1) > pos2) {
                int level = 0;
...
struct Data {
    struct Decl {
        ...
        Decl(Token* def, Function* function)
            : def(def), enumerator(nullptr), function(function), scope(nullptr), var(nullptr)
        {}
        Decl(Token* def, Enumerator* enumerator)
            : def(def), enumerator(enumerator), function(nullptr), scope(nullptr), var(nullptr)
        {}
        void ref(Token* tok)
        {
            if (enumerator)
    ...
    bool hasDecl(const std::string& addr) const { return mDeclMap.find(addr) != mDeclMap.end(); }
...
private:
    ...
    Scope* createScope(TokenList* tokenList,
...
bool clangimport::AstNode::isDefinition() const { return contains(mExtTokens, "definition"); }
...
const ::Type* clangimport::AstNode::addTypeTokens(TokenList* tokenList, const std::string& str, const Scope* scope)
{
    ...
    for (const Token* typeToken = tokenList->back(); Token::Match(typeToken, "&|*|%name%");
...
Token* clangimport::AstNode::createTokens(TokenList* tokenList)
{
    ...
    if (nodeType == EnumDecl) {
        ...
        while (colIndex > 0 && mExtTokens[colIndex].compare(0, 4, "col:") != 0 &&
               mExtTokens[colIndex].compare(0, 5, "line:") != 0)
            --colIndex;
        if (colIndex == 0)
            return nullptr;
...
void clangimport::AstNode::createTokensFunctionDecl(TokenList* tokenList)
{
    ...
    for (int i = 0; i < children.size(); ++i) {
        ...
        if (!prev) {
            function->argumentList.push_back(Variable(
                vartok, child->getType(), nullptr, typeEndToken, i, AccessControl::Argument, recordType, scope));
            if (vartok) {
...
void clangimport::AstNode::createTokensForCXXRecord(TokenList* tokenList)
{
    ...
    if (isDefinition()) {
        ...
        for (AstNodePtr child : children) {
            if (child->nodeType == CXXConstructorDecl || child->nodeType == CXXDestructorDecl ||
                child->nodeType == CXXMethodDecl || child->nodeType == FieldDecl || child->nodeType == VarDecl ||
                child->nodeType == AccessSpecDecl || child->nodeType == TypedefDecl)
                children2.push_back(child);
        }
        Scope* scope = createScope(
            tokenList, isStruct ? Scope::ScopeType::eStruct : Scope::ScopeType::eClass, children2, classToken);
...
Token* clangimport::AstNode::createTokensVarDecl(TokenList* tokenList)
{
    ...
    scope->varlist.push_back(Variable(
        vartok1, unquote(type), startToken, vartok1->previous(), 0, scope->defaultAccess(), recordType, scope));
