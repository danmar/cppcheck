    ...
    if (buf->variable()->dimensions().size() > nr && buf->variable()->dimensions()[nr].known) {
        ...
        if (value.isGreaterThan(dataBase, bufSize - 1)) {
            ...
            dataBase->reportError(tok,
                                  ...
                                  "Array index out of bounds, cannot determine that " + index->expressionString() +
...
static void bufferOverflow(const Token* tok, const ExprEngine::Value& value, ExprEngine::DataBase* dataBase)
{
    ...
    const ExprEngine::FunctionCallArgumentValues* functionCallArguments =
        dynamic_cast<const ExprEngine::FunctionCallArgumentValues*>(&value);
    ...
    for (auto argNrChecks : func->argumentChecks) {
        ...
        std::shared_ptr<ExprEngine::ArrayValue> arrayValue =
            std::dynamic_pointer_cast<ExprEngine::ArrayValue>(argValue);
        ...
        for (const Library::ArgumentChecks::MinSize& minsize : checks.minsizes) {
            if (minsize.type == Library::ArgumentChecks::MinSize::Type::ARGVALUE && minsize.arg > 0 &&
                minsize.arg <= arguments.size()) {
            ...
            } else if (minsize.type == Library::ArgumentChecks::MinSize::Type::STRLEN && minsize.arg > 0 &&
                       minsize.arg <= arguments.size()) {
                if (func->formatstr) {
                    // TODO: implement this properly. check if minsize refers to a format string and check max length of
                    // that..
                    overflowArgument = argnr;
                    bailout = true;
                    break;
                ...
                if (Token::Match(arguments[minsize.arg - 1], "%str%")) {
                    ...
                    std::shared_ptr<ExprEngine::ArrayValue> arrayValue2 =
                        std::dynamic_pointer_cast<ExprEngine::ArrayValue>(otherValue);
    ...
    dataBase->reportError(tok,
                          ...
                          "Buffer read/write, when calling '" + tok->previous()->str() +
                              "' it cannot be determined that " + std::to_string(overflowArgument) +
                              getOrdinalText(overflowArgument) + " argument is not overflowed",
                          CWE(120),
                          false,
                          bailout);
}
...
static void divByZero(const Token* tok, const ExprEngine::Value& value, ExprEngine::DataBase* dataBase)
{
    ...
    if (tok->astParent()->astOperand2() == tok && value.isEqual(dataBase, 0)) {
        const char* const id =
            (tok->valueType() && tok->valueType()->isFloat()) ? "bughuntingDivByZeroFloat" : "bughuntingDivByZero";
        const bool bailout = (value.type == ExprEngine::ValueType::BailoutValue);
...
{
    if (!tok->isArithmeticalOp() || !tok->valueType() || !tok->valueType()->isIntegral() ||
    ...
    errorMessage = "There is integer arithmetic, cannot determine that there can't be overflow (if result is " +
                   errorMessage + ").";
...
static void uninit(const Token* tok, const ExprEngine::Value& value, ExprEngine::DataBase* dataBase)
{
    ...
    if (tok->str() == "[" && tok->astOperand1() && tok->astOperand1()->valueType() &&
        tok->astOperand1()->valueType()->pointer == 0 && tok->astOperand1()->valueType()->container) {
        if (tok->astOperand1()->valueType()->container->stdStringLike)
            return;
        bool pointerType = false;
        for (const Token* typeTok = tok->astOperand1()->valueType()->containerTypeToken;
             Token::Match(typeTok, "%name%|*|::|<");
    ...
    const std::string inconclusiveMessage(
        inconclusive ? ". It is inconclusive if there would be a problem in the function call." : "");
    ...
    if (!uninitStructMember.empty()) {
        ...
        dataBase->reportError(tok,
                              ...
                              "$symbol:" + symbol + "\nCannot determine that '$symbol' is initialized" +
                                  inconclusiveMessage,
    ...
    dataBase->reportError(tok,
                          Severity::SeverityType::error,
                          errorId.c_str(),
                          symbol + "Cannot determine that '" + uninitexpr + "' is initialized" + constMessage +
                              inconclusiveMessage,
                          CWE_USE_OF_UNINITIALIZED_VARIABLE,
                          inconclusive,
                          value.type == ExprEngine::ValueType::BailoutValue);
...
static void checkFunctionCall(const Token* tok, const ExprEngine::Value& value, ExprEngine::DataBase* dataBase)
{
    ...
    if (parent->astOperand1()->function()) {
        ...
        if (arg && arg->nameToken()) {
            ...
            if (arg->nameToken()->getCppcheckAttribute(TokenImpl::CppcheckAttributes::Type::HIGH, &high)) {
                if (!(tok->hasKnownIntValue() && tok->getKnownIntValue() <= high) &&
                    value.isGreaterThan(dataBase, high))
            ...
            if (!bad.empty()) {
                dataBase->reportError(tok,
                                      ...
                                      "There is function call, cannot determine that " + std::to_string(num) +
                                          getOrdinalText(num) + " argument value meets the attribute " + bad,
                                      CWE(0),
                                      false);
                return;
            }
    ...
    for (const Library::InvalidArgValue& invalidArgValue :
         Library::getInvalidArgValues(dataBase->settings->library.validarg(parent->astOperand1(), num))) {
        bool err = false;
        std::string bad;
        ...
        if (err) {
            dataBase->reportError(tok,
                                  ...
                                  "bughuntingInvalidArgValue",
                                  "There is function call, cannot determine that " + std::to_string(num) +
                                      getOrdinalText(num) + " argument value is valid. Bad value: " + bad,
                                  CWE(0),
                                  false);
            break;
        }
    ...
    // Uninitialized function argument..
    if (dataBase->settings->library.isuninitargbad(parent->astOperand1(), num) &&
        dataBase->settings->library.isnullargbad(parent->astOperand1(), num) &&
        value.type == ExprEngine::ValueType::ArrayValue) {
        ...
        for (const auto& v : arrayValue.read(index0)) {
            if (v.second->isUninit()) {
                dataBase->reportError(tok,
                                      ...
                                      "bughuntingUninitArg",
                                      "There is function call, cannot determine that " + std::to_string(num) +
                                          getOrdinalText(num) + " argument is initialized.",
                                      CWE_USE_OF_UNINITIALIZED_VARIABLE,
                                      false);
                break;
            }
...
static void checkAssignment(const Token* tok, const ExprEngine::Value& value, ExprEngine::DataBase* dataBase)
{
    ...
    for (const Scope* s = lhs->variable()->nameToken()->scope(); s->type != Scope::ScopeType::eGlobal;
         s = s->nestedIn) {
        if (s->isExecutable()) {
            executable = true;
            break;
    ...
    if (getMinMaxValue(TokenImpl::CppcheckAttributes::Type::LOW, &low)) {
        if (value.isLessThan(dataBase, low))
            dataBase->reportError(tok,
                                  Severity::SeverityType::error,
                                  "bughuntingAssign",
                                  "There is assignment, cannot determine that value is greater or equal with " +
                                      std::to_string(low),
    ...
    if (getMinMaxValue(TokenImpl::CppcheckAttributes::Type::HIGH, &high)) {
        if (value.isGreaterThan(dataBase, high))
            dataBase->reportError(tok,
                                  Severity::SeverityType::error,
                                  "bughuntingAssign",
                                  "There is assignment, cannot determine that value is lower or equal with " +
                                      std::to_string(high),
                                  CWE_INCORRECT_CALCULATION,
                                  false);
    }
