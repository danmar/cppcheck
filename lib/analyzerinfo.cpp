...
std::string AnalyzerInformation::processFilesTxt(bool clang, const std::string& buildDir, const std::function<void(const char* checkattr, const tinyxml2::XMLElement* e, const Info& filesTxtInfo)>& handler, bool debug)
{
    const std::string filesTxt(buildDir + "/files.txt");
    ...
    while (std::getline(fin, filesTxtLine)) {
        ...
        if (error == tinyxml2::XML_ERROR_FILE_NOT_FOUND) {
            // TODO: hack to appease missing data with --clang - removed when #14456 is fixed
            if (clang)
                continue;
