...
static std::vector<ValueFlow::LifetimeToken> getLifetimeTokens(const Token* tok, ValueFlow::Value::ErrorPath errorPath, int depth)
{
    if (!tok)
    ...
    if (var && var->declarationId() == tok->varId()) {
        ...
        if (f) {
            ...
            for (const Token* returnTok : returns) {
                if (returnTok == tok)
                    continue;
                for (ValueFlow::LifetimeToken& lt : getLifetimeTokens(returnTok, std::move(errorPath), depth - 1)) {
        ...
        if (!astIsContainer(vartok) && !(tokvar && tokvar->isArray() && !tokvar->isArgument()) &&
            ...
            for (const ValueFlow::Value &v : vartok->values()) {
                ...
                return ValueFlow::getLifetimeTokens(v.tokvalue, false, std::move(errorPath));
            }
        } else {
            return ValueFlow::LifetimeToken::setAddressOf(getLifetimeTokens(vartok, std::move(errorPath), depth - 1),
...
static bool isLifetimeBorrowed(const Token *tok, const Settings& settings)
{
    if (!tok)
        return true;
    if (Token::simpleMatch(tok, ","))
...
static void valueFlowForwardLifetime(Token * tok, TokenList& tokenlist, ErrorLogger *errorLogger, const Settings& settings)
{
    ...
    if (Token::Match(tok->previous(), "%var% {|(") && isVariableDecl(tok->previous())) {
        ...
        valueFlowForward(nextAfterAstRightmostLeaf(tok), ValueFlow::getEndOfExprScope(tok), tok->previous(), std::move(values), false, false, tokenlist, errorLogger, settings);
        return;
    }
    ...
    if (parent->str() == "=" && (!parent->astParent() || Token::simpleMatch(parent->astParent(), ";"))) {
        ...
        if (expr->exprId() > 0) {
            valueFlowForward(nextExpression, endOfVarScope->next(), expr, values, false, false, tokenlist, errorLogger, settings);
            ...
            if (Token::simpleMatch(parent->astOperand1(), ".")) {
                ...
                if (parentLifetime && parentLifetime->exprId() > 0) {
                    valueFlowForward(nextExpression, endOfVarScope, parentLifetime, std::move(values), false, false, tokenlist, errorLogger, settings);
                }
            }
        }
        // Constructor
        ...
        // Only forward lifetime values
        values.remove_if(&isNotLifetimeValue);
        valueFlowForward(nextExpression, endOfVarScope, tok, std::move(values), false, false, tokenlist, errorLogger, settings);
        // Cast
...
struct LifetimeStore {
    ...
    static LifetimeStore fromFunctionArg(const Function * f, Token *tok, const Variable *var, const TokenList& tokenlist, ErrorLogger *errorLogger, const Settings& settings) {
        if (!var)
    ...
    void byVal(Token *tok, TokenList& tokenlist, ErrorLogger *errorLogger, const Settings& settings, Predicate pred) const {
        ...
        if (argtok->values().empty()) {
            ErrorPath er;
            er.emplace_back(argtok, message);
            const Variable *var = ValueFlow::getLifetimeVariable(argtok, er);
    ...
    if (Token::Match(tok->tokAt(-2), "std :: ref|cref|tie|front_inserter|back_inserter")) {
        ...
        for (const Token* returnTok : returns) {
            ...
            if (returnVar && returnVar->isArgument() && (returnVar->isConst() || !isVariableChanged(returnVar, &settings, tokenlist.isCPP()))) {
                LifetimeStore ls = LifetimeStore::fromFunctionArg(f, tok, returnVar, tokenlist, errorLogger, settings);
            ...
            for (const ValueFlow::Value &v : returnTok->values()) {
                ...
                LifetimeStore ls = LifetimeStore::fromFunctionArg(f, tok, var, tokenlist, errorLogger, settings);
                if (!ls.argtok)
                    continue;
                ls.inconclusive = inconclusive;
...
static void valueFlowContainerSize(TokenList& tokenlist, const SymbolDatabase& symboldatabase, ErrorLogger * /*errorLogger*/, const Settings& settings)
{
    // declaration
    for (const Variable *var : symboldatabase.variableList()) {
        ...
        value.setKnown();
        valueFlowContainerForward(const_cast<Token*>(var->nameToken()->next()), var->declarationId(), value, settings, tokenlist.isCPP());
    }
        ...
        for (const Token *tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "%name%|;|{|} %var% = %str% ;")) {
                ...
                if (containerTok && containerTok->valueType() && containerTok->valueType()->container && containerTok->valueType()->container->stdStringLike) {
                    ...
                    value.valueType = ValueFlow::Value::ValueType::CONTAINER_SIZE;
                    value.setKnown();
                    valueFlowContainerForward(const_cast<Token*>(containerTok->next()), containerTok->varId(), value, settings, tokenlist.isCPP());
                }
            }
        }
    }
...
void ValueFlowFast::setValues(TokenList& tokenlist, SymbolDatabase& symboldatabase, ErrorLogger *errorLogger, const Settings& settings)
{
    ...
    valueFlowLifetime(tokenlist, errorLogger, settings);
