...
Library::Error Library::load(const tinyxml2::XMLDocument& doc)
{
    ...
    int format = rootnode->IntAttribute("format", 1); // Assume format version 1 if nothing else is specified (very old
                                                      // .cfg files had no 'format' attribute)
    ...
    for (const tinyxml2::XMLElement* node = rootnode->FirstChildElement(); node; node = node->NextSiblingElement()) {
        ...
        if (nodename == "memory" || nodename == "resource") {
            ...
            int allocationId = 0;
            for (const tinyxml2::XMLElement* memorynode = node->FirstChildElement(); memorynode;
            ...
            defines.push_back(std::string(name) + " " + value);
        }
            ...
            for (const tinyxml2::XMLElement* reflectionnode = node->FirstChildElement(); reflectionnode;
                 reflectionnode = reflectionnode->NextSiblingElement()) {
            ...
            for (const tinyxml2::XMLElement* markupnode = node->FirstChildElement(); markupnode;
                 markupnode = markupnode->NextSiblingElement()) {
                const std::string markupnodename = markupnode->Name();
                if (markupnodename == "keywords") {
                    for (const tinyxml2::XMLElement* librarynode = markupnode->FirstChildElement(); librarynode;
                         librarynode = librarynode->NextSiblingElement()) {
                    ...
                    for (const tinyxml2::XMLElement* exporter = markupnode->FirstChildElement(); exporter;
                         exporter = exporter->NextSiblingElement()) {
                        ...
                        for (const tinyxml2::XMLElement* e = exporter->FirstChildElement(); e;
                    ...
                    for (const tinyxml2::XMLElement* blocknode = markupnode->FirstChildElement(); blocknode;
                         blocknode = blocknode->NextSiblingElement()) {
            ...
            for (const tinyxml2::XMLElement* containerNode = node->FirstChildElement(); containerNode;
                 containerNode = containerNode->NextSiblingElement()) {
                ...
                if (containerNodeName == "size" || containerNodeName == "access" || containerNodeName == "other") {
                    for (const tinyxml2::XMLElement* functionNode = containerNode->FirstChildElement(); functionNode;
                         functionNode = functionNode->NextSiblingElement()) {
            ...
            for (const tinyxml2::XMLElement* checkNode = node->FirstChildElement(); checkNode;
                 checkNode = checkNode->NextSiblingElement()) {
                const std::string& checkName = checkNode->Name();
                for (const tinyxml2::XMLElement* checkTypeNode = checkNode->FirstChildElement(); checkTypeNode;
                     checkTypeNode = checkTypeNode->NextSiblingElement()) {
            ...
            for (const tinyxml2::XMLElement* typenode = node->FirstChildElement(); typenode;
                                     ...
                                     std::set<std::string>& unknown_elements)
{
    ...
    for (const tinyxml2::XMLElement* functionnode = node->FirstChildElement(); functionnode;
         functionnode = functionnode->NextSiblingElement()) {
        ...
        if (functionnodename == "noreturn") {
            ...
            for (const tinyxml2::XMLElement* argnode = functionnode->FirstChildElement(); argnode;
    ...
    for (int i = 0; i + 1 < valid.size(); i++) {
        ...
        if (v1.type == InvalidArgValue::Type::le && v2.type == InvalidArgValue::Type::ge) {
            if (v1.isInt()) {
                ...
                if (op1 + 1 == op2 - 1)
                    invalid.push_back(
                        InvalidArgValue{InvalidArgValue::Type::eq, MathLib::toString(op1 + 1), std::string()});
                else
                    invalid.push_back(InvalidArgValue{
                        InvalidArgValue::Type::range, MathLib::toString(op1 + 1), MathLib::toString(op2 - 1)});
            }
        }
    }
...
bool Library::isIntArgValid(const Token* ftok, int argnr, const MathLib::bigint argvalue) const
{
    ...
    for (const Token* tok = tokenList.front(); tok; tok = tok->next()) {
        if (tok->isNumber() && argvalue == MathLib::toLongNumber(tok->str()))
            return true;
        if (Token::Match(tok, "%num% : %num%") && argvalue >= MathLib::toLongNumber(tok->str()) &&
            argvalue <= MathLib::toLongNumber(tok->strAt(2)))
            return true;
        if (Token::Match(tok, "%num% : ,") && argvalue >= MathLib::toLongNumber(tok->str()))
            return true;
        if ((!tok->previous() || tok->previous()->str() == ",") && Token::Match(tok, ": %num%") &&
            argvalue <= MathLib::toLongNumber(tok->strAt(1)))
            return true;
    }
    return false;
}
...
bool Library::isFloatArgValid(const Token* ftok, int argnr, double argvalue) const
{
    ...
    gettokenlistfromvalid(ac->valid, tokenList);
    for (const Token* tok = tokenList.front(); tok; tok = tok->next()) {
        if (Token::Match(tok, "%num% : %num%") && argvalue >= MathLib::toDoubleNumber(tok->str()) &&
            argvalue <= MathLib::toDoubleNumber(tok->strAt(2)))
            return true;
        if (Token::Match(tok, "%num% : ,") && argvalue >= MathLib::toDoubleNumber(tok->str()))
            return true;
        if ((!tok->previous() || tok->previous()->str() == ",") && Token::Match(tok, ": %num%") &&
            argvalue <= MathLib::toDoubleNumber(tok->strAt(1)))
            return true;
    }
    return false;
}
...
std::string Library::getFunctionName(const Token* ftok) const
{
    ...
    if (ftok->astParent()) {
        bool error = false;
        const Token* tok =
            ftok->astParent()->isUnaryOp("&") ? ftok->astParent()->astOperand1() : ftok->next()->astOperand1();
...
const Library::AllocFunc* Library::getAllocFuncInfo(const Token* tok) const
{
    ...
    return isNotLibraryFunction(tok) && functions.find(funcname) != functions.end() ? nullptr
                                                                                    : getAllocDealloc(mAlloc, funcname);
}
...
{
    const std::string funcname = getFunctionName(tok);
    return isNotLibraryFunction(tok) && functions.find(funcname) != functions.end()
               ? nullptr
               : getAllocDealloc(mDealloc, funcname);
}
...
const Library::AllocFunc* Library::getReallocFuncInfo(const Token* tok) const
{
    ...
    return isNotLibraryFunction(tok) && functions.find(funcname) != functions.end()
               ? nullptr
               : getAllocDealloc(mRealloc, funcname);
}
...
bool Library::isScopeNoReturn(const Token* end, std::string* unknownFunc) const
{
    ...
    if (Token::Match(end->tokAt(-2), "!!{ ; }")) {
        ...
        if (Token::simpleMatch(lastTop, "<<") && Token::simpleMatch(lastTop->astOperand1(), "(") &&
...
const Library::Container* Library::detectContainer(const Token* typeStart, bool iterator) const
{
    for (std::map<std::string, Container>::const_iterator i = containers.begin(); i != containers.end(); ++i) {
        ...
        if (!iterator && container.endPattern.empty()) // If endPattern is undefined, it will always match, but
                                                       // itEndPattern has to be defined.
            return &container;
...
bool Library::matchArguments(const Token* ftok, const std::string& functionName) const
{
    ...
    for (std::map<int, ArgumentChecks>::const_iterator it2 = it->second.argumentChecks.cbegin();
         it2 != it->second.argumentChecks.cend();
         ++it2) {
        if (it2->first > args)
            args = it2->first;
...
{
    const std::map<int, Library::ArgumentChecks>& argumentChecksFunc =
        functions.at(getFunctionName(ftok)).argumentChecks;
    for (std::map<int, Library::ArgumentChecks>::const_iterator i = argumentChecksFunc.cbegin();
         i != argumentChecksFunc.cend();
         ++i) {
...
bool Library::formatstr_scan(const Token* ftok) const { return functions.at(getFunctionName(ftok)).formatstr_scan; }

bool Library::formatstr_secure(const Token* ftok) const { return functions.at(getFunctionName(ftok)).formatstr_secure; }

const Library::NonOverlappingData* Library::getNonOverlappingData(const Token* ftok) const
{
    if (isNotLibraryFunction(ftok))
        return nullptr;
    const std::unordered_map<std::string, NonOverlappingData>::const_iterator it =
        mNonOverlappingData.find(getFunctionName(ftok));
...
std::vector<MathLib::bigint> Library::unknownReturnValues(const Token* ftok) const
{
    ...
    const std::map<std::string, std::vector<MathLib::bigint>>::const_iterator it =
        mUnknownReturnValues.find(getFunctionName(ftok));
...
bool Library::hasminsize(const Token* ftok) const
{
    ...
    if (it1 == functions.cend())
        return false;
    for (std::map<int, ArgumentChecks>::const_iterator it2 = it1->second.argumentChecks.cbegin();
         it2 != it1->second.argumentChecks.cend();
         ++it2) {
...
bool Library::processMarkupAfterCode(const std::string& path) const
{
    const std::map<std::string, bool>::const_iterator it =
        mProcessAfterCode.find(Path::getFilenameExtensionInLowerCase(path));
