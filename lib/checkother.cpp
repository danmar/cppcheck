...
static const struct CWE CWE362(362U); // Concurrent Execution using Shared Resource with Improper Synchronization ('Race
                                      // Condition')
...
void CheckOther::suspiciousSemicolonError(const Token* tok)
{
    reportError(tok,
                Severity::warning,
                "suspiciousSemicolon",
                "Suspicious use of ; at the end of '" + (tok ? tok->str() : std::string()) + "' statement.",
                CWE398,
                Certainty::normal);
}
...
void CheckOther::warningOldStylePointerCast()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        ...
        for (; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (!Token::Match(
                    tok, "( const|volatile| const|volatile| %type% * const| ) (| %name%|%num%|%bool%|%char%|%str%"))
                continue;
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (fromType->type != toType->type && fromType->type >= ValueType::Type::BOOL &&
                ...
                invalidPointerCastError(
                    tok, fromType->str(), toType->str(), toType->type == ValueType::Type::CHAR, toType->isIntegral());
            }
        }
    }
}
...
void CheckOther::checkPipeParameterSize()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "pipe ( %var% )") || Token::Match(tok, "pipe2 ( %var% ,")) {
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if ((tok->isAssignmentOp() || Token::Match(tok, "++|--")) && tok->astOperand1()) {
                ...
                if (Token::Match(tok->tokAt(-2), "; %var% =") && tok->tokAt(-2)->isSplittedVarDeclEq()) {
                    ...
                    bool trivial = true;
                    visitAstNodes(tok->astOperand2(), [&](const Token* rhs) {
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (secondBreak && (printInconclusive || !inconclusive)) {
                if (Token::Match(secondBreak, "continue|goto|throw|return") && secondBreak->isKeyword()) {
                ...
                } else if (secondBreak->str() == "break") { // break inside switch as second break statement should not
                                                            // issue a warning
                    ...
                    if (labelName && Token::Match(secondBreak, "while|do|for")) {
                        ...
                        if (scope2) {
                            for (const Token* tokIter = scope2; tokIter != scope2->link() && tokIter;
...
bool CheckOther::checkInnerScope(const Token* tok, const Variable* var, bool& used)
{
    ...
    for (; tok && tok != end; tok = tok->next()) {
        ...
        if (loopVariable && noContinue && tok->scope() == scope && !forHeadEnd && scope->type != Scope::eSwitch &&
            ...
            loopVariable = false;
            int indent = 0;
            for (const Token* tok2 = tok->tokAt(2); tok2; tok2 = tok2->next()) { // Ensure that variable isn't used on
                                                                                 // right side of =, too
                if (tok2->str() == "(")
                    indent++;
                else if (tok2->str() == ")") {
        ...
        if (!bFirstAssignment && Token::Match(tok, "* %varid%", var->declarationId())) // dereferencing means access to
                                                                                       // previous content
            return false;

        if (Token::Match(tok, "= %varid%", var->declarationId()) &&
            (var->isArray() || var->isPointer())) // Create a copy of array/pointer. Bailout, because the memory it
                                                  // points to might be necessary in outer scope
            return false;
...
//---------------------------------------------------------------------------
static int estimateSize(const Type* type,
...
{
    {
        ...
        if (func_scope->type == Function::Type::eConstructor) {
            ...
            if (func_scope->arg->link()->next()->str() == ":") {
                for (const Token* tok2 = func_scope->arg->link()->next()->next(); tok2 != var->scope()->bodyStart;
                     tok2 = tok2->next()) {
    ...
    for (const Token* tok2 = var->scope()->bodyStart; tok2 != var->scope()->bodyEnd; tok2 = tok2->next()) {
        ...
        if (parent->str() == "<<" || isLikelyStreamRead(true, parent)) {
            ...
            if (assignedVar && !assignedVar->isConst() && assignedVar->isReference() &&
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        if (var->isArgument()) {
            ...
            if (isUnusedVariable(var))
                continue;
            if (function->isConstructor() && isVariableMutableInInitializer(function->constructorMemberInitialization(),
                                                                            scope->bodyStart,
                                                                            var->declarationId()))
                continue;
        }
...
void CheckOther::constVariableError(const Variable* var, const Function* function)
{
    ...
    if (var && var->isArgument() && function && function->functionPointerUsage) {
        ...
        id += "Callback";
        message +=
            ". However it seems that '" + function->name() +
...
static bool isConstTop(const Token* tok)
{
    ...
    if (!tok->astParent())
        return true;
    if (Token::simpleMatch(tok->astParent(), ";") && Token::Match(tok->astTop()->previous(), "for|if (") &&
        Token::simpleMatch(tok->astTop()->astOperand2(), ";")) {
...
void CheckOther::zerodivError(const Token* tok, const ValueFlow::Value* value)
{
    ...
    if (value->condition) {
        ...
        errmsg << ValueFlow::eitherTheConditionIsRedundant(value->condition) << " or there is division by zero at line "
               << line << ".";
    } else
...
void CheckOther::checkMisusedScopedObject()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok && tok != scope->bodyEnd; tok = tok->next()) {
            if ((tok->next()->type() ||
                 (tok->next()->function() && tok->next()->function()->isConstructor())) // TODO: The rhs of || should be
                                                                                        // removed; It is a workaround
                                                                                        // for a symboldatabase bug
                && Token::Match(tok, "[;{}] %name% (") && Token::Match(tok->linkAt(2), ") ; !!}") &&
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        if (Token::simpleMatch(scope.bodyEnd, "} else {")) {
            ...
            if (isSameExpression(mTokenizer->isCPP(),
                                 false,
                                 branchTop1->astOperand1(),
                                 branchTop2->astOperand1(),
                                 mSettings->library,
                                 true,
                                 true,
                                 &errorPath) &&
                isSameExpression(mTokenizer->isCPP(),
                                 false,
                                 branchTop1->astOperand2(),
                                 branchTop2->astOperand2(),
                                 mSettings->library,
                                 true,
                                 true,
                                 &errorPath))
...
void CheckOther::checkInvalidFree()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            else if (Token::Match(tok, "%var% = %name% +|-") && tok->varId() == tok->tokAt(2)->varId() &&
            ...
            // added or subtracted from when used to free the memory, report an error.
                     ...
                     Token::Match(tok, "delete [ ] ( %any% +|-") || Token::Match(tok, "delete %any% +|- %any%")) {

                const int varIndex = tok->strAt(1) == "(" ? 2 : tok->strAt(3) == "(" ? 4 : 1;
...
namespace {
bool notconst(const Function* func) { return !func->isConst(); }
...
void getConstFunctions(const SymbolDatabase* symbolDatabase, std::list<const Function*>& constFunctions)
{
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        for (std::pair<const std::string, std::list<const Function*>>& it : functionsByName) {
            const std::list<const Function*>::const_iterator nc =
                std::find_if(it.second.begin(), it.second.end(), notconst);
            if (nc == it.second.end()) {
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            if (tok->str() == "=" && Token::Match(tok->astOperand1(), "%var%")) {
                ...
                if (Token::Match(endStatement, "%var% %assign%")) {
                    ...
                    if (var1 && var2 && Token::Match(var1->previous(), ";|{|} %var%") &&
                        Token::Match(var2->previous(), ";|{|} %var%") && var2->valueType() && var1->valueType() &&
                        ...
                        (tok->astOperand2()->isArithmeticalOp() || tok->astOperand2()->str() == "." ||
            ...
            if (tok->isOp() && tok->astOperand1() && !Token::Match(tok, "+|*|<<|>>|+=|*=|<<=|>>=")) {
                    ...
                    if (styleEnabled && tok->astOperand2() && tok->str() == tok->astOperand1()->str() &&
                        ...
                        while (ast1 && tok->str() == ast1->str()) {
                            if (isSameExpression(mTokenizer->isCPP(),
                                ...
                                // Probably the message should be changed to 'duplicate expressions X in condition or
                                // something like that'.
                ...
                else if (isSameExpression(mTokenizer->isCPP(),
                                          true,
                                          tok->astOperand1(),
                                          tok->astOperand2(),
                                          mSettings->library,
...
void CheckOther::oppositeExpressionError(const Token* opTok, ErrorPath errors)
{
    ...
    reportError(errors,
                Severity::style,
                "oppositeExpression",
                "Opposite expression on both sides of \'" + op +
                    "\'.\n"
                                                                 ...
                                                                 const bool result)
{
    ...
    reportError(
        ...
        "$symbol:" + functionName +
            ...
            varName + "," + varName + ") always evaluates to " + strResult +
            ".\n"
            "The function $symbol is designed to compare two variables. Calling this function with one variable (" +
            varName +
            ") "
            "for both parameters leads to a statement which is always " +
            strResult + ".",
        cweResult,
        Certainty::normal);
}
...
{
    reportError(getErrorPath(tok, v, "Unsigned less than zero"),
                Severity::style,
...
{
    reportError(getErrorPath(tok, v, "Pointer less than zero"),
                Severity::style,
                "pointerLessThanZero",
...
{
    reportError(getErrorPath(tok, v, "Unsigned positive"),
                Severity::style,
                "unsignedPositive",
                "$symbol:" + varname +
                    "\n"
...
{
    reportError(getErrorPath(tok, v, "Pointer positive"),
                Severity::style,
                "pointerPositive",
        ...
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "memset|memcpy|memmove ( %var% ,") &&
                ...
                if (MathLib::toLongNumber(tok->linkAt(1)->strAt(-1)) == var->dimension(0)) {
                    ...
                    } else if (var->valueType()->type == ValueType::Type::BOOL && printPortability) // sizeof(bool) is
                                                                                                    // not 1 on all
                                                                                                    // platforms
    ...
    if (boolean)
        reportError(
            ...
            "$symbol:" + buffer +
                ...
                "\n"
                "Array '" +
                buffer + "' might be filled incompletely. Did you forget to multiply the size given to '" + function +
                "()' with 'sizeof(*" + buffer +
                ")'?\n"
                "The array '" +
                buffer + "' is filled incompletely. The function '" + function +
        ...
        reportError(
            ...
            "$symbol:" + buffer +
                ...
                "\n"
                "Array '" +
                buffer + "' is filled incompletely. Did you forget to multiply the size given to '" + function +
                "()' with 'sizeof(*" + buffer +
                ")'?\n"
                "The array '" +
                buffer + "' is filled incompletely. The function '" + function +
...
void CheckOther::redundantPointerOpError(const Token* tok, const std::string& varname, bool inconclusive)
{
    reportError(tok,
                ...
                "$symbol:" + varname +
                    "\n"
                    "Redundant pointer operation on '$symbol' - it's already a pointer.",
                CWE398,
                inconclusive ? Certainty::inconclusive : Certainty::normal);
}
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "{|}|; %name% :") && tok->strAt(1) != "default") {
                ...
                if (!Token::findsimplematch(
                        scope->bodyStart->next(), tmp.c_str(), tmp.size(), scope->bodyEnd->previous()))
            ...
            for (const Token* tok2 = tok;; tok2 = tok2->astParent()) {
                ...
                if (tok2 == tok && tok->str() == "=" && parent->str() == "=" &&
                    isSameExpression(mTokenizer->isCPP(),
                                     ...
                                     true,
                                     false)) {
                    if (mSettings->severity.isEnabled(Severity::warning) && isSameExpression(mTokenizer->isCPP(),
                                                                                             true,
                                                                                             tok->astOperand1(),
                                                                                             parent->astOperand1(),
                                                                                             mSettings->library,
                ...
                visitAstNodes(
                    (parent->astOperand1() != tok2) ? parent->astOperand1() : parent->astOperand2(),
                    [&](const Token* tok3) {
                        ...
                        if (isSameExpression(
                                mTokenizer->isCPP(), false, tok->astOperand1(), tok3, mSettings->library, true, false))
                            foundError = true;
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        ...
        for (const Token* tok = scopeStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (tok->strAt(1) == ".") {
                ...
                accessOfMoved =
                    !isVariableChanged && checkUninitVar.isVariableUsage(tok, false, CheckUninitVar::NO_ALLOC);
                if (inconclusive) {
                                  ...
                                  bool inconclusive)
{
    if (!tok) {
        ...
        reportError(
            tok, Severity::warning, "accessForwarded", "Access of forwarded variable 'v'.", CWE672, Certainty::normal);
        return;
    }
        ...
        if (warning) {
            ...
            if (order_different) {
                funcArgOrderDifferent(
                    function->name(), function->argDef->next(), function->arg->next(), declarations, definitions);
                continue;
            }
        }
                                       ...
                                       const Token* definition)
{
    ...
    reportError(tokens,
                ...
                "$symbol:" + functionName +
                    ...
                    MathLib::toString(index + 1) + " names different: declaration '" +
                    (declaration ? declaration->str() : std::string("A")) + "' definition '" +
                    (definition ? definition->str() : std::string("B")) + "'.",
                CWE628,
                Certainty::inconclusive);
}
...
{
    std::list<const Token*> tokens = {declarations.size() ? declarations[0] ? declarations[0] : declaration : nullptr,
...
void CheckOther::checkKnownArgument()
{
    ...
    for (const Scope* functionScope : symbolDatabase->functionScopes) {
        for (const Token* tok = functionScope->bodyStart; tok != functionScope->bodyEnd; tok = tok->next()) {
            ...
            std::transform(funcname.begin(), funcname.end(), funcname.begin(), [](int c) { return std::tolower(c); });
            if (funcname.find("assert") != std::string::npos)
                continue;
            knownArgumentError(
                tok, tok->astParent()->previous(), &tok->values().front(), varexpr, isVariableExprHidden);
        }
    }
}
...
void CheckOther::comparePointersError(const Token* tok, const ValueFlow::Value* v1, const ValueFlow::Value* v2)
{
    ...
    reportError(errorPath,
                Severity::error,
                "comparePointers",
                verb + " pointers that point to different objects",
                CWE570,
                Certainty::normal);
}
            ...
            if (tok->isAssignmentOp()) {
                ...
                if (nonOverlappingData->sizeArg <= 0 || nonOverlappingData->sizeArg > args.size()) {
                    if (nonOverlappingData->sizeArg == -1) {
                        ...
                        if (!isSameExpression(
                                mTokenizer->isCPP(), macro, ptr1, ptr2, mSettings->library, pure, follow, &errorPath))
                            continue;
                ...
                const bool follow = true;
                if (!isSameExpression(
                        mTokenizer->isCPP(), macro, buf1, buf2, mSettings->library, pure, follow, &errorPath))
                    continue;
                overlappingWriteFunction(tok);
            }
        }
