...
std::pair<bool, bool> PathAnalysis::checkCond(const Token* tok, bool& known)
{
    ...
    auto it = std::find_if(
        tok->values().begin(), tok->values().end(), [](const ValueFlow::Value& v) { return v.isIntValue(); });
                                                  ...
                                                  const std::function<PathAnalysis::Progress(const Info&)>& f) const
{
    for (const Token* tok = startToken; tok && tok != endToken; tok = tok->next()) {
        if (Token::Match(tok, "asm|goto|break|continue"))
            ...
            if (!tok)
                return Progress::Break;
        } else if (Token::simpleMatch(tok, "}") && Token::simpleMatch(tok->link()->previous(), ") {") &&
            ...
            if (Token::Match(blockStart, "for|while (")) {
                ...
                if (Token::simpleMatch(blockStart, "for") &&
                    !Token::Match(endCond->tokAt(-3), "; ++|--|%var% %var%|++|-- ) {"))
                    traverseLoop = false;
...
bool reaches(const Token* start, const Token* dest, const Library& library, ErrorPath* errorPath)
{
    PathAnalysis::Info info =
        PathAnalysis{start, library}.forwardFind([&](const PathAnalysis::Info& i) { return (i.tok == dest); });
    if (!info.tok)
        return false;
    if (errorPath)
