...
static const std::vector<std::pair<std::string, std::string>> alloc_failed_conds{{"==", "0"},
                                                                                 {"<", "0"},
                                                                                 {"==", "-1"},
                                                                                 {"<=", "-1"}};
static const std::vector<std::pair<std::string, std::string>> alloc_success_conds{{"!=", "0"},
                                                                                  {">", "0"},
                                                                                  {"!=", "-1"},
                                                                                  {">=", "0"}};
...
{
    if (var->valueType() && var->valueType()->type != ValueType::Type::RECORD &&
        var->valueType()->type != ValueType::Type::UNKNOWN_TYPE)
        return false;
...
void VarInfo::print()
{
    ...
    for (std::map<int, AllocInfo>::const_iterator it = alloctype.begin(); it != alloctype.end(); ++it) {
        std::string strusage;
        const std::map<int, std::string>::const_iterator use = possibleUsage.find(it->first);
        ...
        std::cout << "status=" << status << " "
                  ...
                  << "reallocedFrom=" << it->second.reallocedFromType << std::endl;
    }
}
    ...
    for (const Token* tok = startToken; tok && tok != endToken; tok = tok->next()) {
        ...
        if (Token::Match(varTok, "%var% =")) {
            ...
            while (tokRightAstOperand && tokRightAstOperand->isCast())
                tokRightAstOperand = tokRightAstOperand->astOperand2() ? tokRightAstOperand->astOperand2()
                                                                       : tokRightAstOperand->astOperand1();
            ...
            if (Token::Match(fTok, "%type% (")) {
                ...
                const bool arrayNew = (tok2 && (tok2->str() == "[" || (tok2->str() == "(" && tok2->astOperand1() &&
                                                                       tok2->astOperand1()->str() == "[")));
                VarInfo::AllocInfo& varAlloc = alloctype[varTok->varId()];
            ...
            for (const Token* innerTok = tok->tokAt(2); innerTok && innerTok != closingParenthesis;
                ...
                if (Token::Match(innerTok, "%var% =") && innerTok->astParent() == innerTok->next()) {
                    ...
                    if (tokRightAstOperand && Token::Match(tokRightAstOperand->previous(), "%type% (")) {
                        const Library::AllocFunc* f =
                            mSettings->library.getAllocFuncInfo(tokRightAstOperand->previous());
                        if (f && f->arg == -1) {
                        ...
                        const bool arrayNew =
                            (tok2 && (tok2->str() == "[" || (tok2->str() == "(" && tok2->astOperand1() &&
                                                             tok2->astOperand1()->str() == "[")));
                        VarInfo::AllocInfo& varAlloc = alloctype[innerTok->varId()];
                        varAlloc.type = arrayNew ? NEW_ARRAY : NEW;
                        varAlloc.status = VarInfo::ALLOC;
        ...
        // Check smart pointer
            ...
            if (deleterToken) {
                ...
                if (dtok) {
                    ...
                    if (deleterToken->str() == "[" && Token::simpleMatch(deleterToken->link(), "] (") &&
            ...
            const VarInfo::AllocInfo allocation(
                af ? af->groupId : (arrayDelete ? NEW_ARRAY : NEW), VarInfo::OWNED, ftok);
...
const Token* CheckLeakAutoVar::checkTokenInsideExpression(const Token* const tok, VarInfo* varInfo)
{
    ...
    if (tok->varId() > 0) {
        ...
        if (var != varInfo->alloctype.end()) {
            bool unknown = false;
            if (var->second.status == VarInfo::DEALLOC && CheckNullPointer::isPointerDeRef(tok, unknown, mSettings) &&
                !unknown) {
                                    ...
                                    const Library::AllocFunc* af)
{
    ...
    for (const Token* funcArg = tokFirstArg; funcArg; funcArg = funcArg->nextArgument()) {
        ...
        // Check smart pointer
            ...
            const VarInfo::AllocInfo sp_allocation(
                sp_af ? sp_af->groupId : (arrayDelete ? NEW_ARRAY : NEW), VarInfo::OWNED, allocTok);
...
void CheckLeakAutoVar::ret(const Token* tok, VarInfo& varInfo, const bool isEndOfScope)
{
    ...
    for (std::map<int, VarInfo::AllocInfo>::const_iterator it = alloctype.begin(); it != alloctype.end(); ++it) {
        ...
        if (!isEndOfScope && !it->second.managed() &&
            varInfo.conditionalAlloc.find(it->first) != varInfo.conditionalAlloc.end())
            continue;
        ...
        if (var) {
            ...
            for (const Token* tok2 = tok; tok2; tok2 = tok2->next()) {
                ...
                while (tok3 && tok3->isCast() && tok3->valueType() &&
                       (tok3->valueType()->pointer || (tok3->valueType()->typeSize(*mSettings) == 0) ||
