...
static unsigned int getSizeOfType(const Token *typeTok, const Settings *settings)
{
    ...
    if (typeStr == "char")
        ...
        return typeTok->isLong() ? settings->platform.sizeof_long_long : settings->platform.sizeof_long;
    ...
    if ((tok->isNumber() && MathLib::isInt(tok->str())) || (tok->tokType() == Token::eChar)) {
        ...
        if (Token::simpleMatch(tok, "sizeof ( *")) {
            ...
            if (sz > 0) {
                ...
                if (!tok2->isTemplateArg() && settings->platform.type != cppcheck::Platform::Unspecified)
                    value.setKnown();
            ...
            if (var && var->typeStartToken() == var->typeEndToken()) {
                ...
                if (size && count > 0) {
                    ...
                    if (settings->platform.type != cppcheck::Platform::Unspecified)
                        value.setKnown();
                    setTokenValue(tok, value, settings);
                    setTokenValue(tok->next(), value, settings);
                }
...
static void valueFlowString(TokenList *tokenlist, const Settings* settings)
{
    for (Token *tok = tokenlist->front(); tok; tok = tok->next()) {
        if (tok->tokType() == Token::eString) {
            ...
            strvalue.setKnown();
            setTokenValue(tok, strvalue, settings);
        }
    }
}
                             ...
                             const Settings *settings)
{
    ...
    for (Token *tok2 = tok->previous(); ; tok2 = tok2->previous()) {
        ...
        if (tok2->varId() == varid) {
            ...
            constexpr int indirect = 0; // FIXME
            if (isVariableChangedByFunctionCall(tok2, indirect, settings, &inconclusive)) {
                if (settings->debugwarnings)
    ...
    for (Token *tok2 = startToken; tok2 && tok2 != endToken; tok2 = tok2->next()) {
        ...
        // conditional block of code that assigns variable..
            ...
            if ((number_of_if > 0 || Token::findmatch(tok2, "%varid%", start, varid)) &&
                (isEscapeScope(start, tokenlist, settings) ||
                 (Token::simpleMatch(end,"} else {") && isEscapeScope(end->tokAt(2), tokenlist, settings)))) {
                if (settings->debugwarnings)
            ...
            if (isVariableChanged(start, end, varid, var->isGlobal(), settings, tokenlist->isCPP())) {
                if ((!read || number_of_if == 0) &&
                    ...
                    !(Token::simpleMatch(end, "} else {") &&
                      isEscapeScope(end->tokAt(2), tokenlist, settings))) {
                    ++number_of_if;
                    tok2 = end;
                } else {
...
std::string ValueFlowNormal::lifetimeType(const Token *tok, const ValueFlow::Value *val)
{
    std::string result;
    if (!val)
        return "object";
...
ValueFlow::Value ValueFlowNormal::getLifetimeObjValue(const Token *tok)
{
    ValueFlow::Value result;
...
const Variable *ValueFlowNormal::getLifetimeVariable(const Token *tok, ValueFlow::Value::ErrorPath &errorPath)
{
...
bool ValueFlowNormal::isLifetimeBorrowed(const Token *tok, const Settings *settings)
{
    if (!tok)
        return true;
    if (Token::simpleMatch(tok, ","))
    ...
    if (!Token::Match(tok->astParent()->previous(), "%name% (") && !Token::simpleMatch(tok->astParent(), ",")) {
        ...
        if (!vtCont->containerTypeToken)
            return true;
        ValueType vtParent = ValueType::parseDecl(vtCont->containerTypeToken, *settings, true); // TODO: isCpp
        if (isLifetimeBorrowed(vt, &vtParent))
            return true;
        if (isLifetimeOwned(vt, &vtParent))
            return false;
...
static void valueFlowForwardLifetime(Token * tok, TokenList *tokenlist, ErrorLogger *errorLogger, const Settings *settings)
{
    ...
    if (parent->str() == "=" && (!parent->astParent() || Token::simpleMatch(parent->astParent(), ";"))) {
        ...
        if (!ValueFlowNormal::isLifetimeBorrowed(parent->astOperand2(), settings))
            return;
...
struct LifetimeStore {
    ...
    void byVal(Token *tok, TokenList *tokenlist, ErrorLogger *errorLogger, const Settings *settings, Predicate pred) const {
        if (argtok->values().empty()) {
            ErrorPath er;
            er.emplace_back(argtok, message);
            const Variable *var = ValueFlowNormal::getLifetimeVariable(argtok, er);
    ...
    void byDerefCopy(Token *tok, TokenList *tokenlist, ErrorLogger *errorLogger, const Settings *settings, Predicate pred) const {
        for (const ValueFlow::Value &v : argtok->values()) {
            ...
            ErrorPath er = v.errorPath;
            const Variable *var = ValueFlowNormal::getLifetimeVariable(tok2, er);
            er.insert(er.end(), errorPath.begin(), errorPath.end());
            if (!var)
                continue;
...
static void valueFlowLifetimeFunction(Token *tok, TokenList *tokenlist, ErrorLogger *errorLogger, const Settings *settings)
{
    ...
    if (Token::Match(tok->tokAt(-2), "std :: ref|cref|tie|front_inserter|back_inserter")) {
        ...
        } else if (!args.empty() && ValueFlowNormal::isLifetimeBorrowed(args.back(), settings)) {
...
static bool isDecayedPointer(const Token *tok, const Settings *settings)
{
    ...
    ValueType vt = ValueType::parseDecl(tok->scope()->function->retDef, *settings, true); // TODO: isCpp
    if (vt.pointer > 0)
        return true;
    return false;
}
...
static void valueFlowLifetime(TokenList *tokenlist, SymbolDatabase*, ErrorLogger *errorLogger, const Settings *settings)
{
    for (Token *tok = tokenlist->front(); tok; tok = tok->next()) {
        ...
        // container lifetimes
            ...
            setTokenValue(tok->tokAt(3), value, settings);
        ...
        // Check variables
            ...
            ErrorPath errorPath;
            const Variable * var = ValueFlowNormal::getLifetimeVariable(tok, errorPath);
            if (!var)
                continue;
                                 ...
                                 ValueFlow::Value &false_value)
{
    ...
    if (Token::simpleMatch(tok, greaterThan, 1)) {
        true_value = ValueFlow::Value{tok, value + 1};
        false_value = ValueFlow::Value{tok, value};
    } else if (Token::simpleMatch(tok, lessThan, 1)) {
        true_value = ValueFlow::Value{tok, value - 1};
        false_value = ValueFlow::Value{tok, value};
    }
}
...
static void valueFlowFunctionReturn(TokenList *tokenlist, ErrorLogger *errorLogger, const Settings* settings)
{
    for (Token *tok = tokenlist->back(); tok; tok = tok->previous()) {
        ...
        if (!error) {
            ...
            if (function->hasVirtualSpecifier() || function->isImplicitlyVirtual())
                v.setPossible();
            else
                v.setKnown();
...
const ValueFlow::Value *ValueFlowNormal::valueFlowConstantFoldAST(Token *expr, const Settings *settings)
{
...
void ValueFlowNormal::setValues(TokenList *tokenlist, SymbolDatabase* symboldatabase, ErrorLogger *errorLogger, const Settings *settings)
{
    for (Token *tok = tokenlist->front(); tok; tok = tok->next())
        tok->clearValueFlow();
