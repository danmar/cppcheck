    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok->tokAt(2), "malloc|alloca|calloc (")) {
                ...
                else if (tok->link() && Token::Match(tok, "> ( malloc|alloca|calloc (") &&
                         Token::Match(tok->link()->tokAt(-3), "%var% ="))
            ...
            for (; tokSize && tokSize->str() != ")" && tokSize->str() != "," && tokSize->str() != "sizeof";
...
void CheckSizeof::divideBySizeofError(const Token* tok, const std::string& memfunc)
{
    reportError(tok,
                ...
                "Division by result of sizeof(). " + memfunc +
...
{
    reportError(
        tok, Severity::warning, "sizeofFunctionCall", "Found function call inside sizeof().", CWE682, Certainty::normal);
}
...
void CheckSizeof::divideSizeofError(const Token* tok)
{
    reportError(tok,
                ...
                "not the size of the memory area it points to.",
                CWE682,
                Certainty::inconclusive);
}
...
{
    const std::string message =
        "'$symbol' is of type '" + vartype + "'. When using void pointers in calculations, the behaviour is undefined.";
    const std::string verbose =
        message + " Arithmetic operations on 'void *' is a GNU C extension, which defines the 'sizeof(void)' to be 1.";
    reportError(tok,
                Severity::portability,
