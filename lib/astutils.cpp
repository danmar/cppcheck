...
static void astFlattenRecursive(const Token* tok,
...
bool astIsSignedChar(const Token* tok) { return astIsCharWithSign(tok, ValueType::Sign::SIGNED); }

bool astIsUnknownSignChar(const Token* tok) { return astIsCharWithSign(tok, ValueType::Sign::UNKNOWN_SIGN); }
...
{
    return tok && (tok->isBoolean() ||
                   (tok->valueType() && tok->valueType()->type == ValueType::Type::BOOL && !tok->valueType()->pointer));
}

bool astIsPointer(const Token* tok) { return tok && tok->valueType() && tok->valueType()->pointer; }
...
std::string astCanonicalType(const Token* expr)
{
    ...
    if (decl.first && decl.second) {
        std::string ret;
        for (const Token* type = decl.first; Token::Match(type, "%name%|::") && type != decl.second;
             type = type->next()) {
...
static T* nextAfterAstRightmostLeafGeneric(T* tok)
{
    ...
    while (Token::Match(rightmostLeaf->next(), "]|)") &&
...
const Token* astParentSkipParens(const Token* tok) { return astParentSkipParens(const_cast<Token*>(tok)); }
                          ...
                          MathLib::bigint* const lastValue)
{
    ...
    if (!initExpr || !initExpr->isBinaryOp() || initExpr->str() != "=" ||
        !Token::Match(initExpr->astOperand1(), "%var%"))
        return false;
    ...
    *partialCond = Token::Match(condExpr, "%oror%|&&");
    visitAstNodes(condExpr, [varid, &condExpr](const Token* tok) {
        if (Token::Match(tok, "%oror%|&&"))
            return ChildrenToVisit::op1_and_op2;
        if (Token::Match(tok, "<|<=") && tok->isBinaryOp() && tok->astOperand1()->varId() == *varid &&
            tok->astOperand2()->hasKnownIntValue()) {
            if (Token::Match(condExpr, "%oror%|&&") ||
                tok->astOperand2()->getKnownIntValue() < condExpr->astOperand2()->getKnownIntValue())
                condExpr = tok;
        }
        return ChildrenToVisit::none;
    });
    if (!Token::Match(condExpr, "<|<=") || !condExpr->isBinaryOp() || condExpr->astOperand1()->varId() != *varid ||
        !condExpr->astOperand2()->hasKnownIntValue())
        return false;
...
static bool isInLoopCondition(const Token* tok) { return Token::Match(tok->astTop()->previous(), "for|while ("); }
...
static const Token* followVariableExpression(const Token* tok, bool cpp, const Token* end = nullptr)
{
    ...
    const Token* endToken = (isInLoopCondition(tok) || isInLoopCondition(varTok) || var->scope() != tok->scope())
                                ? var->scope()->bodyEnd
                                : lastTok;
    if (!var->isConst() &&
        (!precedes(varTok, endToken) || isVariableChanged(varTok, endToken, tok->varId(), false, nullptr, cpp)))
        return tok;
...
static void followVariableExpressionError(const Token* tok1, const Token* tok2, ErrorPath* errors)
{
    ...
    if (!tok2)
        return;
    ErrorPathItem item =
        std::make_pair(tok2, "'" + tok1->str() + "' is assigned value '" + tok2->expressionString() + "' here.");
...
std::vector<ReferenceToken> followAllReferences(const Token* tok, bool inconclusive, ErrorPath errors, int depth)
{
    struct ReferenceTokenLess {
        bool operator()(const ReferenceToken& x, const ReferenceToken& y) const { return x.token < y.token; }
    };
    if (!tok)
        ...
        if (f) {
            ...
            for (const Token* returnTok : returns) {
                ...
                for (const ReferenceToken& rt :
                     followAllReferences(returnTok, inconclusive, errors, depth - returns.size())) {
                    ...
                    if (argvar->isArgument() && (argvar->isReference() || argvar->isRValueReference())) {
                        ...
                        er.emplace_back(tok->previous(),
                                        "Called function passing '" + argTok->expressionString() + "'.");
                        std::vector<ReferenceToken> refs =
                            followAllReferences(argTok, inconclusive, std::move(er), depth - returns.size());
                        result.insert(refs.begin(), refs.end());
                        if (!inconclusive && result.size() > 1)
                            return {{tok, std::move(errors)}};
                    }
...
static bool compareKnownValue(const Token* const tok1,
                              ...
                              std::function<bool(const ValueFlow::Value&, const ValueFlow::Value&, bool)> compare)
{
...
bool isEqualKnownValue(const Token* const tok1, const Token* const tok2)
{
    return compareKnownValue(
        tok1, tok2, [&](const ValueFlow::Value& v1, const ValueFlow::Value& v2, bool sameLifetime) {
...
static bool isForLoopCondition(const Token* const tok)
{
    ...
    return Token::simpleMatch(parent, ";") && parent->astOperand1() == tok &&
           Token::simpleMatch(parent->astParent(), ";") && Token::simpleMatch(parent->astParent()->astParent(), "(") &&
...
bool isUsedAsBool(const Token* const tok) { return isUsedAsBool_internal(tok, false); }

static bool astIsBoolLike(const Token* tok) { return astIsBool(tok) || isUsedAsBool(tok); }
                      ...
                      ErrorPath* errors)
{
    ...
    // Skip double not
    if (Token::simpleMatch(tok1, "!") && Token::simpleMatch(tok1->astOperand1(), "!") &&
    ...
    if (tok1->varId() != tok2->varId() || tok1->str() != tok2->str() || tok1->originalName() != tok2->originalName()) {
        if ((Token::Match(tok1, "<|>") && Token::Match(tok2, "<|>")) ||
            ...
            return isSameExpression(
                       cpp, macro, tok1->astOperand1(), tok2->astOperand2(), library, pure, followVar, errors) &&
                   isSameExpression(
                       cpp, macro, tok1->astOperand2(), tok2->astOperand1(), library, pure, followVar, errors);
        }
        ...
        if (condTok && condTok->astOperand1() && condTok->astOperand2() && !Token::Match(exprTok, "%comp%")) {
            ...
            if (value) {
                if (value->intvalue == 0 && Token::simpleMatch(exprTok, "!") && Token::simpleMatch(condTok, "==")) {
                    compare = true;
                } else if (value->intvalue == 0 && !Token::simpleMatch(exprTok, "!") &&
    ...
    if (pure && tok1->isName() && tok1->next()->str() == "(" && tok1->str() != "sizeof" &&
        !(tok1->variable() && tok1 == tok1->variable()->nameToken())) {
        if (!tok1->function()) {
            if (Token::simpleMatch(tok1->previous(), ".")) {
                ...
                const bool lhsIsConst =
                    (lhs->variable() && lhs->variable()->isConst()) ||
            ...
            if (tok1->function() && !tok1->function()->isConst() && !tok1->function()->isAttributeConst() &&
    ...
    if ((Token::Match(tok1, "%name% <") && tok1->next()->link()) ||
        ...
        if (pure && Token::simpleMatch(tok1->next()->link(), "> (") &&
            !(tok1->function() && tok1->function()->isConst()) && tok1->str() != "dynamic_cast")
            return false;
    ...
    if (tok1->str() == "(" && tok1->previous() && !tok1->previous()->isName() &&
        !(tok1->previous()->str() == ">" && tok1->previous()->link())) {
        const Token* t1 = tok1->next();
        const Token* t2 = tok2->next();
        while (t1 && t2 && t1->str() == t2->str() && t1->isLong() == t2->isLong() &&
               t1->isUnsigned() == t2->isUnsigned() && t1->isSigned() == t2->isSigned() &&
               (t1->isName() || t1->str() == "*")) {
            t1 = t1->next();
            t2 = t2->next();
        }
    ...
    if (cpp && tok1->str() == "+" && tok1->isBinaryOp()) {
        ...
        if (!(vt1 && (vt1->type >= ValueType::VOID || vt1->pointer) && vt2 &&
              (vt2->type >= ValueType::VOID || vt2->pointer)))
            return false;
    }

    const bool commutative = tok1->isBinaryOp() && Token::Match(tok1, "%or%|%oror%|+|*|&|&&|^|==|!=");
    bool commutativeEquals =
        commutative &&
                    ...
                    ErrorPath* errors)
{
        ...
        for (const Token* tok1 : {cond1->astOperand1(), cond1->astOperand2()}) {
            for (const Token* tok2 : {cond2->astOperand1(), cond2->astOperand2()}) {
                if (isSameExpression(cpp, true, tok1, tok2, library, pure, followVar, errors)) {
                    if (isOppositeCond(
                            isNot, cpp, tok1->astSibling(), tok2->astSibling(), library, pure, followVar, errors))
                        return true;
                }
            }
        }
    ...
    if (cond1->str() == "!") {
        if (cond2->str() == "!=") {
            if (cond2->astOperand1() && cond2->astOperand1()->str() == "0")
                return isSameExpression(
                    cpp, true, cond1->astOperand1(), cond2->astOperand2(), library, pure, followVar, errors);
            if (cond2->astOperand2() && cond2->astOperand2()->str() == "0")
                return isSameExpression(
                    cpp, true, cond1->astOperand1(), cond2->astOperand1(), library, pure, followVar, errors);
        }
    ...
    if (!isNot) {
        if (cond1->str() == "==" && cond2->str() == "==") {
            ...
            if (isSameExpression(
                    cpp, true, cond1->astOperand2(), cond2->astOperand2(), library, pure, followVar, errors))
    ...
    if (isSameExpression(cpp, true, cond1->astOperand1(), cond2->astOperand1(), library, pure, followVar, errors) &&
        ...
        comp2 = cond2->str();
    } else if (isSameExpression(
                   cpp, true, cond1->astOperand1(), cond2->astOperand2(), library, pure, followVar, errors) &&
               isSameExpression(
                   cpp, true, cond1->astOperand2(), cond2->astOperand1(), library, pure, followVar, errors)) {
        comp2 = cond2->str();
        if (comp2[0] == '>')
            comp2[0] = '<';
        else if (comp2[0] == '<')
    ...
    return ((comp1 == "==" && comp2 == "!=") || (comp1 == "!=" && comp2 == "==") || (comp1 == "<" && comp2 == ">=") ||
            (comp1 == "<=" && comp2 == ">") || (comp1 == ">" && comp2 == "<=") || (comp1 == ">=" && comp2 == "<") ||
            (!isNot && ((comp1 == "<" && comp2 == ">") || (comp1 == ">" && comp2 == "<") ||
...
bool isConstExpression(const Token* tok, const Library& library, bool pure, bool cpp)
{
    ...
    if (tok->str() == "{")
        return false;
    return isConstExpression(tok->astOperand1(), library, pure, cpp) &&
           isConstExpression(tok->astOperand2(), library, pure, cpp);
}
    ...
    if (tok->function()) {
        ...
        for (const std::list<Variable>* vars : setOfVars) {
            ...
            bool other = std::any_of(vars->cbegin(), vars->cend(), [=](const Variable& v) {
                ...
                return v.isFloatingType() == var->isFloatingType() && v.isEnumType() == var->isEnumType() &&
                       v.isClass() == var->isClass() && v.isArray() == var->isArray() &&
                       v.isPointer() == var->isPointer() && v.name() != var->name();
            });
            if (other)
                return false;
        }
...
static bool hasNoreturnFunction(const Token* tok, const Library* library, const Token** unknownFunc)
{
    ...
    if (ftok) {
        ...
        return hasNoreturnFunction(tok->astOperand1(), library, unknownFunc) ||
               hasNoreturnFunction(tok->astOperand2(), library, unknownFunc);
    }
...
const Token* getTokenArgumentFunction(const Token* tok, int& argn)
{
    ...
    while (argtok && argtok->astParent() &&
           (!Token::Match(argtok->astParent(), ",|(|{") || argtok->astParent()->isCast())) {
...
std::vector<const Variable*> getArgumentVars(const Token* tok, int argnr)
{
    ...
    if (Token::Match(tok->previous(), "%type% (|{") || Token::simpleMatch(tok, "{") || tok->variable()) {
        const bool constructor =
            Token::simpleMatch(tok, "{") || (tok->variable() && tok->variable()->nameToken() == tok);
...
bool isVariableChangedByFunctionCall(const Token* tok, int indirect, const Settings* settings, bool* inconclusive)
{
    ...
    const bool possiblyPassedByReference =
        (parenTok->next() == tok1 || Token::Match(tok1->previous(), ", %name% [,)}]"));
    ...
    if (!tok->function() && !tok->variable() && Token::Match(tok, "%name%")) {
        ...
        if (!addressOf && settings) {
            ...
            if (argDirection == Library::ArgumentChecks::Direction::DIR_IN)
                ...
                // With out or inout the direction of the content is specified, not a pointer itself, so ignore pointers
                // for now
...
bool isVariableChanged(const Token* tok, int indirect, const Settings* settings, bool cpp, int depth)
{
    ...
    // Member function call
    if (tok->variable() && Token::Match(tok2->astParent(), ". %name%") && isFunctionCall(tok2->astParent()->next()) &&
    ...
    if (Token::Match(tok2->astParent(), ":|=") && tok2 == tok2->astParent()->astOperand2() &&
        Token::simpleMatch(tok2->astParent()->previous(), "]")) {
    ...
    if (Token::simpleMatch(tok2->astParent(), ":") && tok2->astParent()->astParent() &&
        Token::simpleMatch(tok2->astParent()->astParent()->previous(), "for (")) {
    ...
    for (Token* tok = start; tok != end; tok = tok->next()) {
        if (tok->exprId() != exprid) {
            ...
            if ((Token::Match(tok, "%var% (") || isVariableChangedByFunctionCall(tok, 1, settings)) &&
                std::any_of(
                    tok->values().begin(), tok->values().end(), std::mem_fn(&ValueFlow::Value::isLifetimeValue))) {
                bool aliased = false;
...
bool isVariableChanged(const Variable* var, const Settings* settings, bool cpp, int depth)
{
    ...
    if (Token::Match(start, "; %varid% =", var->declarationId()))
        start = start->tokAt(2);
    return isVariableChanged(
        start->next(), var->scope()->bodyEnd, var->declarationId(), var->isGlobal(), settings, cpp, depth);
}
                         ...
                         int depth)
{
    ...
    const Token* result = findAstNode(expr, [&](const Token* tok) {
        ...
        if (tok->exprId() > 0 && isVariableChanged(start,
                                                   end,
                                                   tok->valueType() ? tok->valueType()->pointer : 0,
                                                   tok->exprId(),
                                                   global,
                                                   settings,
                                                   cpp,
...
{
    return tok && Token::simpleMatch(tok->previous(), "> (") && tok->astOperand2() && tok->astOperand1() &&
           isCPPCastKeyword(tok->astOperand1());
}
...
bool isConstVarExpression(const Token* tok, const char* skipMatch)
{
    ...
    if (Token::Match(tok->previous(), "%name% (")) {
        ...
        std::vector<const Token*> args = getArguments(tok);
        return std::all_of(
            args.begin(), args.end(), [&](const Token* t) { return isConstVarExpression(t, skipMatch); });
    }
    if (isCPPCast(tok)) {
    ...
    if (tok->isEnumerator())
        return true;
    if (tok->variable())
        return tok->variable()->isConst() && tok->variable()->nameToken() &&
               tok->variable()->nameToken()->hasKnownValue();
    return false;
}
                        ...
                        bool local)
{
    for (const Token* tok = startToken; tok != endToken; tok = tok->next()) {
        ...
        while (parent->astParent() && !parent->astParent()->isAssignmentOp() &&
               parent->astParent()->tokType() != Token::Type::eIncDecOp) {
...
bool isNullOperand(const Token* expr)
{
    ...
    return Token::Match(castOp, "NULL|nullptr") ||
           (MathLib::isInt(castOp->str()) && MathLib::isNullValue(castOp->str()));
}
...
bool isGlobalData(const Token* expr, bool cpp)
{
    ...
    bool var = false;
    visitAstNodes(expr, [expr, cpp, &globalData, &var](const Token* tok) {
        if (tok->varId())
            var = true;
        ...
        if (tok->originalName() == "->") {
            ...
            if (lhsvar->isPointer()) {
                ...
                globalData = true;
                return ChildrenToVisit::none;
            } else if (lhsvar->isArgument() &&
                       (!lhstype || (lhstype->type <= ValueType::Type::VOID && !lhstype->container))) {
                globalData = true;
                return ChildrenToVisit::none;
            }
        }
        ...
        if (cpp && tok->str() == "." && tok->astOperand1() && tok->astOperand1()->variable() &&
...
struct FwdAnalysis::Result FwdAnalysis::checkRecursive(const Token* expr,
                                                       ...
                                                       int depth)
{
    ...
    for (const Token* tok = startToken; precedes(tok, endToken); tok = tok->next()) {
        ...
        if (const Token* lambdaEndToken = findLambdaEndToken(tok)) {
            tok = lambdaEndToken;
            const Result lambdaResult = checkRecursive(
                expr, lambdaEndToken->link()->next(), lambdaEndToken, exprVarIds, local, inInnerClass, depth);
        ...
        if (Token::Match(tok, "return|throw")) {
            ...
            FwdAnalysis::Result result = checkRecursive(
                expr, startEndTokens.first, startEndTokens.second->next(), exprVarIds, local, true, depth);
            if (result.type != Result::Type::NONE)
                return result;
        ...
        if (tok->str() == "}") {
            ...
            if (tok->scope()->isLoopScope()) {
                ...
                const struct FwdAnalysis::Result& result =
                    checkRecursive(expr, tok->link(), tok, exprVarIds, local, inInnerClass, depth);
            ...
            if (condTok->hasKnownIntValue()) {
                ...
                if (cond) {
                    FwdAnalysis::Result result =
                        checkRecursive(expr, bodyStart, bodyStart->link(), exprVarIds, local, true, depth);
                    if (result.type != Result::Type::NONE)
                        return result;
        ...
        if (mWhat == What::Reassign && Token::simpleMatch(tok, ";") && Token::simpleMatch(tok->astParent(), ";") &&
        ...
        if (expr->isName() && Token::Match(tok, "%name% (") && tok->str().find("<") != std::string::npos &&
            tok->str().find(expr->str()) != std::string::npos)
            return Result(Result::Type::BAILOUT);
        ...
        if (exprVarIds.find(tok->varId()) != exprVarIds.end()) {
            ...
            if (mWhat != What::ValueFlow && same && Token::simpleMatch(parent->astParent(), "[") &&
                parent == parent->astParent()->astOperand2()) {
            ...
            if (Token::simpleMatch(parent->astParent(), "=") && parent == parent->astParent()->astOperand1()) {
                ...
                return Result(Result::Type::READ);
            } else if (mWhat == What::Reassign && parent->valueType() && parent->valueType()->pointer &&
                       Token::Match(parent->astParent(), "%assign%") && parent == parent->astParent()->astOperand1()) {
                return Result(Result::Type::READ);
            } else if (Token::Match(parent->astParent(), "%assign%") && !parent->astParent()->astParent() &&
                       parent == parent->astParent()->astOperand1()) {
                if (mWhat == What::Reassign)
                    return Result(Result::Type::BAILOUT, parent->astParent());
                continue;
            } else if (mWhat == What::UnusedValue && parent->isUnaryOp("&") &&
                       Token::Match(parent->astParent(), "[,(]")) {
                ...
                if (ftok && Token::Match(ftok->previous(), "%name% (")) {
                    ...
                    if (argnr < args.size()) {
                        ...
                        if (functionInfo) {
                            ...
                            if (it != functionInfo->argumentChecks.end() &&
                                it->second.direction == Library::ArgumentChecks::Direction::DIR_OUT)
                                continue;
                        }
                    }
                }
        ...
        if (Token::Match(tok, ")|do {")) {
            ...
            const Result& result1 =
                checkRecursive(expr, tok->tokAt(2), tok->linkAt(1), exprVarIds, local, inInnerClass, depth);
            ...
            if (mWhat == What::Reassign && result1.type == Result::Type::BREAK) {
                ...
                if (scopeEndToken) {
                    const Result& result2 =
                        checkRecursive(expr, scopeEndToken->next(), endToken, exprVarIds, local, inInnerClass, depth);
            ...
            if (Token::simpleMatch(tok->linkAt(1), "} else {")) {
                ...
                const Result& result2 =
                    checkRecursive(expr, elseStart, elseStart->link(), exprVarIds, local, inInnerClass, depth);
...
{
    bool ret = false;
    visitAstNodes(expr, [&ret](const Token* tok) {
...
bool FwdAnalysis::isGlobalData(const Token* expr) const { return ::isGlobalData(expr, mCpp); }
        ...
        if (tok->varId() > 0) {
            ...
            if (!Token::simpleMatch(tok->previous(), ".")) {
                ...
                if (var && var->isReference() && var->isLocal() && Token::Match(var->nameToken(), "%var% [=(]") &&
                    ...
                    return ChildrenToVisit::none;
                const bool deref =
                    tok->astParent() && (tok->astParent()->isUnaryOp("*") ||
...
bool FwdAnalysis::unusedValue(const Token* expr, const Token* startToken, const Token* endToken)
{
    ...
    Result result = check(expr, startToken, endToken);
    return (result.type == FwdAnalysis::Result::Type::NONE || result.type == FwdAnalysis::Result::Type::RETURN) &&
           !possiblyAliased(expr, startToken);
}
        ...
        if (Token::Match(tok, "%name% (") && !Token::Match(tok, "if|while|for")) {
            ...
            for (int argnr = 0; argnr < args.size(); ++argnr) {
                ...
                if (tok->function() && tok->function()->getArgumentVar(argnr) &&
                    !tok->function()->getArgumentVar(argnr)->isReference() && !tok->function()->isConst())
                    continue;
