...
public:
    ...
    enum class NeedInitialization { Unknown, True, False } needInitialization;
    ...
    public:
        ...
        bool operator<(const BaseInfo& rhs) const { return this->type < rhs.type; }
    };
...
public:
    explicit Enumerator(const Scope* scope_)
        : scope(scope_), name(nullptr), value(0), start(nullptr), end(nullptr), value_known(false)
    {}
    const Scope* scope;
    const Token* name;
...
class CPPCHECKLIB Variable {
     ...
     */
    bool getFlag(unsigned int flag_) const { return ((mFlags & flag_) != 0); }
     ...
     */
    void setFlag(unsigned int flag_, bool state_) { mFlags = state_ ? mFlags | flag_ : mFlags & ~flag_; }
...
public:
     ...
     * @return true if the type is known and it's a floating type (float, double and long double) or a pointer/array to
     * it
     */
...
private:
     ...
     * @param t type
     */
    void type(const Type* t) { mType = t; }
...
public:
    ...
    bool argsMatch(const Scope* scope,
                   const Token* first,
                   const Token* second,
...
public:
    ...
    Scope(const SymbolDatabase* check_,
    ...
    bool isLocal() const
    {
        return (type == eIf || type == eElse || type == eFor || type == eWhile || type == eDo || type == eSwitch ||
                type == eUnconditional || type == eTry || type == eCatch);
    }
