...
ErrorMessage::ErrorMessage(const tinyxml2::XMLElement* const errmsg)
    : incomplete(false), severity(Severity::none), cwe(0U), certainty(Certainty::normal)
{
...
void ErrorMessage::setmsg(const std::string& msg)
{
    ...
    const std::string symbolName =
        mSymbolNames.empty() ? std::string() : mSymbolNames.substr(0, mSymbolNames.find('\n'));
...
std::string ErrorMessage::serialize() const
{
    ...
    for (std::list<ErrorMessage::FileLocation>::const_iterator loc = callStack.begin(); loc != callStack.end(); ++loc) {
        ...
        smallStream << (*loc).line << '\t' << (*loc).column << '\t' << (*loc).getfile(false) << '\t'
...
bool ErrorMessage::deserialize(const std::string& data)
{
    ...
    while (iss.good()) {
        ...
        // (*loc).line << '\t' << (*loc).column << '\t' << (*loc).getfile(false) << '\t' << loc->getOrigFile(false) <<
        // '\t' << loc->getinfo();

        ErrorMessage::FileLocation loc(
            substrings[3], MathLib::toLongNumber(substrings[0]), MathLib::toLongNumber(substrings[1]));
...
std::string ErrorMessage::getXMLFooter() { return "    </errors>\n</results>"; }
        ...
        for (const FileLocation& fileLocation : callStack) {
            ...
            if (text.find("{code}") != std::string::npos) {
                ...
                if (pos == std::string::npos)
                    ...
                    endl = "\r";
                findAndReplace(
                    text, "{code}", readCode(fileLocation.getOrigFile(), fileLocation.line, fileLocation.column, endl));
            }
            result += '\n' + text;
        }
    }
...
bool ErrorLogger::reportUnmatchedSuppressions(const std::list<Suppressions::Suppression>& unmatched)
{
    ...
    for (const Suppressions::Suppression& s : unmatched) {
        ...
        reportErr(ErrorMessage(callStack,
                               ...
                               "Unmatched suppression: " + s.errorId,
...
ErrorMessage::FileLocation::FileLocation(const Token* tok, const TokenList* tokenList)
    : fileIndex(tok->fileIndex()),
      line(tok->linenr()),
      column(tok->column()),
      mOrigFileName(tokenList->getOrigFile(tok)),
      mFileName(tokenList->file(tok))
{}
...
std::string ErrorLogger::plistData(const ErrorMessage& msg)
{
    ...
    for (std::list<ErrorMessage::FileLocation>::const_iterator it = msg.callStack.begin(); it != msg.callStack.end();
         ++it) {
        if (prev != it) {
            plist << "    <dict>\r\n"
        ...
        const std::string message =
            (it->getinfo().empty() && next == msg.callStack.end() ? msg.shortMessage() : it->getinfo());
