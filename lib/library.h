...
public:
    ...
    const AllocFunc* getAllocFuncInfo(const char name[]) const { return getAllocDealloc(mAlloc, name); }

    /** get deallocation info for function by name (deprecated, use other alloc) */
    const AllocFunc* getDeallocFuncInfo(const char name[]) const { return getAllocDealloc(mDealloc, name); }
    ...
    static bool ismemory(const int id) { return ((id > 0) && ((id & 1) == 0)); }
    static bool ismemory(const AllocFunc* const func) { return ((func->groupId > 0) && ((func->groupId & 1) == 0)); }

    /** is allocation type resource? */
    static bool isresource(const int id) { return ((id > 0) && ((id & 1) == 1)); }
    static bool isresource(const AllocFunc* const func) { return ((func->groupId > 0) && ((func->groupId & 1) == 1)); }
    ...
    public:
        ...
        enum class Direction {
            ...
            DIR_INOUT,  ///< Input to called function, and output to caller. Data is passed by reference or address and
    ...
    struct InvalidArgValue {
        ...
        std::string op1;
        std::string op2;
        bool isInt() const { return MathLib::isInt(op1); }
    };
    ...
    const std::set<std::string>& markupExtensions() const { return mMarkupExtensions; }
    ...
    bool isexporter(const std::string& prefix) const { return mExporters.find(prefix) != mExporters.end(); }
    ...
    bool isreflection(const std::string& token) const { return mReflection.find(token) != mReflection.end(); }
    ...
    struct PlatformType {
        PlatformType()
            : mSigned(false), mUnsigned(false), mLong(false), mPointer(false), mPtrPtr(false), mConstPtr(false)
        {}
        bool operator==(const PlatformType& type) const
        {
            return (mSigned == type.mSigned && mUnsigned == type.mUnsigned && mLong == type.mLong &&
                    mPointer == type.mPointer && mPtrPtr == type.mPtrPtr && mConstPtr == type.mConstPtr &&
                    mType == type.mType);
        }
        bool operator!=(const PlatformType& type) const { return !(*this == type); }
        std::string mType;
        bool mSigned;
        bool mUnsigned;
        bool mLong;
    ...
    static bool isContainerYield(const Token* const cond,
...
private:
    ...
    Error loadFunction(const tinyxml2::XMLElement* const node,
                       const std::string& name,
                       std::set<std::string>& unknown_elements);

    class ExportedFunctions {
    public:
        void addPrefix(const std::string& prefix) { mPrefixes.insert(prefix); }
        void addSuffix(const std::string& suffix) { mSuffixes.insert(suffix); }
        bool isPrefix(const std::string& prefix) const { return (mPrefixes.find(prefix) != mPrefixes.end()); }
        bool isSuffix(const std::string& suffix) const { return (mSuffixes.find(suffix) != mSuffixes.end()); }
    ...
    public:
        ...
        void setStart(const char* s) { mStart = s; }
        void setEnd(const char* e) { mEnd = e; }
        void setOffset(const int o) { mOffset = o; }
        void addBlock(const char* blockName) { mBlocks.insert(blockName); }
        const std::string& start() const { return mStart; }
        const std::string& end() const { return mEnd; }
        int offset() const { return mOffset; }
        bool isBlock(const std::string& blockName) const { return mBlocks.find(blockName) != mBlocks.end(); }
    ...
    std::map<std::string, ExportedFunctions> mExporters;       // keywords that export variables/functions to libraries
                                                               // (meta-code/macros)
