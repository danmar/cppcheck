...
void CheckMemoryLeakInClass::variable(const Scope *scope, const Token *tokVarname)
{
    ...
    for (const Function &func : scope->functionList) {
        ...
        for (const auto& tok : IterateTokens(func.arg->link(), end)) {
...
bool CheckMemoryLeakStructMember::isMalloc(const Variable *variable)
{
    ...
    bool alloc = false;
    for (const auto &tok2 : IterateTokens(variable->nameToken(), variable->scope()->bodyEnd)) {
...
void CheckMemoryLeakStructMember::checkStructVariable(const Variable * const variable)
{
    ...
    auto deallocInFunction = [this](const Token* tok, int structid) -> bool {
        ...
        for (const auto &tok2 : IterateTokens(tok, end)) {
        ...
        // Struct member is allocated => check if it is also properly deallocated..
            ...
            for (const Token *tok3 = tok2; tok3; tok3 = tok3->next()) {
                ...
                // succeeded allocation
                    ...
                    int indentlevel4 = 0;
                    for (const auto& tok4 : IterateTokens(tok3)) {
                        if (tok4->str() == "{")
                            ++indentlevel4;
                        else if (tok4->str() == "}") {
                            --indentlevel4;
        ...
        if (Token::Match(tok, "%name% (")) {
            ...
            if (pointerType && functionCalled) {
                ...
                if (functionCalled->strAt(1) == "<") {
                    functionName += '<';
                    for (const auto& tok2 : IterateTokens(functionCalled->tokAt(2), functionCalled->next()->link()))
                        functionName += tok2->str();
                    functionName += '>';
                }
                std::string objectTypeName;
                for (const auto& tok2 : IterateTokens(pointerType->tokAt(2), pointerType->next()->link()))
