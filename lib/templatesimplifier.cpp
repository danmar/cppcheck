...
public:
    ...
    bool operator()(const TemplateSimplifier::TokenAndName& tokenAndName) const { return tokenAndName.name() == mName; }
      ...
      mFlags(0)
{
    ...
    if (mToken && mNameToken && mParamEnd) {
        ...
        if (isAlias() && isPartialSpecialization()) {
            throw InternalError(
                mToken, "partial specialization of alias templates is not permitted", InternalError::SYNTAX);
        }
        if (isAlias() && isSpecialization()) {
            throw InternalError(
                mToken, "explicit specialization of alias templates is not permitted", InternalError::SYNTAX);
        }
        ...
        if ((isFunction() || isClass()) &&
            ...
            while (start && (Token::Match(start->tokAt(-2), "%name% ::") ||
                             (Token::simpleMatch(start->tokAt(-2), "> ::") && start->tokAt(-2)->findOpeningBracket() &&
...
TemplateSimplifier::TokenAndName::TokenAndName(const TokenAndName& other)
    : mToken(other.mToken),
      ...
      mNameToken(other.mNameToken),
      mParamEnd(other.mParamEnd),
      mFlags(other.mFlags)
{
    if (mToken)
        mToken->templateSimplifierPointer(this);
...
TemplateSimplifier::TemplateSimplifier(Tokenizer* tokenizer)
    : mTokenizer(tokenizer),
      mTokenList(tokenizer->list),
      mSettings(tokenizer->mSettings),
      mErrorLogger(tokenizer->mErrorLogger),
      mChanged(false)
{}
                ...
                if (tok2->str() == "(")
                    ...
                    if (Token::simpleMatch(tok2->previous(), "operator <"))
                        ...
                        inclevel = true;
                             ...
                             usedtypes.find(tok2->previous()->str()) != usedtypes.end())
                        inclevel = true;
...
bool TemplateSimplifier::getTemplateDeclarations()
{
    ...
    for (Token* tok = mTokenList.front(); tok; tok = tok->next()) {
        ...
        if (!Token::Match(tok, "%any% %any%"))
            syntaxError(tok);
        if (tok->strAt(2) == "typename" && !Token::Match(tok->tokAt(3), "%name%|...|,|=|>"))
...
void TemplateSimplifier::addInstantiation(Token* token, const std::string& scope)
{
    ...
    std::list<TokenAndName>::iterator it =
        std::find(mTemplateInstantiations.begin(), mTemplateInstantiations.end(), instantiation);
    ...
    for (Token* tok = mTokenList.front(); tok; tok = tok->next()) {
        ...
        if (Token::simpleMatch(tok, "template <")) {
            ...
            if (tok->strAt(1) == "(") {
                ...
                for (auto pos = functionNameMap.lower_bound(tok->str()); pos != functionNameMap.upper_bound(tok->str());
                     ++pos) {
            ...
            if (templateParameters(tok->next()) || tok->strAt(2) == ">") {
                ...
                while (true) {
                    const std::string fullName = scopeName + (scopeName.empty() ? "" : " :: ") + qualification +
                                                 (qualification.empty() ? "" : " :: ") + tok->str();
                    const std::list<TokenAndName>::const_iterator it = std::find_if(
                        mTemplateDeclarations.begin(), mTemplateDeclarations.end(), FindFullName(fullName));
                    if (it != mTemplateDeclarations.end()) {
                        ...
                        for (const auto& nameSpace : tok->scopeInfo()->usingNamespaces) {
                            std::string fullNameSpace = scopeName + (scopeName.empty() ? "" : " :: ") + nameSpace +
                                                        (qualification.empty() ? "" : " :: ") + qualification;
                            std::string newFullName = fullNameSpace + " :: " + tok->str();
                            const std::list<TokenAndName>::const_iterator it1 = std::find_if(
                                mTemplateDeclarations.begin(), mTemplateDeclarations.end(), FindFullName(newFullName));
...
void TemplateSimplifier::useDefaultArgumentValues(TokenAndName& declaration)
{
    ...
    for (Token* tok = declaration.token()->next(); tok; tok = tok->next()) {
        ...
        if (tok->str() == "<" &&
            (tok->strAt(1) == ">" ||
             (tok->previous()->isName() && typeParameterNames.find(tok->strAt(-1)) == typeParameterNames.end())))
            ++templateParmDepth;
    ...
    for (const auto& entry : eq) {
        ...
        for (tok2 = eqtok->next(); tok2; tok2 = tok2->next()) {
            ...
            if (Token::Match(tok2, "(|{|["))
                ...
                std::list<TokenAndName>::iterator ti =
                    std::find_if(mTemplateInstantiations.begin(), mTemplateInstantiations.end(), FindToken(tok2));
        ...
        std::list<TokenAndName>::iterator ti2 =
            std::find_if(mTemplateInstantiations.begin(), mTemplateInstantiations.end(), FindName(declaration.name()));
...
void TemplateSimplifier::simplifyTemplateAliases()
{
    for (std::list<TokenAndName>::iterator it1 = mTemplateDeclarations.begin(); it1 != mTemplateDeclarations.end();) {
        ...
        bool found = false;
        for (std::list<TokenAndName>::iterator it2 = mTemplateInstantiations.begin();
             it2 != mTemplateInstantiations.end();) {
            ...
            Token* end =
                TokenList::copyTokens(
                    dst, aliasDeclaration.aliasStartToken(), aliasDeclaration.aliasEndToken()->previous(), false)
                    ->next();
            ...
            for (Token* tok1 = dst->next(); tok1 != end; tok1 = tok1->next()) {
                ...
                if (aliasParameterNames.find(tok2->str()) == aliasParameterNames.end()) {
                    ...
                    if (Token::Match(tok1, "%name% <")) {
                        const std::list<TokenAndName>::iterator it = std::find_if(
                            mTemplateInstantiations.begin(), mTemplateInstantiations.end(), FindToken(tok1));
                        if (it != mTemplateInstantiations.end())
    ...
    if (token != tokStart->str() || tok->strAt(-1) != "::") {
        if (insert) {
            if (!inTemplate)
                ...
                mTokenList.back()->tokAt(offset)->str(mTokenList.back()->strAt(offset) +
                                                      templateDeclaration.scope().substr(start));
            mTokenList.back()->tokAt(offset)->insertToken("::", "");
        } else {
            if (!inTemplate)
                mTokenList.addtoken(
                    templateDeclaration.scope().substr(start), tok->linenr(), tok->column(), tok->fileIndex());
            else
                                        ...
                                        bool copy)
{
    ...
    if (copy && isClass) {
        ...
        templateDeclaration.token()->insertToken(";", "", true);
               ...
               (isClass && isSpecialization &&
                mTemplateSpecializationMap.find(templateDeclaration.token()) != mTemplateSpecializationMap.end())) {
        ...
        if (!isSpecialization && it != mTemplateForwardDeclarationsMap.end()) {
            ...
            if (it != mTemplateForwardDeclarationsMap.end()) {
                std::list<TokenAndName>::iterator it1 = std::find_if(
                    mTemplateForwardDeclarations.begin(), mTemplateForwardDeclarations.end(), FindToken(it->second));
        ...
        while (start && start != end) {
            ...
            while (itype < typeParametersInDeclaration.size() &&
                   typeParametersInDeclaration[itype]->str() != start->str())
                ++itype;
            ...
            if (itype < typeParametersInDeclaration.size() && itype < mTypesUsedInTemplateInstantiation.size() &&
                ...
                const bool isVariadicTemplateArg =
                    templateDeclaration.isVariadic() && itype + 1 == typeParametersInDeclaration.size();
                ...
                if (isSpecialization && !copy && !scope.empty() &&
                    Token::Match(start, (scope + templateDeclarationNameToken->str()).c_str())) {
                    // skip scope
    ...
    for (Token* tok3 = mTokenList.front(); tok3; tok3 = tok3 ? tok3->next() : nullptr) {
        ...
        // member function implemented outside class definition
        else if (inTemplateDefinition && Token::Match(tok3, "%name% <") &&
                 templateInstantiation.name() == tok3->str() &&
                 instantiateMatch(
                     tok3, typeParametersInDeclaration.size(), templateDeclaration.isVariadic(), ":: ~| %name% (")) {
            ...
            while (tok5 && tok5 != tok3) {
                ...
                if (Token::Match(tok5, "%name% <") && tok5->str() == templateInstantiation.name()) {
                    ...
                    if (tok5->isName() && !Token::Match(tok5, "class|typename|struct") && !tok5->isStandardType()) {
                        ...
                        unsigned int itype = 0;
                        while (itype < typeParametersInDeclaration.size() &&
                               typeParametersInDeclaration[itype]->str() != tok5->str())
                            ++itype;

                        // replace type with given type..
                        if (itype < typeParametersInDeclaration.size() &&
                            itype < mTypesUsedInTemplateInstantiation.size()) {
                            unsigned int typeindentlevel = 0;
                            std::stack<Token*> brackets1; // holds "(" and "{" tokens
                            for (const Token* typetok = mTypesUsedInTemplateInstantiation[itype].token();
            ...
            std::list<TokenAndName>::iterator it = std::find_if(
                mTemplateDeclarations.begin(), mTemplateDeclarations.end(), FindToken(startOfTemplateDeclaration));
            if (it != mTemplateDeclarations.end())
        ...
        const std::string lastName =
            (templateInstantiation.name().find(' ') != std::string::npos)
                ? templateInstantiation.name().substr(templateInstantiation.name().rfind(' ') + 1)
                : templateInstantiation.name();
        ...
        for (; tok3; tok3 = tok3->next()) {
            if (tok3->isName() && !Token::Match(tok3, "class|typename|struct") && !tok3->isStandardType()) {
                ...
                unsigned int itype = 0;
                while (itype < typeParametersInDeclaration.size() &&
                       typeParametersInDeclaration[itype]->str() != tok3->str())
                    ++itype;
            ...
            // link() newly tokens manually
                ...
                if (tok3->str() == "{") {
                    ...
                    if (tok3->strAt(1) == ";") {
                        ...
                        mTokenList.addtoken(
                            tokSemicolon, tokSemicolon->linenr(), tokSemicolon->column(), tokSemicolon->fileIndex());
                    }
                    brackets.pop();
...
static bool isLowerThanOr(const Token* lower) { return isLowerThanLogicalAnd(lower) || lower->str() == "&&"; }
static bool isLowerThanXor(const Token* lower) { return isLowerThanOr(lower) || lower->str() == "|"; }
static bool isLowerThanAnd(const Token* lower) { return isLowerThanXor(lower) || lower->str() == "^"; }
static bool isLowerThanShift(const Token* lower) { return isLowerThanAnd(lower) || lower->str() == "&"; }
static bool isLowerThanPlusMinus(const Token* lower)
{
    return isLowerThanShift(lower) || Token::Match(lower, "%comp%|<<|>>");
}
static bool isLowerThanMulDiv(const Token* lower) { return isLowerThanPlusMinus(lower) || Token::Match(lower, "+|-"); }
...
bool TemplateSimplifier::simplifyNumericCalculations(Token* tok, bool isTemplate)
{
    ...
    while (tok->tokAt(3) && tok->isNumber() && tok->tokAt(2)->isNumber()) { // %any% %num% %any% %num% %any%
        ...
        if (Token::Match(before, "* %num% /") && (num2 != "0") &&
            num1 == MathLib::multiply(num2, MathLib::divide(num1, num2))) {
            // Division where result is a whole number
        } else if (!((op->str() == "*" && (isLowerThanMulDiv(before) || before->str() == "*") &&
                      isLowerEqualThanMulDiv(after)) || // associative
                     (Token::Match(op, "[/%]") && isLowerThanMulDiv(before) &&
                      isLowerEqualThanMulDiv(after)) || // NOT associative
                     (Token::Match(op, "[+-]") && isLowerThanMulDiv(before) && isLowerThanMulDiv(after)) || // Only
                                                                                                            // partially
                                                                                                            // (+)
                                                                                                            // associative,
                                                                                                            // but
                                                                                                            // handled
                                                                                                            // later
                     (Token::Match(op, ">>|<<") && isLowerThanShift(before) &&
                      isLowerThanPlusMinus(after)) ||                                             // NOT associative
        ...
        if (Token::Match(op, "[/%] 0")) {
            if (isTemplate)
                throw InternalError(
                    op, "Instantiation error: Divide by zero in template instantiation.", InternalError::INSTANTIATION);
            else
                return ret;
        }
...
void TemplateSimplifier::simplifyTemplateArgs(Token* start, Token* end)
{
    ...
    while (again) {
        ...
        for (Token* tok = first->next(); tok && tok != end; tok = tok->next()) {
            ...
            } else if (Token::Match(tok, "%num% %comp% %num%") && MathLib::isInt(tok->str()) &&
...
bool TemplateSimplifier::simplifyCalculations(Token* frontToken, Token* backToken, bool isTemplate)
{
    ...
    for (Token* tok = frontToken; tok && tok != backToken; tok = tok->next()) {
        ...
        if (validTokenEnd(bounded, tok, backToken, 4) &&
            (Token::Match(tok->next(), "( %name% ) ;|)|,|]") ||
             (Token::Match(tok->next(), "( %name% ) %cop%") &&
              (tok->tokAt(2)->varId() > 0 || !Token::Match(tok->tokAt(4), "[*&+-~]")))) &&
            !tok->isName() && tok->str() != ">" && tok->str() != ")" && tok->str() != "]") {
            tok->deleteNext();
            tok = tok->next();
            tok->deleteNext();
            ret = true;
        ...
        if (validTokenEnd(bounded, tok, backToken, 5) && Token::Match(tok, "decltype ( %type% { } )")) {
            tok->deleteThis();
            tok->deleteThis();
            tok->deleteNext();
            tok->deleteNext();
        ...
        if (validTokenEnd(bounded, tok, backToken, 3) && Token::Match(tok, "decltype ( %bool%|%num% )")) {
            tok->deleteThis();
            tok->deleteThis();
            if (tok->isBoolean())
                tok->str("bool");
        ...
        if (validTokenEnd(bounded, tok, backToken, 2) &&
            (Token::Match(tok, "char|short|int|long { }") || Token::Match(tok, "char|short|int|long ( )"))) {
        ...
        if (tok && tok->isNumber()) {
            if (validTokenEnd(bounded, tok, backToken, 2) && simplifyNumericCalculations(tok, isTemplate)) {
                ret = true;
                Token* prev = tok->tokAt(-2);
                while (validTokenStart(bounded, tok, frontToken, -2) && prev &&
                       simplifyNumericCalculations(prev, isTemplate)) {
                    tok = prev;
                    prev = prev->tokAt(-2);
                }
            }
            ...
            if (validTokenStart(bounded, tok, frontToken, -1) && validTokenEnd(bounded, tok, backToken, 1) &&
                (Token::Match(tok->previous(), "[(=,] 0 &&") || Token::Match(tok->previous(), "[(=,] 1 %oror%"))) {
                unsigned int par = 0;
            ...
            if ((Token::Match(tok->tokAt(-2), "%any% * 1") && validTokenStart(bounded, tok, frontToken, -2)) ||
                (Token::Match(tok->previous(), "%any% 1 *") && validTokenStart(bounded, tok, frontToken, -1))) {
                tok = tok->previous();
                if (tok->str() == "*")
                    tok = tok->previous();
                tok->deleteNext(2);
            ...
            if (validTokenStart(bounded, tok, frontToken, -2) && Token::Match(tok->tokAt(-2), "%op%|< ( %num% )") &&
                tok->strAt(-2) != ">") {
                tok = tok->previous();
                tok->deleteThis();
                tok->deleteNext();
            ...
            if (validTokenStart(bounded, tok, frontToken, -1) && validTokenEnd(bounded, tok, backToken, 1) &&
                (Token::Match(tok->previous(), "( 0 [|+]") || Token::Match(tok->previous(), "[|+-] 0 )"))) {
                tok = tok->previous();
                if (Token::Match(tok, "[|+-]"))
                    tok = tok->previous();
                tok->deleteNext(2);
            ...
            if (validTokenEnd(bounded, tok, backToken, 2) && Token::Match(tok, "%num% %comp% %num%") &&
                MathLib::isInt(tok->str()) && MathLib::isInt(tok->strAt(2))) {
                if (validTokenStart(bounded, tok, frontToken, -1) && Token::Match(tok->previous(), "(|&&|%oror%") &&
                                             ...
                                             const std::list<const Token*>& specializations)
{
    ...
    for (std::list<const Token*>::const_iterator it = specializations.begin(); it != specializations.end(); ++it) {
        ...
        while (declToken != endToken) {
            if (declToken->str() != instToken->str() || declToken->isSigned() != instToken->isSigned() ||
                declToken->isUnsigned() != instToken->isUnsigned() || declToken->isLong() != instToken->isLong()) {
                int nr = 0;
...
std::string TemplateSimplifier::getNewName(Token* tok2, std::list<std::string>& typeStringsUsedInTemplateInstantiation)
{
    std::string typeForNewName;
    unsigned int indentlevel = 0;
    const Token* endToken = tok2->next()->findClosingBracket();
    for (Token* tok3 = tok2->tokAt(2); tok3 != endToken && (indentlevel > 0 || tok3->str() != ">");
         tok3 = tok3->next()) {
                                                        ...
                                                        std::set<std::string>& expandedtemplates)
{
    ...
    for (const TokenAndName& instantiation : mTemplateInstantiations) {
        ...
        if (numberOfTemplateInstantiations != mTemplateInstantiations.size()) {
            ...
            if (recursiveCount > mSettings->maxTemplateRecursion) {
                ...
                const std::string typeForNewName =
        ...
        if (templateDeclaration.isFunction() && instantiation.isFunction()) {
            ...
            if (declFuncArgs.size() != instFuncParams.size()) {
                ...
                if (instFuncParams.size() < (declFuncArgs.size() - count) ||
                    instFuncParams.size() > declFuncArgs.size())
                    continue;
            }
        }
        ...
        if (mErrorLogger && !mTokenList.getFiles().empty())
            mErrorLogger->reportProgress(mTokenList.getFiles()[0],
                                         "TemplateSimplifier::simplifyTemplateInstantiations()",
                                         tok2->progressValue());
#ifdef MAXTIME
        ...
        if (Token::Match(startToken->previous(), ";|{|}|=|const") &&
            (!specialized && !instantiateMatch(tok2,
                                               typeParametersInDeclaration.size(),
                                               templateDeclaration.isVariadic(),
                                               isfunc ? "(" : isVar ? ";|%op%|(" : "*|&|::| %name%")))
            continue;
        ...
        if ((typeForNewName.empty() && !templateDeclaration.isVariadic()) ||
            (!typeParametersInDeclaration.empty() &&
             !instantiateMatch(tok2, typeParametersInDeclaration.size(), templateDeclaration.isVariadic(), nullptr))) {
            if (printDebug && mErrorLogger) {
                std::list<const Token*> callstack(1, tok2);
                mErrorLogger->reportErr(ErrorMessage(callstack,
                                                     &mTokenList,
                                                     Severity::debug,
                                                     "debug",
                                                     "Failed to instantiate template \"" + instantiation.name() +
        ...
        const std::string newFullName(templateDeclaration.scope() +
                                      (templateDeclaration.scope().empty() ? "" : " :: ") + newName);

        if (expandedtemplates.insert(newFullName).second) {
            expandTemplate(
                templateDeclaration, instantiation, typeParametersInDeclaration, newName, !specialized && !isVar);
            instantiated = true;
            mChanged = true;
        }
    ...
    if (!instantiated && specialized) {
        ...
        // if (Token::Match(startToken->previous(), ";|{|}|=|const") &&
        //     (!specialized && !instantiateMatch(tok2, typeParametersInDeclaration.size(), isfunc ? "(" : isVar ?
        //     ";|%op%|(" : "*|&|::| %name%"))) return false;
        ...
        if (typeForNewName.empty()) {
            if (printDebug && mErrorLogger) {
                ...
                mErrorLogger->reportErr(ErrorMessage(callstack,
                                                     ...
                                                     Severity::debug,
                                                     "debug",
                                                     "Failed to instantiate template \"" + templateDeclaration.name() +
                                                         "\". The checking continues anyway.",
                                                     Certainty::normal));
            }
            return false;
        ...
        if (expandedtemplates.insert(newFullName).second) {
            expandTemplate(
                templateDeclaration, templateDeclaration, typeParametersInDeclaration, newName, !specialized && !isVar);
            instantiated = true;
            mChanged = true;
        }
                ...
                if (forwardDecl.fullName() == decl.fullName()) {
                    ...
                    for (size_t k = 0; k < params1.size(); k++) {
                        ...
                        if (params1[k]->strAt(1) == "=" && params2[k]->strAt(1) != "=") {
                            ...
                            if (end)
                                TokenList::copyTokens(
                                    const_cast<Token*>(params2[k]), params1[k]->next(), end->previous());
                            break;
                        }
                    }
            ...
            if (mapItem.first == decl.token()) {
                ...
                for (const auto& forwardDecl : mTemplateForwardDeclarations) {
                    if (mapItem.second == forwardDecl.token()) {
                        ...
                        std::cout << "    mTemplateDeclarations[" << declIndex << "] => mTemplateForwardDeclarations["
                                  << forwardIndex << "]" << std::endl;
                        break;
                    }
                    forwardIndex++;
                }
            ...
            if (decl1.isSpecialization() && mapItem.first == decl1.token()) {
                ...
                for (const auto& decl2 : mTemplateDeclarations) {
                    if (mapItem.second == decl2.token()) {
                        ...
                        std::cout << "    mTemplateDeclarations[" << decl1Index << "] => mTemplateDeclarations["
                                  << decl2Index << "]" << std::endl;
                        found = true;
                        break;
                    }
...
void TemplateSimplifier::simplifyTemplates(const std::time_t maxtime, bool& codeWithTemplates)
{
    ...
    for (; passCount < passCountMax; ++passCount) {
        ...
        for (std::list<TokenAndName>::reverse_iterator iter1 = mTemplateDeclarations.rbegin();
             iter1 != mTemplateDeclarations.rend();
             ++iter1) {
            ...
            for (std::list<TokenAndName>::const_iterator iter2 = mTemplateDeclarations.begin();
                 iter2 != mTemplateDeclarations.end();
                 ++iter2) {
            ...
            const bool instantiated =
                simplifyTemplateInstantiations(*iter1, specializations, maxtime, expandedtemplates);
            if (instantiated)
                mInstantiatedTemplates.push_back(*iter1);
        }

        for (std::list<TokenAndName>::const_iterator it = mInstantiatedTemplates.begin();
             it != mInstantiatedTemplates.end();
             ++it) {
        ...
        while (!mMemberFunctionsToDelete.empty()) {
            const std::list<TokenAndName>::iterator it =
                std::find_if(mTemplateDeclarations.begin(),
            ...
            if (it != mTemplateDeclarations.end()) {
                ...
                const std::list<TokenAndName>::iterator it1 =
                    std::find_if(mTemplateForwardDeclarations.begin(),
