        ...
        for (const Token* tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
            ...
            for (const ValueFlow::Value& value : tok->values()) {
                ...
                if (Token::Match(tok, "%name% . %name% (") &&
                    container->getYield(tok->strAt(2)) == Library::Container::Yield::START_ITERATOR) {
                ...
                if (container->arrayLike_indexOp && Token::Match(parent, "[")) {
                    const ValueFlow::Value* indexValue =
                        parent->astOperand2() ? parent->astOperand2()->getMaxValue(false) : nullptr;
                    if (indexValue && indexValue->intvalue >= value.intvalue) {
                        outOfBoundsError(parent,
                                         tok->expressionString(),
                                         &value,
                                         parent->astOperand2()->expressionString(),
                                         indexValue);
                        continue;
                    }
                    if (mSettings->severity.isEnabled(Severity::warning)) {
                                ...
                                const ValueFlow::Value* indexValue)
{
    ...
    if (!containerSize)
        ...
        if (containerSize->condition)
            errmsg = ValueFlow::eitherTheConditionIsRedundant(containerSize->condition) + " or expression '" +
                     expression + "' cause access out of bounds.";
        else if (indexValue == nullptr && !index.empty())
            errmsg = "Out of bounds access in expression '" + expression + "' because '$symbol' is empty and '" +
                     index + "' may be non-zero.";
        else
            errmsg = "Out of bounds access in expression '" + expression + "' because '$symbol' is empty.";
    } else if (indexValue) {
        if (containerSize->condition)
            errmsg = ValueFlow::eitherTheConditionIsRedundant(containerSize->condition) + " or $symbol size can be " +
                     MathLib::toString(containerSize->intvalue) + ". Expression '" + expression +
            ...
            errmsg = ValueFlow::eitherTheConditionIsRedundant(indexValue->condition) + " or '" + index +
                     "' can have the value " + indexValueString(*indexValue) + ". Expression '" + expression +
                     "' cause access out of bounds.";
        else
            errmsg = "Out of bounds access in '" + expression + "', if '$symbol' size is " +
                     MathLib::toString(containerSize->intvalue) + " and '" + index + "' is " +
    ...
    reportError(errorPath,
                (containerSize && !containerSize->errorSeverity()) || (indexValue && !indexValue->errorSeverity())
                    ? Severity::warning
                    : Severity::error,
...
void CheckStl::iteratorsError(const Token* tok, const std::string& containerName1, const std::string& containerName2)
{
    reportError(tok,
                ...
                "$symbol:" + containerName1 +
                    ...
                    containerName1 + "' and '" + containerName2 + "'.",
                CWE664,
                Certainty::normal);
}
...
void CheckStl::iteratorsError(const Token* tok, const Token* containerTok, const std::string& containerName)
{
    ...
    reportError(callstack,
                ...
                "$symbol:" + containerName +
                    ...
                    containerName + "' that are defined in different scopes.",
                CWE664,
                Certainty::normal);
}
...
static bool isIterator(const Variable* var, bool& inconclusiveType)
{
    ...
    if (!var || !var->isLocal() ||
        !Token::Match(var->typeEndToken(), "iterator|const_iterator|reverse_iterator|const_reverse_iterator|auto"))
        return false;
...
enum OperandPosition { Left, Right };
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        ...
        for (const Token* tok2 = var->nameToken(); tok2 && tok2 != var->scope()->bodyEnd; tok2 = tok2->next()) {
            ...
            if (Token::Match(tok2, "%name% . insert|erase ( *| %varid% )|,", iteratorId) && !isVector(tok2)) {
                ...
                else if (tok2->strAt(2) == "erase" &&
                         (tok2->strAt(4) != "*" || (containerToken && tok2->varId() == containerToken->varId()))) {
                    validIterator = false;
                    eraseToken = tok2;
...
void CheckStl::mismatchingContainerExpressionError(const Token* tok1, const Token* tok2)
{
    ...
    reportError(tok1,
                ...
                "Iterators to containers from different expressions '" + expr1 + "' and '" + expr2 +
                    "' are used together.",
                CWE664,
                Certainty::normal);
}
...
void CheckStl::mismatchingContainers()
{
    ...
    for (const Variable* var : symbolDatabase->variableList()) {
        if (var && var->isStlStringType() && Token::Match(var->nameToken(), "%var% (") &&
            Token::Match(var->nameToken()->tokAt(2), pattern2.c_str())) {
...
void CheckStl::stlOutOfBounds()
{
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        visitAstNodes(condition, [&](const Token* cond) {
        ...
        for (const Token* cond : conds) {
            ...
            const Library::Container* container =
                containerToken->valueType() ? containerToken->valueType()->container : nullptr;
            if (!container)
                continue;
...
void CheckStl::stlOutOfBoundsError(const Token* tok, const std::string& num, const std::string& var, bool at)
{
    if (at)
        reportError(tok,
                    Severity::error,
                    "stlOutOfBounds",
                    "$symbol:" + var + "\nWhen " + num + "==$symbol.size(), $symbol.at(" + num + ") is out of bounds.",
                    CWE788,
                    Certainty::normal);
    else
        reportError(tok,
                    Severity::error,
                    "stlOutOfBounds",
                    "$symbol:" + var + "\nWhen " + num + "==$symbol.size(), $symbol[" + num + "] is out of bounds.",
                    CWE788,
                    Certainty::normal);
}
...
static bool if_findCompare(const Token* const tokBack, bool stdStringLike)
{
    ...
    if (tok->str() == ".")
        return true; // Dereferencing is OK, the programmer might know that the element exists - TODO: An inconclusive
...
void CheckStl::if_find()
{
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        for (const Token* tok = conditionStart; tok->str() != "{"; tok = tok->next()) {
            ...
            // check also for vector-like or pointer containers
            else if (tok->variable() && tok->astParent() &&
                     (tok->astParent()->str() == "*" || tok->astParent()->str() == "[")) {
                ...
                if (tok->variable()->isArrayOrPointer())
                    ...
                    container = mSettings->library.detectContainer(tok->variable()->typeStartToken()); // outer
                                                                                                       // container
                    tok2 =
                        Token::findsimplematch(tok->variable()->typeStartToken(), "<", tok->variable()->typeEndToken());
                                    ...
                                    const Library& library)
{
    ...
    if (Token::simpleMatch(top, "(") && Token::Match(top->astOperand1(), ". insert|emplace (") &&
        !astIsIterator(top->astOperand1()->tokAt(2))) {
        icontainerTok = top->astOperand1()->astOperand1();
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            ...
            if (mSettings->standards.cpp < Standards::CPP17 &&
                !(keyTok && keyTok->valueType() &&
                  (keyTok->valueType()->isIntegral() || keyTok->valueType()->pointer > 0)))
                continue;
...
void CheckStl::size()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart->next(); tok != scope->bodyEnd; tok = tok->next()) {
            if (Token::Match(tok, "%var% . size ( )") || Token::Match(tok, "%name% . %var% . size ( )")) {
                // get the variable
...
void CheckStl::redundantCondition()
{
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        if (var1->str() == var2->str() && var2->str() == var3->str() && any1->str() == any2->str()) {
...
namespace {
const std::set<std::string> stl_string_stream = {"istringstream", "ostringstream", "stringstream", "wstringstream"};
}
...
void CheckStl::string_c_str()
{
    ...
    for (const Scope& scope : symbolDatabase->scopeList) {
        ...
        for (const Token* tok = scope.bodyStart; tok && tok != scope.bodyEnd; tok = tok->next()) {
            ...
            if (Token::Match(tok, "throw %var% . c_str|data ( ) ;") && isLocal(tok->next()) &&
                ...
                if (Token::Match(tok->tokAt(2), "%var% . str ( ) . c_str|data ( ) ;")) {
                    ...
                    if (var->isPointer() && var2 && var2->isStlType(stl_string_stream))
                        string_c_strError(tok);
                           ...
                           Token::Match(tok->linkAt(3), ") . c_str|data ( ) ;") && tok->tokAt(2)->function() &&
                           Token::Match(tok->tokAt(2)->function()->retDef, "std :: string|wstring %name%")) {
                    const Variable* var = tok->variable();
                    if (var->isPointer())
                        string_c_strError(tok);
                }
            } else if (printPerformance && tok->function() && Token::Match(tok, "%name% ( !!)") &&
                       c_strFuncParam.find(tok->function()) != c_strFuncParam.end() && tok->str() != scope.className) {
                const std::pair<std::multimap<const Function*, int>::const_iterator,
                                std::multimap<const Function*, int>::const_iterator>
                    range = c_strFuncParam.equal_range(tok->function());
                for (std::multimap<const Function*, int>::const_iterator i = range.first; i != range.second; ++i) {
                    ...
                    if (tok2 && Token::Match(tok2->tokAt(-4), ". c_str|data ( )")) {
                        ...
                        } else if (Token::Match(tok2->tokAt(-9), "%name% . str ( )")) { // Check ss.str().c_str() as
                                                                                        // parameter
            ...
            // Using c_str() to get the return value is only dangerous if the function returns a char*
            else if ((returnType == charPtr ||
                      (printPerformance && (returnType == stdString || returnType == stdStringConstRef))) &&
                     tok->str() == "return") {
                bool err = false;
                ...
                if (Token::Match(tok2, "std :: string|wstring (") &&
                    ...
                    err = true;
                } else if (Token::simpleMatch(tok2, "(") && Token::Match(tok2->link(), ") . c_str|data ( ) ;")) {
                    ...
                    for (const Token* search_tok = tok2->next(); search_tok != search_end;
                         search_tok = search_tok->next()) {
...
void CheckStl::string_c_strParam(const Token* tok, nonneg int number)
{
    ...
    oss << "Passing the result of c_str() to a function that takes std::string as argument no. " << number
...
namespace {
const std::set<std::string> stl_containers_with_empty_and_clear = {"deque",
                                                                   "forward_list",
                                                                   "list",
                                                                   "map",
                                                                   "multimap",
...
void CheckStl::uselessCalls()
{
    ...
    for (const Scope* scope : symbolDatabase->functionScopes) {
        for (const Token* tok = scope->bodyStart; tok != scope->bodyEnd; tok = tok->next()) {
            ...
            } else if (printPerformance && Token::Match(tok, "%var% . substr (") && tok->variable() &&
                       tok->variable()->isStlStringType()) {
            ...
            } else if (printWarning && Token::Match(tok, "[{};] %var% . empty ( ) ;") && !tok->tokAt(4)->astParent() &&
                       tok->next()->variable() &&
                       tok->next()->variable()->isStlType(stl_containers_with_empty_and_clear))
...
void CheckStl::uselessCallsReturnValueError(const Token* tok, const std::string& varname, const std::string& function)
{
    ...
    errmsg << "It is inefficient to call '" << varname << "." << function << "(" << varname
           ...
           << "code is inefficient. It is possible either the string searched ('" << varname << "') or searched for ('"
           << varname << "') is wrong.";
...
void CheckStl::uselessCallsSubstrError(const Token* tok, bool empty)
{
    if (empty)
        reportError(tok,
                    Severity::performance,
                    "uselessCallsSubstr",
                    "Ineffective call of function 'substr' because it returns an empty string.",
                    CWE398,
                    Certainty::normal);
    else
        reportError(
...
void CheckStl::checkDereferenceInvalidIterator()
{
    ...
    for (const Scope& scope : mTokenizer->getSymbolDatabase()->scopeList) {
        ...
        const bool isOrExpression = Token::findsimplematch(startOfCondition, "||", endOfCondition) != nullptr;
        const bool isAndExpression = Token::findsimplematch(startOfCondition, "&&", endOfCondition) != nullptr;
    ...
    for (const Token* tok = mTokenizer->tokens(); tok; tok = tok->next()) {
        ...
        for (const ValueFlow::Value& value : tok->values()) {
            ...
            if (!value.isIteratorValue())
                continue;
            const bool isInvalidIterator = (value.isIteratorEndValue() && value.intvalue >= 0) ||
                                           (value.isIteratorStartValue() && value.intvalue < 0);
...
void CheckStl::dereferenceInvalidIteratorError(const Token* tok, const ValueFlow::Value* value, bool inconclusive)
{
    ...
    const std::string errmsgcond("$symbol:" + varname + '\n' +
                                 ...
                                 " or there is possible dereference of an invalid iterator: $symbol.");
    if (!tok || !value) {
        reportError(tok,
                    Severity::error,
                    "derefInvalidIterator",
                    "Dereference of an invalid iterator",
                    CWE825,
                    Certainty::normal);
        reportError(
            tok, Severity::warning, "derefInvalidIteratorRedundantCheck", errmsgcond, CWE825, Certainty::normal);
        return;
    }
...
void CheckStl::readingEmptyStlContainerError(const Token* tok, const ValueFlow::Value* value)
{
    ...
    if (value && value->condition)
        errmsg = "Reading from container '$symbol'. " + ValueFlow::eitherTheConditionIsRedundant(value->condition) +
                 " or '$symbol' can be empty.";
    else
    ...
    reportError(errorPath,
                value ? (value->errorSeverity() ? Severity::error : Severity::warning) : Severity::style,
                "reademptycontainer",
                "$symbol:" + varname + "\n" + errmsg,
                CWE398,
                !value ? Certainty::inconclusive : Certainty::normal);
}
...
void CheckStl::useStlAlgorithmError(const Token* tok, const std::string& algoName)
{
    reportError(tok,
                Severity::style,
                "useStlAlgorithm",
                "Consider using " + algoName + " algorithm instead of a raw loop.",
                CWE398,
                Certainty::normal);
}
...
static const Token* singleAssignInScope(const Token* start, nonneg int varid, bool& input)
{
    ...
    if (isVariableChanged(assignTok->next(), endStatement, varid, false, nullptr, true))
        return nullptr;
    input =
        Token::findmatch(assignTok->next(), "%varid%", endStatement, varid) || !Token::Match(start->next(), "%var% =");
    return assignTok;
}
...
{
    if (Token::Match(tok, "+= %any% ;") && tok->tokAt(1)->hasKnownIntValue() && tok->tokAt(1)->getValue(1)) {
        return true;
    }
    if (Token::Match(tok, "= %varid% + %any% ;", varid) && tok->tokAt(3)->hasKnownIntValue() &&
...
{
    if (Token::Match(tok, "%assign% %bool% ;") && tok->tokAt(1)->hasKnownIntValue()) {
        return true;
    }
    if (Token::Match(tok, "= %varid% %oror%|%or%|&&|& %bool% ;", varid) && tok->tokAt(3)->hasKnownIntValue()) {
        return true;
    }
    return false;
}
        ...
        for (const Token* tok = function->bodyStart; tok != function->bodyEnd; tok = tok->next()) {
            ...
            if (assignTok) {
                ...
                if (assignVarId == loopVar->varId()) {
                    ...
                    else if (Token::Match(assignTok, "= %var% <|<=|>=|> %var% ? %var% : %var%") &&
                             hasVarIds(assignTok->tokAt(6), loopVar->varId(), assignVarId))
                        algo = minmaxCompare(assignTok->tokAt(2),
                                             loopVar->varId(),
                                             assignVarId,
                                             assignTok->tokAt(5)->varId() == assignVarId);
                    else
                        algo = "std::accumulate";
                }
            ...
            if (memberAccessTok) {
                ...
                if (contVarId == loopVar->varId())
                    continue;
                if (memberCallTok->str() == "push_back" || memberCallTok->str() == "push_front" ||
            ...
            if (condBodyTok) {
                ...
                if (isEarlyExit(condBodyTok)) {
                    const Token* loopVar2 =
                        Token::findmatch(condBodyTok, "%varid%", condBodyTok->link(), loopVar->varId());
                    std::string algo;
                    if (loopVar2)
                        algo = "std::find_if";
                    else
...
void CheckStl::knownEmptyContainerError(const Token* tok, const std::string& algo)
{
    ...
    reportError(tok, Severity::style, "knownEmptyContainer", msg, CWE398, Certainty::normal);
}
