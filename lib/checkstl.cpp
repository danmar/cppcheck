...
void CheckStl::negativeIndexError(const Token *tok, const ValueFlow::Value &index)
{
    ...
    reportError(errorPath, index.errorSeverity() ? Severity::error : Severity::warning, "negativeContainerIndex", errmsg.str(), CWE786, index.isKnown() ? Certainty::normal : Certainty::inconclusive);
}
