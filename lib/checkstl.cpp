...
void CheckStl::useMemcpyError(const Token *tok, const bool cpp)
{
    const std::string expected = cpp ? "std::memcpy" : "memcpy";
    reportError(tok, Severity::performance,
                "useMemcpy",
                "Concider using " + expected + " instead of for-loop. Library implementation should work faster in general cases.");
}
