...
int main(int argc, char** argv)
{
    ...
    std::cout << "// $ cppcheck --enable=warning,information --inline-suppr --platform=unix64 " << testfile
              << std::endl;
...
static std::string functionCall(const std::string& functionName,
                                bool useretval,
                                int argNr,
                                int numberOfArgs,
                                const char badArg[])
...
static std::string testFunctionArg(const std::string& functionName,
                                   bool useretval,
                                   const char type[],
                                   int argNr,
                                   int numberOfArgs,
...
{
    std::ostringstream ostr;
    ostr << "void test__" << replaceCharInString(functionName, ':', '_') << "__arg" << argNr << "__" << type << "() {"
         << std::endl;
    if (*code)
        ostr << "  " << code << std::endl;
...
static void testfunction(const tinyxml2::XMLElement* node, const std::string& functionName)
{
    ...
    if (noreturn) {
        ...
        std::cout << "  if (cond) { x=100; " << functionCall(functionName, useretval, -1, numberOfArgs, "") << "; }"
                  << std::endl;
    ...
    for (int argNr = 1; argNr <= numberOfArgs; ++argNr) {
        for (const tinyxml2::XMLElement* child = node->FirstChildElement(); child;
            ...
            for (const tinyxml2::XMLElement* argCheck = child->FirstChildElement(); argCheck;
                 argCheck = argCheck->NextSiblingElement()) {
            ...
            if (notbool) {
                std::cout
                    << testFunctionArg(
                           functionName, useretval, "notbool", argNr, numberOfArgs, "", "invalidFunctionArgBool", "!x")
                    << std::endl
                    << std::endl;
            }

            if (notnull) {
                std::cout << testFunctionArg(
                                 functionName, useretval, "notnull", argNr, numberOfArgs, "", "nullPointer", "NULL")
                          << std::endl
                          << std::endl;
            }
            ...
            if (notuninit) {
                ...
                std::cout << testFunctionArg(
                                 functionName, useretval, "notuninit", argNr, numberOfArgs, code, "uninitvar", "x")
                          << std::endl
                          << std::endl;
            }
        }
