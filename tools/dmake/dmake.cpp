...
static void write_ossfuzz_makefile(std::vector<std::string> libfiles_prio, std::vector<std::string> extfiles)
{
    ...
    fout << '\n';
    fout << "# TODO: add *.c/*.cpp from ../samples\n";
    fout << "do-fuzz: oss-fuzz-client\n";
    fout << "\tmkdir -p corpus\n";
    fout << "\t./oss-fuzz-client -only_ascii=1 -timeout=3 -detect_leaks=0 corpus ../test/cli/fuzz-crash ../test/cli/fuzz-timeout\n";
    fout << '\n';
    fout << "# TODO: add *.c/*.cpp from ../samples\n";
    fout << "dedup-corpus: oss-fuzz-client\n";
    fout << "\tmv corpus corpus_\n";
    fout << "\tmkdir -p corpus\n";
    fout << "\t./oss-fuzz-client -only_ascii=1 -timeout=3 -detect_leaks=0 corpus corpus_ ../test/cli/fuzz-crash ../test/cli/fuzz-timeout -merge=1\n";
    fout << '\n';
    fout << "# jobs:\n";
    fout << "# ./oss-fuzz-client -only_ascii=1 -timeout=3 -detect_leaks=0 corpus ../test/cli/fuzz-crash ../test/cli/fuzz-timeout -workers=12 -jobs=9\n";
    fout << '\n';
    fout << "# minimize:\n";
