...
def get_cppcheck(cppcheck_path):
    ...
    for i in range(5):
        if os.path.exists(cppcheck_path):
            ...
            subprocess.call(
                ['git', 'clone', 'https://github.com/danmar/cppcheck.git', cppcheck_path])
    ...
    if os.path.exists(destfile):
        if os.path.isfile(destfile):
            ...
            print(
                'Error: ' +
                destfile +
                ' exists but it is not a file! Please check the path and delete it manually.')
            sys.exit(1)
        ...
        for name in files:
            ...
            try:
                ...
                try:
                    ...
                    # Python3 directly reads the data into a string object that
                    # has no decode()
                    pass
                f.close()
...
def scan_package(work_path, cppcheck_path, jobs):
    ...
    for library, includes in library_includes.items():
        if os.path.exists(os.path.join(cppcheck_path, 'cfg',
                                       library + '.cfg')) and has_include('temp', includes):
            libraries += ' --library=' + library

    # Reference for GNU C:
    # https://gcc.gnu.org/onlinedocs/cpp/Common-Predefined-Macros.html
    options = jobs + libraries + \
        ' -D__GNUC__ --check-library --inconclusive --enable=style,information --platform=unix64 --template=daca2 -rp=temp temp'
    ...
    print('cppcheck finished with ' + str(returncode) +
          ('' if sig_num == -1 else ' (signal ' + str(sig_num) + ')'))
    ...
    if returncode in (-11, -6, -4, -8, -7) or sig_num in (11, 6, 4, 8, 7):
        ...
        if cppcheck_path == 'cppcheck':
            ...
            cmd = 'gdb --batch --eval-command=run --eval-command=bt --return-child-result --args ' + \
                cppcheck_cmd + " -j1"
    ...
    return count, ''.join(issue_messages_list), ''.join(
        information_messages_list), elapsed_time, options
...
def split_results(results):
    ...
    for line in results.split('\n'):
        if line.endswith(']') and re.search(
                r': (error|warning|style|performance|portability|information|debug):', line):
            if w is not None:
                ret.append(w.strip())
            w = ''
        if w is not None:
...
for arg in sys.argv[1:]:
    ...
    if arg.startswith('--stop-time='):
        ...
        if max_packages is None:
            print('Error: Max. packages value "{}" is invalid. Must be a positive number or 0.'.format(
                arg_value))
            sys.exit(1)
        ...
        print(
            '  --work-path=path     Work folder path. Default path is ' +
            work_path)
...
if not check_requirements():
    sys.exit(1)
if bandwidth_limit and isinstance(bandwidth_limit, str):
    if subprocess.call(['wget', '--limit-rate=' + bandwidth_limit,
                        '-q', '--spider', 'cppcheck.osuosl.org']) is 2:
        print(
            'Error: Bandwidth limit value "' +
            bandwidth_limit +
            '" is invalid.')
        sys.exit(1)
    else:
...
while True:
    if max_packages:
        if packages_processed >= max_packages:
            print('Processed the specified number of {} package(s). Exiting now.'.format(
                max_packages))
            break
        else:
            print(
                'Processing package {} of the specified {} package(s).'.format(
    ...
    while len(package) == 0:
        print(
            "network or server might be temporarily down.. will try again in 30 seconds..")
        time.sleep(30)
        package = get_package(server_address)
    ...
    for ver in cppcheck_versions:
        ...
        c, errout, info, t, cppcheck_options = scan_package(
            work_path, current_cppcheck_dir, jobs)
        if c < 0:
            crash = True
            count += ' Crash!'
        else:
