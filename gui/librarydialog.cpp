...
public:
    FunctionListItem(QListWidget* view, CppcheckLibraryData::Function* function, bool selected)
...
LibraryDialog::LibraryDialog(QWidget* parent) : QDialog(parent), mUi(new Ui::LibraryDialog), mIgnoreChanges(false)
{
    mUi->setupUi(this);
...
LibraryDialog::~LibraryDialog() { delete mUi; }
...
void LibraryDialog::openCfg()
{
    ...
    const QString selectedFile =
        QFileDialog::getOpenFileName(this, tr("Open library file"), datadir, filter, &selectedFilter);
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        QMessageBox msg(
            QMessageBox::Critical, tr("Cppcheck"), tr("Cannot open file %1.").arg(selectedFile), QMessageBox::Ok, this);
        msg.exec();
        return;
    }
    ...
    for (CppcheckLibraryData::Function& function : mData.functions) {
        mUi->functions->addItem(new FunctionListItem(mUi->functions, &function, false));
    }
...
void LibraryDialog::saveCfg()
{
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        QMessageBox msg(
            QMessageBox::Critical, tr("Cppcheck"), tr("Cannot save file %1.").arg(mFileName), QMessageBox::Ok, this);
        msg.exec();
    }
}
...
void LibraryDialog::saveCfgAs()
{
    ...
    QString selectedFile = QFileDialog::getSaveFileName(this, tr("Save the library as"), path, filter);
    if (selectedFile.isEmpty())
        return;
...
void LibraryDialog::sortFunctions(bool sort)
{
    if (sort) {
        ...
        mUi->functions->clear();
        for (CppcheckLibraryData::Function& function : mData.functions) {
            mUi->functions->addItem(new FunctionListItem(mUi->functions, &function, selfunction == &function));
        }
...
void LibraryDialog::editArg()
{
    ...
    if (mUi->arguments->selectedItems().count() != 1)
        return;
    CppcheckLibraryData::Function::Arg& arg =
        function->args[mUi->arguments->row(mUi->arguments->selectedItems().first())];
