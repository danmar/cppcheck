...
void CheckOther::clarifyCalculationError(const Token* tok, const std::string& op)
{
    ...
    reportError(tok,
                Severity::style,
                "clarifyCalculation",
                "Clarify calculation precedence for " + op +
                    " and ?\n"
                    "Suspicious calculation. Please use parentheses to clarify the code. "
                    "The code " +
                    calc + " should be written as either " + s1 + " or " + s2 + ".");
}
...
void CheckOther::warningOldStylePointerCast()
{
    ...
    for (const Token* tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (!Token::Match(tok, "( const| %type% * ) %var%") && !Token::Match(tok, "( const| %type% * ) (| new"))
            continue;
            ...
            if (tok->tokAt(tokIdx)->varId() > 0) {
                ...
                if (declTok) {
                    if (Token::simpleMatch(declTok->next(), "[")) {
                        ...
                        if (!(Token::Match(declTok->next(), "= %str%")) &&
                            !(Token::simpleMatch(declTok->next(), "= {")) &&
                            !(Token::simpleMatch(declTok->next(), ";"))) {
                            if (Token::simpleMatch(declTok->next(), ",")) {
...
void CheckOther::checkSelfAssignment()
{
    ...
    while (tok) {
        if (Token::Match(tok->previous(), "[;{}]") && tok->varId() && tok->varId() == tok->tokAt(2)->varId() &&
...
void CheckOther::checkIncorrectLogicOperator()
{
    ...
    while (tok && endTok) {
        ...
        if (NULL !=
            (logicTok = Token::findmatch(
                 tok, "( %any% !=|==|<|>|>=|<= %any% ) &&|%oror% ( %any% !=|==|<|>|>=|<= %any% ) %any%", endTok))) {
        ...
        if (logicTok) {
            ...
            for (unsigned int i = 0; i < (sizeof(conditions) / sizeof(conditions[0])); i++) {
                if (!((conditions[i].position1 == NA) || (((conditions[i].position1 == First) && varFirst1) ||
                                                          ((conditions[i].position1 == Second) && !varFirst1))))
                    continue;

                if (!((conditions[i].position2 == NA) || (((conditions[i].position2 == First) && varFirst2) ||
                                                          ((conditions[i].position2 == Second) && !varFirst2))))
                    continue;
...
void CheckOther::sizeofForNumericParameterError(const Token* tok)
{
    reportError(tok,
                ...
                "Using sizeof with a numeric constant as function "
...
void CheckOther::sizeofForArrayParameterError(const Token* tok)
{
    reportError(tok,
                ...
                "Using sizeof for array given as function argument "
...
public:
    ...
    ScopeInfo* parent() { return _parent; }
...
public:
    ...
    public:
        ...
        /** is variable unused? */
        bool unused() const { return (!_read && !_write); }
    ...
    void clear() { _varUsage.clear(); }
    VariableMap& varUsage() { return _varUsage; }
    ...
    void erase(unsigned int varid) { _varUsage.erase(varid); }
...
void Variables::addVar(const Token* name, VariableType type, ScopeInfo* scope, bool write_)
{
    if (name->varId() > 0)
...
static int doAssignment(Variables& variables, const Token* tok, bool dereference, ScopeInfo* scope)
{
            ...
            if (var2) { // local variable (alias or read it)
                if (var1->_type == Variables::pointer) {
                    if (dereference)
                        ...
                        if (addressOf || var2->_type == Variables::array || var2->_type == Variables::pointer) {
                            bool replace = true;
    ...
    // char c[10]; a.b = c;
        ...
        if (Token::Match(tok->tokAt(2), "%var%")) {
            ...
            if (var2 && (var2->_type == Variables::array || var2->_type == Variables::pointer)) {
...
void CheckOther::functionVariableUsage()
{
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token* tok = tok1; tok; tok = tok->next()) {
            ...
            if (Token::Match(tok, "[;{}] static| %type% %var% ;|=") && !Token::Match(tok->next(), "return|throw")) {
                tok = tok->next();
                ...
                if (tok->isStandardType() || isRecordTypeWithoutSideEffects(tok->next())) {
                    variables.addVar(tok->next(), Variables::standard, info, tok->tokAt(2)->str() == "=" || isStatic);
                }
                tok = tok->next();
            }
            ...
            // std::string s; std::string s = "string";
                ...
                tok = tok->tokAt(3);
                variables.addVar(tok, Variables::standard, info, tok->next()->str() == "=" || isStatic);
            }
            ...
            else if (Token::Match(tok, "[;{}] static| %type% %var% ( %any% ) ;") && nextIsStandardType(tok)) {
                tok = tok->next();
            ...
            // int i[10]; int j[2] = { 0, 1 }; static int k[2] = { 2, 3 };
                ...
                if (tok->str() != "return" && tok->str() != "throw") {
                    ...
                    variables.addVar(nametok,
                                     isPointer ? Variables::pointerArray : Variables::array,
                                     info,
            ...
            // int * i(j); int * k(i); static int * l(i);
                ...
                if (varid > 0) {
                    ...
                    if (type == Variables::pointer) {
                        ...
                        if (var && (var->_type == Variables::array || var->_type == Variables::pointer))
            ...
            else if (Token::Match(tok, "free|g_free|kfree|vfree ( %var% )") || Token::Match(tok, "delete %var% ;") ||
            ...
            // assignment
                ...
                if (dereference) {
                    ...
                    // Consider allocating memory separately because allocating/freeing alone does not constitute using
                    // the variable
                    ...
                    if (var2) {
                        if (var2->_type == Variables::reference) {
                            ...
                            variables.read(tok->varId());
                        else if (tok->varId() != varid1 && var2->_type == Variables::standard && tok->strAt(-1) != "&")
            ...
            // function parameter
                ...
                variables.use(tok->next()->varId()); // use = read + write
            else if (Token::Match(tok, "[(,] (") && Token::Match(tok->next()->link(), ") %var% [,)]"))
        ...
        for (it = variables.varUsage().begin(); it != variables.varUsage().end(); ++it) {
            ...
            if (usage._type == Variables::pointerPointer || usage._type == Variables::pointerArray ||
...
{
    reportError(
        tok, Severity::style, "unreadVariable", "Variable '" + varname + "' is assigned a value that is never used");
}
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token* tok = scope->classStart; tok; tok = tok->next()) {
            ...
            if (indentlevel > 0 && Token::Match(tok, "[{};]")) {
                ...
                if ((tok1->str() == "return") || (tok1->str() == "throw") || (tok1->str() == "delete") ||
                    (tok1->str() == "goto") || (tok1->str() == "else"))
                    continue;
    ...
    for (const Token* tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (Token::Match(tok, "struct|union %type% {")) {
            ...
            if (Token::simpleMatch(tok->previous(), "extern"))
                continue;
            if ((!tok->previous() || Token::simpleMatch(tok->previous(), ";")) &&
                Token::Match(tok->tokAt(2)->link(), ("} ; " + tok->strAt(1) + " %var% ;").c_str()))
                continue;
        ...
        if (Token::Match(tok, "[{};(,] const| char *| %var% [;=,)]") ||
            ...
            for (const Token* tok2 = tok->next(); tok2; tok2 = tok2->next()) {
                ...
                if (isPointer && Token::Match(tok2, "[;{}] %var% = %any% [&|] ( * %varid% ) ;", tok->varId())) {
                    // it's ok with a bitwise and where the other operand is 0xff or less..
                    if (tok2->strAt(4) == "&" && tok2->tokAt(3)->isNumber() &&
                        MathLib::isGreater("0x100", tok2->strAt(3)))
                        continue;
...
void CheckOther::checkZeroDivision()
{
    for (const Token* tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (Token::Match(tok, "/ %num%") && MathLib::isInt(tok->next()->str()) &&
            MathLib::toLongNumber(tok->next()->str()) == 0L) {
            zerodivError(tok);
        } else if (Token::Match(tok, "div|ldiv|lldiv|imaxdiv ( %num% , %num% )") &&
                   MathLib::isInt(tok->tokAt(4)->str()) && MathLib::toLongNumber(tok->tokAt(4)->str()) == 0L) {
            zerodivError(tok);
        }
    }
}
...
void CheckOther::checkMathFunctions()
{
    for (const Token* tok = _tokenizer->tokens(); tok; tok = tok->next()) {
        ...
        if (tok->varId() == 0 && Token::Match(tok, "log|log10 ( %num% )") &&
            MathLib::isNegative(tok->tokAt(2)->str()) && MathLib::isInt(tok->tokAt(2)->str()) &&
            MathLib::toLongNumber(tok->tokAt(2)->str()) <= 0) {
            mathfunctionCallError(tok);
        }
        // case log(-2.0)
        else if (tok->varId() == 0 && Token::Match(tok, "log|log10 ( %num% )") &&
                 MathLib::isNegative(tok->tokAt(2)->str()) && MathLib::isFloat(tok->tokAt(2)->str()) &&
        ...
        else if (tok->varId() == 0 && Token::Match(tok, "acos|asin ( %num% )") &&
                 std::fabs(MathLib::toDoubleNumber(tok->tokAt(2)->str())) > 1.0) {
            mathfunctionCallError(tok);
        }
        // sqrt( x ): if x is negative the result is undefined
        else if (tok->varId() == 0 && Token::Match(tok, "sqrt|sqrtf|sqrtl ( %num% )") &&
                 MathLib::isNegative(tok->tokAt(2)->str())) {
            mathfunctionCallError(tok);
        }
        // atan2 ( x , y): x and y can not be zero, because this is mathematically not defined
        else if (tok->varId() == 0 && Token::Match(tok, "atan2 ( %num% , %num% )") &&
                 MathLib::isNullValue(tok->tokAt(2)->str()) && MathLib::isNullValue(tok->tokAt(4)->str())) {
            mathfunctionCallError(tok, 2);
        }
        // fmod ( x , y) If y is zero, then either a range error will occur or the function will return zero
        // (implementation-defined).
        else if (tok->varId() == 0 && Token::Match(tok, "fmod ( %num% , %num% )") &&
                 MathLib::isNullValue(tok->tokAt(4)->str())) {
            mathfunctionCallError(tok, 2);
        }
        // pow ( x , y) If x is zero, and y is negative --> division by zero
        else if (tok->varId() == 0 && Token::Match(tok, "pow ( %num% , %num% )") &&
                 MathLib::isNullValue(tok->tokAt(2)->str()) && MathLib::isNegative(tok->tokAt(4)->str())) {
            mathfunctionCallError(tok, 2);
        }
    }
}
...
void CheckOther::checkMisusedScopedObject()
{
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token* tok = scope->classStart; tok; tok = tok->next()) {
            ...
            if (Token::Match(tok, "[;{}] %var% (") && Token::simpleMatch(tok->tokAt(2)->link(), ") ;") &&
...
static bool expressionHasSideEffects(const Token* first, const Token* last)
{
    for (const Token* tok = first; tok != last->next(); tok = tok->next()) {
        ...
        else if (Token::Match(tok, "%var% (") && !(Token::Match(tok, "c_str|string") || tok->isStandardType()))
            return true;
    }
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token* tok = scope->classStart; tok && tok != scope->classStart->link(); tok = tok->next()) {
            if (Token::simpleMatch(tok, "if (") && tok->strAt(-1) != "else" &&
                ...
                while (Token::simpleMatch(tok1, "} else if (") && Token::simpleMatch(tok1->tokAt(3)->link(), ") {")) {
                    // get the expression from the token stream
    ...
    for (scope = symbolDatabase->scopeList.begin(); scope != symbolDatabase->scopeList.end(); ++scope) {
        ...
        for (const Token* tok = scope->classStart; tok && tok != scope->classStart->link(); tok = tok->next()) {
            if (Token::simpleMatch(tok, "if (") && tok->strAt(-1) != "else" &&
                ...
                // save if branch code
                std::string branch1 =
                    stringifyTokens(tok->next()->link()->tokAt(2), tok->next()->link()->next()->link()->previous());
        ...
        for (const Token* tok = scope->classStart; tok && tok != scope->classStart->link(); tok = tok->next()) {
            if (Token::Match(tok, "(|&&|%oror% %var% &&|%oror%|==|!=|<=|>=|<|>|-|%or% %var% )|&&|%oror%") &&
                ...
                duplicateExpressionError(tok->next(), tok->tokAt(3), tok->strAt(2));
                           ...
                           tok,
                           "(|&&|%oror% %var% . %var% &&|%oror%|==|!=|<=|>=|<|>|-|%or% %var% . %var% )|&&|%oror%") &&
...
void CheckOther::duplicateExpressionError(const Token* tok1, const Token* tok2, const std::string& op)
{
    ...
    reportError(toks,
                Severity::style,
                "duplicateExpression",
                "Same expression on both sides of \'" + op +
                    "\'.\n"
...
void CheckOther::alwaysTrueFalseStringCompare(const Token* tok, const std::string& str1, const std::string& str2)
{
    ...
    if (str1 == str2) {
        ...
        reportError(tok,
                    ...
                    "The compared strings, '" +
                        string1 + "' and '" + string2 +
                        "', are static and always different. "
                        "If the purpose is to compare these two strings, the comparison is unnecessary.");
    }
}
...
{
    reportError(
        tok, Severity::error, "dangerousUsageStrtol", "Invalid radix in call to strtol or strtoul. Must be 0 or 2-36");
}
...
void CheckOther::sprintfOverlappingDataError(const Token* tok, const std::string& varname)
{
    reportError(
        ...
        "The variable '" +
            varname +
            "' is used both as a parameter and as a destination in "
...
void CheckOther::passedByValueError(const Token* tok, const std::string& parname)
{
    reportError(tok,
                Severity::performance,
                "passedByValue",
                "Function parameter '" + parname +
                    "' should be passed by reference.\n"
                    "Parameter '" +
                    parname +
                    "' is passed as a value. It could be passed "
...
void CheckOther::variableScopeError(const Token* tok, const std::string& varname)
{
    reportError(tok,
                ...
                "variableScope",
                "The scope of the variable '" + varname +
                    "' can be reduced\n"
                    "The scope of the variable '" +
                    varname +
                    "' can be reduced. Warning: It can be unsafe "
...
void CheckOther::zerodivError(const Token* tok) { reportError(tok, Severity::error, "zerodiv", "Division by zero"); }
    ...
    if (tok) {
        if (numParam == 1)
            reportError(tok,
                        Severity::error,
                        "wrongmathcall",
                        "Passing value " + tok->tokAt(2)->str() + " to " + tok->str() + "() leads to undefined result");
        else if (numParam == 2)
            reportError(tok,
                        Severity::error,
                        "wrongmathcall",
                        "Passing value " + tok->tokAt(2)->str() + " and " + tok->tokAt(4)->str() + " to " + tok->str() +
...
void CheckOther::fflushOnInputStreamError(const Token* tok, const std::string& varname)
{
    reportError(tok,
                Severity::error,
                "fflushOnInputStream",
                "fflush() called on input stream \"" + varname + "\" may result in undefined behaviour");
}
...
{
    reportError(tok, Severity::warning, "sizeofCalculation", "Found calculation inside sizeof()");
}

void CheckOther::redundantAssignmentInSwitchError(const Token* tok, const std::string& varname)
{
    reportError(
        tok, Severity::warning, "redundantAssignInSwitch", "Redundant assignment of \"" + varname + "\" in switch");
}

void CheckOther::switchCaseFallThrough(const Token* tok)
{
    reportError(tok, Severity::style, "switchCaseFallThrough", "Switch falls through case without comment");
}

void CheckOther::selfAssignmentError(const Token* tok, const std::string& varname)
{
    reportError(tok, Severity::warning, "selfAssignment", "Redundant assignment of \"" + varname + "\" to itself");
}
...
void CheckOther::assignmentInAssertError(const Token* tok, const std::string& varname)
{
    reportError(tok,
                ...
                "Assert statement modifies '" + varname +
                    ...
                    "Variable '" +
                    varname +
                    "' is modified inside assert statement. "
...
void CheckOther::incorrectLogicOperatorError(const Token* tok, bool always)
{
    if (always)
        reportError(tok,
                    ...
                    "Mutual exclusion over || always evaluates to true. Did you intend to use && instead?");
    else
        reportError(tok,
                    Severity::warning,
                    "incorrectLogicOperator",
...
{
    reportError(
        tok, Severity::error, "unusedScopedObject", "instance of \"" + varname + "\" object destroyed immediately");
}
...
void CheckOther::incorrectStringCompareError(const Token* tok,
                                             ...
                                             const std::string& len)
{
    reportError(tok,
                ...
                "String literal " + string + " doesn't match length argument for " + func + "(" + len + ").");
}
...
void CheckOther::comparisonOfBoolWithIntError(const Token* tok, const std::string& varname)
{
    reportError(tok,
                ...
                "The expression \"!" +
                    varname + "\" is of type 'bool' but is compared against a non-zero 'int'.");
}
...
void CheckOther::unsignedLessThanZero(const Token* tok, const std::string& varname)
{
    reportError(tok,
                ...
                "Checking if unsigned variable '" + varname +
                    "' is less than zero.\n"
